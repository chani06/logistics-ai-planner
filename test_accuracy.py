...
# โหลดข้อมูล Punthai (แผนจริง) - ถ้ามี
print("\n📂 ตรวจสอบข้อมูล Punthai (แผนจริง)...")
try:
    punthai_file = 'Dc/Punthai_reference.xlsx'
    df_punthai = pd.read_excel(punthai_file)
    print(f"   ✅ พบไฟล์ Punthai: {len(df_punthai)} สาขา, {df_punthai['Trip'].nunique()} ทริป")
    
    # วิเคราะห์ Punthai
    print("\n📊 วิเคราะห์ Punthai:")
    print(f"   - จำนวนสาขา: {len(df_punthai)}")
    print(f"   - จำนวนทริป: {df_punthai['Trip'].nunique()}")
    print(f"   - เฉลี่ยสาขา/ทริป: {len(df_punthai) / df_punthai['Trip'].nunique():.1f}")
    
    # การใช้รถ
    if 'Vehicle_Type' in df_punthai.columns:
        print(f"\n🚛 การใช้รถใน Punthai:")
        for vehicle in df_punthai['Vehicle_Type'].unique():
            count = (df_punthai['Vehicle_Type'] == vehicle).sum()
            trips = df_punthai[df_punthai['Vehicle_Type'] == vehicle]['Trip'].nunique()
            print(f"   - {vehicle}: {count} สาขา ({trips} ทริป)")
except:
    print(f"   ⚠️  ไม่พบไฟล์ {punthai_file}")
    print("   💡 ถ้ามีไฟล์แผนจริง ให้ตั้งชื่อเป็น 'Punthai_reference.xlsx' แล้ววางในโฟลเดอร์ Dc/")
