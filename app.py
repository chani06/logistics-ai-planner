...
# หลักการ: จังหวัดที่ไปคนละทาง/ต้องข้ามภูเขา ห้ามรวมทริป
NO_CROSS_ZONE_PAIRS = [
    ('เพชรบูรณ์', 'ชัยภูมิ'),
    ('เพชรบูรณ์', 'เลย'),
    ('น่าน', 'พะเยา'),      # น่านไปสาย 101, พะเยาไปสาย 1 - คนละทาง
    ('พะเยา', 'แพร่'),      # พะเยาไปสาย 1, แพร่ไปสาย 11 - คนละทาง
    ('พะเยา', 'อุตรดิตถ์'), # พะเยาไปสาย 1, อุตรดิตถ์ไปสาย 11 - คนละทาง
    ('ตาก', 'สุโขทัย'),
    ('ใต้ฝั่งอันดามัน', 'ใต้ฝั่งอ่าวไทย'),
    ('กระบี่', 'สุราษฎร์ธานี'),
]
# หมายเหตุ: น่าน-แพร่ รวมได้ (ต้องผ่านแพร่ไปน่าน สาย 101)
# หมายเหตุ: แพร่-อุตรดิตถ์ รวมได้ (สาย 11 เดียวกัน)
...
def predict_trips(test_df, model_data, punthai_buffer=1.0, maxmart_buffer=1.10):
    ...
    # ==========================================
    print("🎯 กำลังจัดทริปใหม่แบบ Zone-Strict (LOGISTICS_ZONES + NO_CROSS_ZONE_PAIRS)...")
    
    # 1️⃣ คำนวณ LOGISTICS_ZONE ของแต่ละสาขา
    print("   📊 กำลังคำนวณ LOGISTICS_ZONE...")
    df['_logistics_zone'] = df.apply(
        lambda r: get_logistics_zone(r.get('_province', ''), r.get('_district', ''), r.get('_subdistrict', '')), 
        axis=1
    )
    df['_zone_priority'] = df['_logistics_zone'].apply(get_zone_priority)
