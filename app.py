    ...
    while all_codes:
        ...
        while all_codes:
            ...
            for code in all_codes:
                ...
                # 🆕 เช็คว่าจังหวัดใกล้เคียงกันหรือไม่ (กลุ่มเดียวกัน)
                same_province_group = is_nearby_province(seed_province, code_province)
                
                # 🚨 กฎการรวมข้ามภูมิภาค:
                # 1. ถ้าเป็นจังหวัดในกลุ่มเดียวกัน → อนุญาตให้รวมได้
                # 2. ถ้าเป็นภาคไกลต่างกลุ่ม → ห้ามรวมกับภาคกลาง
                # 3. ถ้าเป็นภาคกลาง → ห้ามรวมกับภาคไกลต่างกลุ่ม
                if not same_province_group:
                    if is_far_region and code_region == 'nearby':
                        continue  # ภาคไกลห้ามรวมกับภาคกลาง (ถ้าไม่ใช่กลุ่มเดียวกัน)
                    elif seed_region == 'nearby' and code_region in ['north', 'south', 'far']:
                        continue  # ภาคกลางห้ามรวมกับภาคไกล (ถ้าไม่ใช่กลุ่มเดียวกัน)
                ...
                # - จังหวัดกลุ่มเดียวกัน → ยืดหยุ่นระยะเป็น 60km
                ...
                if same_subdistrict:
                    ...
                    if dist_from_last > 80:
                        continue
                elif same_province_group:
                    # จังหวัดกลุ่มเดียวกัน → ยืดหยุ่นเป็น 60km
                    if dist_from_last > 60:
                        continue
                else:
                ...
                # 🔥 เลือกสาขา: 1) ตำบลเดียวกัน > 2) อำเภอเดียวกัน > 3) จังหวัดกลุ่มเดียวกัน > 4) ใกล้ที่สุด
                    ...
                    if st.session_state.processed and st.session_state.result_df is not None:
                        ...
                        try:
                            ...
                            for t in result_df['Trip'].unique():
                                ...
                                for code in trip_data['Code'].unique():
                                    if 'Province' in trip_data.columns:
                                        prov_data = trip_data[trip_data['Code'] == code]
                                        if len(prov_data) > 0 and pd.notna(prov_data['Province'].iloc[0]):
                                            trip_provinces.add(str(prov_data['Province'].iloc[0]))
                                
                                # เช็คว่าเป็นภาคไกลหรือไม่
                                is_far_trip = any(get_region_type(p) in ['north', 'south', 'far'] for p in trip_provinces)
                                
                                # 🔴 เงื่อนไขสีแดงแยกตามภูมิภาค:
                                # - ภาคไกล (เหนือ/ใต้/อีสาน): ต่ำกว่า 75% หรือเกิน 105%
                                # - ภาคกลาง (nearby): ต่ำกว่า 50% หรือเกิน 105%
                                if is_far_trip:
                                    # ภาคไกล: ต้องได้อย่างน้อย 75%
                                    min_util_threshold = 75
                                else:
                                    # ภาคกลาง: ขั้นต่ำ 50%
                                    min_util_threshold = 50
                                
                                if max_util > 105:  # เกิน 105%
                                    over_util_trips.add(t)
                                    failed_trips.add(t)
                                elif max_util < min_util_threshold:  # ต่ำกว่าเกณฑ์ขั้นต่ำ
                            ...
                            for t in result_df['Trip'].unique():
                                ...
                                is_far_trip = any(get_region_type(p) in ['north', 'south', 'far'] for p in trip_provinces)
                                min_util_threshold = 75 if is_far_trip else 50
                                
                                if max_util > 105 or max_util < min_util_threshold:
