...
# Model paths
MODEL_DIR = 'models'
MODEL_PATH = os.path.join(MODEL_DIR, 'trip_model.pkl')
...
# 2. HELPER FUNCTIONS
# ==========================================
def save_model(G, req, regions, learning_stats, source_files):
    """บันทึกโมเดลและข้อมูลลงไฟล์"""
    ...
    model_info = {
        ...
        'total_trips': learning_stats['total_trips'],
        'total_branches': learning_stats['total_branches'],
...
# 4. AI CORE
# ==========================================
def test_model_accuracy(G, df_test):
    """ทดสอบความแม่นยำของโมเดล - เทียบกับทริปจริง"""
    if 'Trip' not in df_test.columns:
        return None
    
    # จัดกลุ่มตาม Trip จริง
    actual_trips = {}
    for trip_id, group in df_test.groupby('Trip'):
        codes = sorted(group['Code'].unique())
        if len(codes) > 1:
            actual_trips[trip_id] = codes
    
    # ตรวจสอบว่า edges ในโมเดลตรงกับทริปจริงหรือไม่
    total_pairs = 0
    matched_pairs = 0
    missing_pairs = []
    
    for trip_id, codes in actual_trips.items():
    ...
    return {
        ...
        'missing_pairs': missing_pairs[:10],  # แสดงแค่ 10 คู่แรก
        'accuracy': accuracy
    }
...
def main():
    ...
    if st.button("🚀 เริ่มวางแผน", type="primary", use_container_width=True):
        if not test_file:
            st.error("❌ กรุณาอัปโหลดไฟล์ออเดอร์")
            return
        
        # ตรวจสอบว่ามีโมเดลหรือไม่
        if not os.path.exists(MODEL_PATH):
            st.error("❌ ยังไม่มีโมเดล - กรุณาไปที่แท็บ 'เทรนโมเดล' เพื่อสอนระบบก่อน")
            return
        
        with st.spinner("⏳ กำลังประมวลผล..."):
            # โหลดโมเดลที่บันทึกไว้
            G, const, regions, learning_stats, model_info = load_model()
            
            if G is None:
                st.error("❌ ไม่สามารถโหลดโมเดลได้")
                return
            
            # แสดงข้อมูลโมเดลที่โหลด
            st.info(f"📚 ใช้โมเดลจาก: {', '.join(model_info.get('source_files', [])[:3])}...")
            st.info(f"🔗 จำนวน edges: {G.number_of_edges()} | สาขา: {G.number_of_nodes()}")
            
            known_regions = len([k for k, v in regions.items() if v != 'UNKNOWN'])
            if known_regions > 0:
                st.success(f"✅ รู้จักภูมิภาค: {known_regions} สาขา")
            
            # แสดงสถิติ
            with st.expander("📊 ข้อมูลโมเดล", expanded=False):
                col1, col2, col3, col4 = st.columns(4)
                with col1:
                    st.metric("🚚 ทริปที่เรียนรู้", f"{learning_stats['total_trips']}")
                with col2:
                    st.metric("🏪 สาขาทั้งหมด", f"{learning_stats['total_branches']}")
                with col3:
