...
if missing_cols:
    ...
    # ตรวจสอบอีกครั้ง
    if 'Weight' not in df.columns or 'Cube' not in df.columns or 'Code' not in df.columns:
        print(f"\n❌ ยังขาดคอลัมน์สำคัญ!")
        print(f"ต้องการ: Code, Name, Weight, Cube")
        print(f"มีอยู่: {[c for c in df.columns if c in ['Code', 'Name', 'Weight', 'Cube']]}")
        sys.exit(1)
...
try:
    from app import predict_trips, MASTER_DATA
    import streamlit as st
    
    # โหลด Master Data (ใช้ st.cache_data)
    print("\n📖 โหลด Master Data...")
    @st.cache_data
    def load_master_from_file():
        ...
        try:
            ...
            print(f"✅ โหลด Master: {len(master)} รายการ")
            return master
        except:
            print("⚠️ ไม่พบ Master Data")
            return pd.DataFrame()
    
    # กำหนด MASTER_DATA
    import app
    app.MASTER_DATA = load_master_from_file()
    
    # สร้าง model_data เปล่า
    print("📚 สร้าง model_data...")
    model_data = {
        'model': None,
        'trip_pairs': set(),
        'branch_info': {},
    ...
    if len(summary_df) > 0:
        ...
        print(f"- ⚠️ ไม่มีทริป - อาจมีปัญหาในข้อมูล")
