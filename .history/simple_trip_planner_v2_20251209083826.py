...
def plan_trips_v2(df, master_data):
    ...
    # Phase 3.75: เรียงลำดับหมายเลขทริปใหม่และสร้าง Trip_No
    unique_trips = sorted([t for t in result_df['Trip'].unique() if t != 0])
    trip_mapping = {}
    trip_no_mapping = {}
    
    # นับจำนวนแต่ละประเภทรถ
    truck_counters = {'6W': 1, 'JB': 1, '4W': 1}
    
    for new_num, old_trip in enumerate(unique_trips, start=1):
        trip_mapping[old_trip] = new_num
        
        # หาประเภทรถของทริปนี้
        trip_data = result_df[result_df['Trip'] == old_trip]
        truck_type = trip_data['Truck'].iloc[0] if len(trip_data) > 0 else '4W'
        
        # สร้างรหัสทริป
        if truck_type == '6W':
            trip_no = f"6W{truck_counters['6W']:03d}"
            truck_counters['6W'] += 1
        elif truck_type == 'JB':
            trip_no = f"4WJ{truck_counters['JB']:03d}"
            truck_counters['JB'] += 1
        else:  # 4W
            trip_no = f"4W{truck_counters['4W']:03d}"
            truck_counters['4W'] += 1
        
        trip_no_mapping[new_num] = trip_no
    
    # อัพเดท Trip และเพิ่มคอลัมน์ Trip_No
    result_df['Trip'] = result_df['Trip'].map(lambda x: trip_mapping.get(x, x))
    result_df['Trip_No'] = result_df['Trip'].map(lambda x: trip_no_mapping.get(x, ''))
