...
def plan_trips_v2(df, master_data):
    ...
    # Phase 2.8: รวม single-branch 6W ที่อยู่ใกล้กัน (ระยะทาง < 100km)
    merge_count = 0
    single_6w_trips = []
    
    # หา single-branch 6W trips
    for trip in sorted(result_df['Trip'].unique()):
        if trip == 0:
            continue
        trip_data = result_df[result_df['Trip'] == trip]
        if len(trip_data) == 1 and trip_data['Truck'].iloc[0] == '6W':
            single_6w_trips.append({
                'trip': trip,
                'index': trip_data.index[0],
                'lat': trip_data['Latitude'].iloc[0],
                'lon': trip_data['Longitude'].iloc[0],
                'cube': trip_data['Cube'].iloc[0],
                'weight': trip_data['Weight'].iloc[0]
            })
    
    # รวม single-branch 6W ที่อยู่ใกล้กัน
    merged_trips = set()
    for i, trip1 in enumerate(single_6w_trips):
        if trip1['trip'] in merged_trips:
            continue
        
        for j, trip2 in enumerate(single_6w_trips[i+1:], start=i+1):
            if trip2['trip'] in merged_trips:
                continue
            
            # คำนวณระยะทาง
            if trip1['lat'] and trip2['lat']:
                dist = haversine_distance(trip1['lat'], trip1['lon'], trip2['lat'], trip2['lon'])
                
                # ถ้าอยู่ใกล้กัน (< 100km) และรวมกันได้ตาม limit
                if dist < MAX_DISTANCE_BETWEEN_BRANCHES:
                    total_cube = trip1['cube'] + trip2['cube']
                    total_weight = trip1['weight'] + trip2['weight']
                    
                    if total_cube <= LIMITS['6W']['max_c'] and total_weight <= LIMITS['6W']['max_w']:
                        # รวม trip2 เข้า trip1
                        result_df.at[trip2['index'], 'Trip'] = trip1['trip']
                        merged_trips.add(trip2['trip'])
                        merge_count += 1
                        
                        # อัพเดท cube และ weight ของ trip1 สำหรับการเช็ครอบถัดไป
                        trip1['cube'] = total_cube
                        trip1['weight'] = total_weight
    
    if merge_count > 0:
        print(f"🔗 รวม single-branch 6W ที่อยู่ใกล้กัน: {merge_count} ทริป")
