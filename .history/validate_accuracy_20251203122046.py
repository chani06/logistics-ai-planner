...
# สุ่มน้ำหนัก/คิวแบบสมจริง (ใกล้เคียงค่าจริง)
import random
random.seed(42)  # สำหรับความสม่ำเสมอ

test_data = []
for _, row in df_punthai.iterrows():
    # สุ่มน้ำหนัก 50-500 kg, คิว 0.1-2.0 CBM
    weight = random.uniform(50, 500)
    cube = random.uniform(0.1, 2.0)
    
    test_data.append({
        'Code': row['BranchCode'],
        'Name': row.get('BU', ''),
        'Province': '',  # จะดึงจาก Master ภายหลัง
