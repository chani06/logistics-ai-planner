...
def plan_trips_v2(df, master_data):
    ...
    # เรียงลำดับ: ระยะทาง (ไกล→ใกล้) → ภาค → จังหวัด → อำเภอ → ตำบล → ชื่อสาขา
    result_df = result_df.sort_values(
        by=['Distance_DC', 'Region', 'Province', 'District', 'Subdistrict', 'Base_Name'],
    ...
    # กำหนดเลขทริปและชื่อทริป (เช่น 4WJ001, 6W018)
    trip_counters = {'4W': 1, '4WJ': 1, 'JB': 1, '6W': 1}  # นับทริปแต่ละประเภท
    
    for trip_num, trip_indices, truck_type, all_punthai in trips:
        # กำหนดประเภทรถตามข้อมูล
        if truck_type == '4W' and not all_punthai:
            trip_type = '4WJ'  # 4W คละ
        elif truck_type == 'JB' and not all_punthai:
            trip_type = '4WJ'  # JB คละ → ใช้ 4WJ
        else:
            trip_type = truck_type  # 4W (PT), JB (PT), 6W
        
        # สร้างชื่อทริป เช่น 4WJ001, 6W018
        trip_name = f"{trip_type}{trip_counters[trip_type]:03d}"
        trip_counters[trip_type] += 1
        
        for idx in trip_indices:
            result_df.at[idx, 'Trip'] = trip_num  # เก็บเลขเพื่อ sorting
            result_df.at[idx, 'Trip_Name'] = trip_name  # ชื่อทริปสำหรับ export
...
def export_with_colors(result_df, output_file, original_file, sheet_name="2.Punthai"):
    ...
    # สีเหลืองเข้ม (ทอง) และส้มมอม
    gold = PatternFill(start_color='FFD700', end_color='FFD700', fill_type='solid')  # ทอง
    orange = PatternFill(start_color='FFCC99', end_color='FFCC99', fill_type='solid')  # ส้มมอม
    
    code_col = 3
    trip_col = 9
    
    # สร้าง mapping: code → trip_name
    code_to_trip_name = dict(zip(result_df['Code'].astype(str), result_df['Trip_Name']))
    code_to_trip_num = dict(zip(result_df['Code'].astype(str), result_df['Trip']))
    ...
    for row_idx in range(3, ws.max_row + 1):
        ...
        if code and code in code_to_trip_name:
            trip_num = code_to_trip_num[code]
            trip_name = code_to_trip_name[code]
            
            if current_trip != trip_num:
                current_trip = trip_num
                use_gold = not use_gold
            
            # ใช้สีสลับ: ทอง/ส้มมอม
            fill = gold if use_gold else orange
            for col in range(1, ws.max_column + 1):
                ws.cell(row=row_idx, column=col).fill = fill
            
            # เขียนชื่อทริป (4WJ001, 6W018) ลงคอลัมน์ 9
            ws.cell(row=row_idx, column=trip_col, value=trip_name)
