...
def process_dataframe(df):
    ...
    # ใช้ list comprehension แทน pandas operations เพื่อหลีกเลี่ยงปัญหา duplicate index
    exclude_indices = []
    for idx, row in df.iterrows():
        ...
        if 'Name' in df.columns and pd.notna(row['Name']):
            if any(k in str(row['Name']) for k in EXCLUDE):
                exclude_indices.append(idx)
    
    # ลบแถวที่ต้อง exclude
    df = df.drop(exclude_indices).reset_index(drop=True)
