...
def plan_trips_v2(df, master_data):
    ...
    # Phase 2.6: ตรวจสอบอีกครั้งหลังเปลี่ยน JB → 4W (อาจมีทริปที่เกิน limit)
    # Loop หลายรอบเพื่อให้แน่ใจว่าแยกทริปหมดแล้ว
    for round_num in range(3):  # ลูปสูงสุด 3 รอบ
        max_trip = int(result_df['Trip'].max())
        ...
        for trip in sorted(result_df['Trip'].unique()):
            ...
            # 4W เกิน limit → แยกเป็นหลายคัน
            if truck == '4W':
                cube_limit = LIMITS['4W']['max_c'] if all_punthai_trip else 3.5
                
                if total_cubes > cube_limit or len(trip_data) > LIMITS['4W']['max_drops']:
                    ...
                    for idx in trip_data.index:
                        ...
                        if (current_cubes + cube > cube_limit or current_drops >= LIMITS['4W']['max_drops']) and current_cubes > 0:
                            new_trip_num += 1
                            current_cubes = 0
                            current_drops = 0
            ...
            # JB เกิน limit → แยกเป็นหลายคัน
            elif truck == 'JB' and (total_cubes > LIMITS['JB']['max_c'] or len(trip_data) > LIMITS['JB']['max_drops']):
                found_over_limit = True
                new_trip_num = max_trip + 1
                current_cubes = 0
                current_drops = 0
