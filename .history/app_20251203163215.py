...
def predict_trips(test_df, model_data):
    ...
    def add_distance_and_sort(df):
        ...
        # เรียงลำดับภายในแต่ละทริป: Trip → ระยะทางจาก DC (ใกล้ไปไกล)
        # จะเรียงใหม่อีกครั้งหลังคำนวณระยะทางละเอียด
    ...
    def calculate_detailed_distances(df):
        ...
        for trip_num in df['Trip'].unique():
            ...
            # คำนวณระยะทางจาก DC สำหรับแต่ละสาขา
            branch_distances = []
            for code in trip_data['Code']:
                ...
                if len(m) > 0:
                    ...
                    if lat and lon:
                        ...
                        branch_distances.append((code, dist, lat, lon))
                    else:
                        branch_distances.append((code, 999999, 0, 0))
                else:
                    branch_distances.append((code, 999999, 0, 0))
            
            # เรียงตามระยะทาง (ใกล้→ไกล)
            branch_distances.sort(key=lambda x: x[1])
            codes = [x[0] for x in branch_distances]
    ...
    # เพิ่มคอลัมน์เตือนสำหรับทริปที่เกิน 12 สาขา (เตือนเฉพาะรถเล็ก)
    def check_branch_count(row):
        ...
        # เตือนเฉพาะรถเล็ก (4W, JB) ถ้าเกิน 12 สาขา
        # รถใหญ่ (6W) ไม่เตือนแม้เกิน
        if trip_branch_count > 12 and current_vehicle in ['4W', 'JB']:
            return f"⚠️ รถ{current_vehicle} เกิน 12 สาขา ({trip_branch_count} สาขา)"
        elif trip_branch_count > 12:
            return f"✅ {trip_branch_count} สาขา (6W รองรับได้)"
        else:
