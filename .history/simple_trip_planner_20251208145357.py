...
def simple_plan_trips(df, master_data):
    ...
    for idx, row in result_df.iterrows():
        ...
        if len(current_trip_codes) == 0:
            ...
            # 2. คำนวณขีดจำกัดคิว (ตามเดิม)
            if all_punthai:
                max_cubes = 5  # punthai ล้วน 5 คิว
            else:
                max_cubes = 4  # คละ 3-4 คิว
            
            # 3. เช็คว่าเกินคิวหรือไม่ (ตัดทริปเมื่อเกิน)
            if current_trip_cubes + cube > max_cubes:
