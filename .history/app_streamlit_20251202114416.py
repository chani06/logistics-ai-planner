...
def load_excel(content):
    try:
        ...
        # ลำดับความสำคัญในการเลือกชีต: 2.Punthai > 2. > punthai > order > history
...
def main():
    ...
    with col_clear2:
        if st.button("🔄 รีเซ็ตระบบ", help="ล้างข้อมูลเก่าและเริ่มใหม่"):
            st.cache_data.clear()
            st.rerun()
    ...
    # แสดงคำเตือนให้ผู้ใช้รู้ว่าถ้าอัปไฟล์ใหม่ต้องรีเซ็ต
    st.info("💡 หากอัปโหลดไฟล์ใหม่แล้วไม่เห็นการเปลี่ยนแปลง ให้กดปุ่ม '🔄 รีเซ็ตระบบ' ด้านบน")
