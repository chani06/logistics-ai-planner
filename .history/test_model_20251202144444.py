...
def load_excel_sheet(file_path, sheet_name=None):
    ...
    try:
        ...
        print(f"📄 โหลด sheet: {target_sheet}")
...
def process_dataframe(df):
    ...
    for col in df.columns:
        ...
        if col_str == 'BranchCode':
            ...
            rename_map[col] = 'Cube'
        # Partial match
        elif 'BRANCHCODE' in col_upper or 'รหัสสาขา' in col:
            rename_map[col] = 'Code'
...
def main():
    ...
    if not files:
        print(f"❌ ไม่พบไฟล์ในโฟลเดอร์ {dc_folder}")
        return
    
    print(f"\n📂 พบไฟล์ {len(files)} ไฟล์:")
    for f in files:
        print(f"  • {os.path.basename(f)}")
    
    # เลือกไฟล์แรกสำหรับทดสอบ
    test_file = files[0]
    print(f"\n🎯 ใช้ไฟล์: {os.path.basename(test_file)}")
    print("="*80)
    ...
    # เรียนรู้รูปแบบทริป
    print("\n📚 ขั้นตอนที่ 2: เรียนรู้รูปแบบทริป")
    trip_pairs, trip_details = learn_trip_patterns(df)
    
    # สร้างทริปใหม่
    print("\n🔨 ขั้นตอนที่ 3: สร้างทริปใหม่จากโมเดล")
    result = reconstruct_trips(df, trip_pairs)
    
    if result is None:
        print("❌ ไม่สามารถสร้างทริปได้")
        return
    ...
    if comparison:
        ...
        if comparison['accuracy'] >= 100:
            print("🎉 ยอดเยี่ยม! ความแม่นยำ 100% - พร้อมใช้งาน!")
        elif comparison['accuracy'] >= 95:
            print("✅ ดีมาก! ความแม่นยำ >= 95% - ใช้งานได้")
        elif comparison['accuracy'] >= 80:
            print("⚠️ พอใช้ ความแม่นยำ >= 80% - ควรปรับปรุง")
        else:
            print("❌ ต่ำเกินไป! ความแม่นยำ < 80% - ต้องแก้ไข")
        
        print(f"\nรายละเอียด:")
        print(f"  • ความแม่นยำ: {comparison['accuracy']:.2f}%")
        print(f"  • คู่ที่ตรงกัน: {comparison['matched_pairs']}/{comparison['total_pairs']}")
        print(f"  • ทริปเดิม: {comparison['original_trip_count']} ทริป")
        print(f"  • ทริปใหม่: {comparison['reconstructed_trip_count']} ทริป")
        print(f"  • ส่วนต่าง: {abs(comparison['original_trip_count'] - comparison['reconstructed_trip_count'])} ทริป")
