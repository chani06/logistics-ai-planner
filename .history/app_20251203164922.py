...
# จำนวนสาขาต่อทริป - ปรับเพื่อคิวเต็ม (ไม่จำกัดดรอปมาก)
MAX_BRANCHES_PER_TRIP = 18  # สูงสุด 18 สาขาต่อทริป (เพิ่มจาก 12)
TARGET_BRANCHES_PER_TRIP = 12  # เป้าหมาย 12 สาขาต่อทริป (เพิ่มจาก 9)
...
def predict_trips(test_df, model_data):
    ...
    st.text("Phase 1.25: ย้ายสาขาเดียวไปรวมกับทริปใกล้เคียง...")
    reassign_count = 0
    ...
    for trip_num in sorted(test_df['Trip'].unique()):
        ...
        if current_priority > allowed_priority or util_allowed > 130:
            if util_allowed <= 130:
                ...
                # ใส่ไม่ได้ → แยกเป็นหลายทริปเล็ก (เน้นเต็มรถ ~100%)
                # กลยุทธ์: เติมให้เต็มที่สุด ถ้าเกิน 5m³ → ขยับเป็น JB
                new_trips = []
                current_group = []
                current_group_w = 0
                current_group_c = 0
                current_vehicle_type = max_allowed  # เริ่มจากรถที่อนุญาต
                ...
                for _, row in sorted_data.iterrows():
                    ...
                    # ลองใส่ในกลุ่มปัจจุบัน
                    test_w = current_group_w + w
                    test_c = current_group_c + c
                    
                    # เป้าหมาย: เต็มรถ ~95-100% (ไม่เกิน 120%)
                    current_w_limit = LIMITS[current_vehicle_type]['max_w']
                    current_c_limit = LIMITS[current_vehicle_type]['max_c']
                    test_util = max((test_w / current_w_limit) * 100, (test_c / current_c_limit) * 100)
                    ...
                    if test_util <= 120 or len(current_group) == 0:
                        ...
                        # เต็มแล้ว → ลองอัพเกรดรถก่อน
                        upgraded = False
                        
                        # ถ้าใช้ 4W อยู่ และเกิน 5m³ → ลองขยับเป็น JB
                        if current_vehicle_type == '4W' and test_c > 5.0 and test_c <= 7.0:
                            # ลองใส่ JB
                            jb_util = max((test_w / LIMITS['JB']['max_w']) * 100, 
                                        (test_c / LIMITS['JB']['max_c']) * 100)
                            if jb_util <= 120:
                                # อัพเกรดเป็น JB ได้
                                current_vehicle_type = 'JB'
                                current_group.append(code)
                                current_group_w += w
                                current_group_c += c
                                upgraded = True
                        ...
                        if not upgraded:
                            # ไม่สามารถอัพเกรดได้ → บันทึกกลุ่มปัจจุบัน และเริ่มกลุ่มใหม่
                            new_trips.append({
                            ...
                            current_group_w = w
                            current_group_c = c
                            current_vehicle_type = max_allowed  # รีเซ็ตเป็นรถเล็ก
                ...
                if len(new_trips) > 1:
                    # ทริปแรกใช้เลขเดิม
                    for code in new_trips[0]['codes']:
                        test_df.loc[test_df['Code'] == code, 'Trip'] = trip_num
                    trip_recommended_vehicles[trip_num] = new_trips[0]['vehicle']
                    
                    # ทริปถัดไปสร้างใหม่
                    for trip_info in new_trips[1:]:
                        new_trip_num = test_df['Trip'].max() + 1
                        for code in trip_info['codes']:
                    ...
                    # มีแค่กลุ่มเดียว → ปรับรถตามที่คำนวณได้
                    trip_recommended_vehicles[trip_num] = new_trips[0]['vehicle']
                    fix_count += 1
