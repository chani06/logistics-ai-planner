...
EXCLUDE_BRANCHES = ['DC011', 'PTDC', 'DCวังน้อย']

# Distance thresholds for truck selection
MAX_DISTANCE_4W = 80.0       # 4 ล้อ ตู้ทึบ: ระยะทาง ≤ 80 km
MAX_DISTANCE_4W_JB = 150.0   # 4 ล้อ จัมโบ้: ระยะทาง ≤ 150 km
# 6 ล้อ: ระยะทาง > 150 km หรือน้ำหนัก/คิวเกิน
...
def is_nearby_province(prov1, prov2):
    """เช็คว่าจังหวัดใกล้เคียงกันหรือไม่"""
    ...
    # จัดกลุ่มจังหวัดที่ใกล้เคียงกัน
    province_groups = {
        'กรุงเทพและปริมณฑล': ['กรุงเทพ', 'นนทบุรี', 'ปทุมธานี', 'สมุทรปราการ', 'สมุทรสาคร', 'นครปฐม'],
        'ภาคกลางตะวันตก': ['ราชบุรี', 'กาญจนบุรี', 'สุพรรณบุรี', 'สมุทรสงคราม', 'เพชรบุรี', 'ประจวบคีรีขันธ์'],
        'ภาคกลางเหนือ': ['ชัยนาท', 'สิงห์บุรี', 'อ่างทอง', 'ลพบุรี', 'สระบุรี', 'อยุธยา', 'พระนครศรีอยุธยา'],
        'ภาคตะวันออก': ['ฉะเชิงเทรา', 'ชลบุรี', 'ระยอง', 'จันทบุรี', 'ตราด', 'ปราจีนบุรี', 'สระแก้ว'],
        'ภาคเหนือบน': ['เชียงใหม่', 'เชียงราย', 'ลำพูน', 'ลำปาง', 'พะเยา', 'แพร่', 'น่าน'],
        'ภาคเหนือล่าง': ['พิษณุโลก', 'พิจิตร', 'เพชรบูรณ์', 'สุโขทัย', 'อุตรดิตถ์', 'ตาก', 'กำแพงเพชร'],
        'อีสานเหนือ': ['อุดรธานี', 'หนองคาย', 'เลย', 'บึงกาฬ', 'สกลนคร', 'นครพนม', 'มุกดาหาร'],
        'อีสานกลาง': ['ขอนแก่น', 'มหาสารคาม', 'กาฬสินธุ์', 'ร้อยเอ็ด'],
        'อีสานใต้': ['นครราชสีมา', 'โคราช', 'ชัยภูมิ', 'บุรีรัมย์', 'สุรินทร์', 'ศีขรภูมิ', 'ยโสธร', 'อำนาจเจริญ', 'อุบลราชธานี'],
        'ภาคใต้บน': ['ชุมพร', 'สุราษฎร์ธานี', 'ระนอง', 'พังงา', 'ภูเก็ต', 'กระบี่', 'นครศรีธรรมราช'],
        'ภาคใต้ล่าง': ['ตรัง', 'พัทลุง', 'สงขลา', 'สตูล', 'ปัตตานี', 'ยะลา', 'นราธิวาส']
    }
...
def process_dataframe(df):
    ...
    if 'Code' in df.columns:
        ...
        # ตัดสาขาที่ชื่อมี 'DCวังน้อย' หรือ 'Distribution Center'
        if 'Name' in df.columns:
            df = df[~df['Name'].str.contains('DCวังน้อย|Distribution Center', case=False, na=False)]
...
def select_truck(w, c, min_rank, avg_distance=0, cube_utilization=0):
    ...
    กลยุทธ์:
    1. ใช้ 4W ถ้าใส่ได้พอดี และระยะทางไม่ไกลเกิน 80 km
    2. ใช้ 4W Jumbo ถ้าระยะทาง 80-150 km หรือน้ำหนัก/คิวเกิน 4W
    3. ใช้ 6W ถ้าระยะทางไกลมาก (>150km) หรือน้ำหนัก/คิวเกิน 4W Jumbo
    """
    ...
    # ถ้าระยะทางไกลมาก (> 150 km) → บังคับใช้ 6W
    if avg_distance > MAX_DISTANCE_4W_JB:
        return '6 ล้อ ตู้ทึบ'
    
    # ถ้าน้ำหนักหรือคิวเกิน 4W Jumbo → ใช้ 6W
    if w > LIMITS['JB']['max_w'] or c > LIMITS['JB']['max_c']:
        return '6 ล้อ ตู้ทึบ'
    
    # ถ้าระยะทางกลาง (80-150 km) → ใช้ 4W Jumbo ขึ้นไป
    if avg_distance > MAX_DISTANCE_4W:
        # ถ้าน้ำหนักและคิวพอใส่ 4W Jumbo
...
def predict_trips(test_df, model_data):
    ...
    trip_distances = {}  # เก็บระยะทางรวมของแต่ละทริป
...
def main():
                    ...
                    with col1:
                        with st.spinner("⏳ กำลังประมวลผล..."):
                            ...
                            with st.expander("📋 ดูรายละเอียดรายสาขา (พร้อมข้อมูลรถ)"):
                                ...
                                display_cols = ['Trip', 'Code', 'Name', 'Weight', 'Cube', 'Truck']
                                if 'Province' in result_df.columns:
                                    display_cols.insert(3, 'Province')
                                
                                display_df = result_df[display_cols].copy()
                                display_df.columns = ['ทริป', 'รหัส', 'ชื่อสาขา', 'น้ำหนัก(kg)', 'คิว(m³)', 'รถ'] if 'Province' not in result_df.columns else ['ทริบ', 'รหัส', 'ชื่อสาขา', 'จังหวัด', 'น้ำหนัก(kg)', 'คิว(m³)', 'รถ']
    ...
    with tab2:
        ...
        with col1:
            ...
            - เตรียมไฟล์ Excel (.xlsx) ที่มีข้อมูลสาขาและออเดอร์
            - ต้องมีคอลัมน์: รหัสสาขา, ชื่อสาขา, น้ำหนัก, คิว
            
            #### ขั้นตอนที่ 2: อัปโหลดไฟล์
            - คลิกปุ่ม "Browse files" ในแท็บ "จัดทริปส่งของ"
            - เลือกไฟล์ Excel ที่เตรียมไว้
            
            #### ขั้นตอนที่ 3: ตรวจสอบข้อมูล
            - ระบบจะแสดงสรุปข้อมูลที่อ่านได้
            - สามารถดูข้อมูลตัวอย่างได้
            
            #### ขั้นตอนที่ 4: จัดทริป
            - คลิกปุ่ม "เริ่มจัดทริป"
            - ระบบจะประมวลผลและแสดงผลลัพธ์
            
            #### ขั้นตอนที่ 5: ดาวน์โหลดผลลัพธ์
            - คลิกปุ่ม "ดาวน์โหลดผลลัพธ์"
            - ไฟล์ Excel จะมี 2 sheet: รายละเอียดทริป และ สรุปทริป
            """)
            ...
            ระบบจะจัดทริปโดยใช้กฎ **3 ลำดับความสำคัญ**:
            
            1. **📜 ประวัติการจัดส่ง** (ความแม่นยำ 100%)
               - ถ้าสาขาเคยไปด้วยกันในประวัติ → จัดเข้าทริปเดียวกัน
               - ใช้รถประเภทเดียวกับที่เคยใช้
            
            2. **🏢 ชื่อสาขาคล้ายกัน**
               - เช่น: นครราชสีมา1, นครราชสีมา2, นครราชสีมา3
               - จะจัดเข้าทริปเดียวกัน
            
            3. **🤖 ระบบทำนายอัจฉริยะ**
               - วิเคราะห์จากจังหวัด, น้ำหนัก, คิว, ระยะทาง
               - แนะนำการจัดทริปที่เหมาะสม
            """)
        
        with col2:
            st.image("https://raw.githubusercontent.com/twitter/twemoji/master/assets/svg/1f4e6.svg", width=150)
            ...
            **💡 เคล็ดลับ**
            ...
            - ระบบจะเลือกรถอัตโนมัติ
            - สีเขียว = ใช้รถเต็มประสิทธิภาพ
            - สีแดง = ใช้รถน้อย (อาจปรับปรุงได้)
            """)
