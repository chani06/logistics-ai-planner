...
def export_with_colors(result_df, output_file, original_file, sheet_name="2.Punthai"):
    """Export กลับไฟล์เดิมพร้อมสีเหลืองโทนส้ม-ขาว เขียนแค่สาขาและทริป"""
    ...
    # สีใหม่: เหลืองโทนส้ม-ขาว
    yellow_orange = PatternFill(start_color='FFD700', end_color='FFD700', fill_type='solid')
    white = PatternFill(start_color='FFFFFF', end_color='FFFFFF', fill_type='solid')
    
    code_col = 3      # Code column (สาขา)
    trip_col = 9      # Trip column
    
    # สร้าง mapping สำหรับ Trip เท่านั้น
    ...
    for row_idx in range(3, ws.max_row + 1):
        ...
        if code and code in code_to_trip:
            ...
            # สลับสีเหลืองโทนส้ม-ขาว
            fill = yellow_orange if use_yellow else white
            for col in range(1, ws.max_column + 1):
                ws.cell(row=row_idx, column=col).fill = fill
            
            # เขียนแค่ Trip (ไม่มี Trip_No)
