...
# - Punthai ล้วน: 4W max_c = 5.0
# - ผสม (Punthai + อื่น): 4W max_c = 4.0 (ถ้า cube 3-4 ใช้ 4W, ถ้าเกิน → JB)
LIMITS_4W_PUNTHAI_ONLY = 5.0   # Punthai ล้วน
LIMITS_4W_MIXED = 4.0          # ผสม BU

# 🔒 ขีดจำกัดสำหรับ Punthai ล้วน
# - JB (Jumbo): ไม่เกิน 7 drop, Cube ≤ 8
...
# 🚨 จำกัดระยะห่างสาขาในทริปเดียวกัน (เข้มงวด!)
MAX_DISTANCE_IN_TRIP = 30  # km - สาขาในทริปเดียวกันห้ามห่างกันเกิน 30km
...
# Performance Config - Optimized for < 30 seconds
...
MAX_PROCESSING_TIME = 20  # วินาที - เป้าหมาย 30 วินาที รวม
...
def predict_trips(test_df, model_data):
    ...
    def get_distance_between(code1, code2):
        """คำนวณระยะทางระหว่าง 2 สาขา"""
    ...
    # 4. ตัดทริปใหม่เมื่อ: เต็ม / ระยะเกิน / ข้ามภาค-จังหวัด
    # 5. หาสาขาที่ยังไม่ได้จัดที่ไกลสุดเป็น seed ใหม่
    
    def farthest_first_nearest_neighbor(codes):
        """
        🚀 Algorithm: Farthest First + Nearest Neighbor
        สร้างทริปโดยเริ่มจากสาขาที่ไกลจาก DC ที่สุด แล้วหาสาขาใกล้สุดในพื้นที่เดียวกัน
        """
        if not codes:
            return []
        
        # เรียงตามระยะทางจาก DC (ไกลสุดอยู่หน้า)
        sorted_by_dist_from_dc = sorted(
            codes,
            key=lambda c: get_distance_from_dc(c),
            reverse=True  # ไกลสุดอยู่หน้า
        )
        
        result = []
        remaining = set(sorted_by_dist_from_dc)
        
        while remaining:
            # 🎯 หาสาขาที่ไกลจาก DC ที่สุด (ยังไม่ได้จัด) เป็น seed
            remaining_sorted = sorted(remaining, key=lambda c: get_distance_from_dc(c), reverse=True)
            seed = remaining_sorted[0]
            remaining.remove(seed)
            
            trip_codes = [seed]
            seed_province = get_province(seed)
            seed_lat, seed_lon = coord_cache.get(seed, (None, None))
            current_lat, current_lon = seed_lat, seed_lon
            
            # 🔄 หาสาขาใกล้สุดจาก seed (same province + ระยะใกล้)
            while remaining:
                # หาสาขาที่:
                # 1. อยู่ในจังหวัดเดียวกัน (หรือใกล้เคียง)
                # 2. ระยะจากสาขาปัจจุบัน <= MAX_DISTANCE_IN_TRIP
                # 3. ระยะจากทุกสาขาในทริป <= MAX_DISTANCE_IN_TRIP
                ...
                for code in remaining:
                    ...
                    if not current_lat or not code_lat:
                        continue
                    
                    # ระยะจากสาขาปัจจุบัน
                    dist_from_current = haversine_distance(current_lat, current_lon, code_lat, code_lon)
                    
                    # เช็คระยะจากทุกสาขาในทริป
                    max_dist_in_trip = 0
                    ...
                    # 🎯 Priority: จังหวัดเดียวกัน > ระยะใกล้
                    same_province = (code_province == seed_province) or seed_province == 'UNKNOWN' or code_province == 'UNKNOWN'
                    
                    if same_province and dist_from_current <= MAX_DISTANCE_IN_TRIP:
                        # ให้คะแนนสูงสำหรับจังหวัดเดียวกัน + ระยะใกล้
                        candidates.append((code, dist_from_current, 0))  # priority 0 = best
                    elif dist_from_current <= MAX_DISTANCE_IN_TRIP * 0.5:
                        # ระยะใกล้มาก แม้ต่างจังหวัด (อาจเป็นขอบจังหวัด)
                        candidates.append((code, dist_from_current, 1))
                
                if not candidates:
                    break
                
                # เลือกสาขาที่ดีที่สุด: priority ก่อน แล้ว distance
                candidates.sort(key=lambda x: (x[2], x[1]))
                best_code = candidates[0][0]
                
                trip_codes.append(best_code)
                remaining.remove(best_code)
                current_lat, current_lon = coord_cache.get(best_code, (current_lat, current_lon))
            
            # เพิ่มทั้งทริปใน result
            result.extend(trip_codes)
        
        return result
    
    # 🚀 ใช้ Farthest First + Nearest Neighbor แทน spatial_clusters
    all_codes_list = []
    for cluster in spatial_clusters:
        all_codes_list.extend(cluster)
    
    # จัดเรียงใหม่ด้วย Farthest First + Nearest Neighbor
    all_codes = farthest_first_nearest_neighbor(all_codes_list)
    ...
    while all_codes:
        ...
        # 🚀 **NEW: ใช้สาขาที่อยู่ถัดไปใน all_codes โดยตรง**
        # เพราะ farthest_first_nearest_neighbor จัดเรียงสาขาที่ควรอยู่ด้วยกันไว้ติดกันแล้ว
        
        seed_province = get_province(seed_code)
        seed_lat, seed_lon = coord_cache.get(seed_code, (None, None))
        
        # หา Cube และ Weight ปัจจุบัน
        current_weight = test_df[test_df['Code'] == seed_code]['Weight'].sum()
        current_cube = test_df[test_df['Code'] == seed_code]['Cube'].sum()
        
        # กำหนดขีดจำกัด (เริ่มจาก JB เพราะเป็นขนาดกลาง)
        max_cube = LIMITS['JB']['max_c'] * BUFFER  # JB = 7 cube
        max_weight = LIMITS['JB']['max_w'] * BUFFER  # JB = 3500 kg
        
        # 🔄 Loop: เพิ่มสาขาถัดไปที่ยังใกล้กันและไม่เกิน capacity
        while all_codes:
            next_code = all_codes[0]  # ดูสาขาถัดไป (ยังไม่ pop)
            next_province = get_province(next_code)
            next_lat, next_lon = coord_cache.get(next_code, (None, None))
            
            # 1. เช็คระยะทางจาก seed
            dist_from_seed = 9999
            if seed_lat and next_lat:
                dist_from_seed = haversine_distance(seed_lat, seed_lon, next_lat, next_lon)
            
            # 2. เช็คระยะทางจากสาขาปัจจุบัน (สุดท้ายในทริป)
            last_code = current_trip[-1]
            last_lat, last_lon = coord_cache.get(last_code, (None, None))
            dist_from_last = 9999
            if last_lat and next_lat:
                dist_from_last = haversine_distance(last_lat, last_lon, next_lat, next_lon)
            
            # 3. เช็คระยะทางจากทุกสาขาในทริป
            max_dist_in_trip = 0
            for trip_code in current_trip:
                trip_lat, trip_lon = coord_cache.get(trip_code, (None, None))
                if trip_lat and next_lat:
                    dist = haversine_distance(next_lat, next_lon, trip_lat, trip_lon)
                    max_dist_in_trip = max(max_dist_in_trip, dist)
            
            # 4. เงื่อนไขตัดทริปใหม่:
            should_cut = False
            
            # 4.1 ระยะห่างจากสาขาสุดท้ายเกิน MAX_DISTANCE_IN_TRIP → ตัด
            if dist_from_last > MAX_DISTANCE_IN_TRIP:
                should_cut = True
            
            # 4.2 ระยะห่างจากสาขาใดๆ ในทริปเกิน MAX_DISTANCE_IN_TRIP → ตัด
            if max_dist_in_trip > MAX_DISTANCE_IN_TRIP:
                should_cut = True
            
            # 4.3 ต่างจังหวัดและไม่ใกล้มาก (> 15km) → ตัด
            if seed_province != 'UNKNOWN' and next_province != 'UNKNOWN':
                if seed_province != next_province and dist_from_last > 15:
                    should_cut = True
            
            # 4.4 เช็ค capacity
            next_weight = test_df[test_df['Code'] == next_code]['Weight'].sum()
            next_cube = test_df[test_df['Code'] == next_code]['Cube'].sum()
            
            # ตรวจสอบว่าเพิ่มแล้วจะเกิน capacity หรือไม่
            new_weight = current_weight + next_weight
            new_cube = current_cube + next_cube
            
            # Auto-upgrade vehicle ถ้าจำเป็น
            if new_cube > LIMITS['JB']['max_c'] * BUFFER or new_weight > LIMITS['JB']['max_w'] * BUFFER:
                # ลองใช้ 6W
                max_cube = LIMITS['6W']['max_c'] * BUFFER
                max_weight = LIMITS['6W']['max_w'] * BUFFER
            
            if new_cube > max_cube or new_weight > max_weight:
                should_cut = True
            
            # 🎯 ถ้าต้องตัดทริป → break ออกจาก loop
            if should_cut:
                break
            
            # ✅ ผ่านทุกเงื่อนไข → เพิ่มสาขานี้เข้าทริป
            all_codes.pop(0)
            current_trip.append(next_code)
            assigned_trips[next_code] = trip_counter
        ...
        for code in remaining_sorted:
            ...
            if len(current_trip) >= 3:  # เช็คเฉพาะเมื่อทริปมีสาขา >= 3
                ...
                if code_lat:
                    ...
                    if current_trip_distances:
                        ...
                        # 🚨 เข้มงวดขึ้น: ระยะเฉลี่ย > 15km หรือ ระยะไกลสุด > 25km → ข้าม
                        if avg_dist_current > 15 or max_dist_current > 25:
                            continue
            ...
            if should_pair:
                ...
                if not all_within_limit and not names_are_similar and not has_history:
                    # เช็คว่าระยะทางเฉลี่ยพอรับได้หรือไม่ (< 18km) - เข้มงวดขึ้น
                    if avg_dist_to_trip > 18:
                ...
                # 🔒 แต่ห้ามเกิน 100% เด็ดขาด (BUFFER = 1.0)
                if not can_fit:
                    ...
                    # 🔒 ห้ามเกิน 100% - ไม่ยอมให้เกินอีกแล้ว
                    # ถ้ารถยังใส่ได้พอดี 100% → รวมได้
                    if trip_weight <= max_w and trip_cube <= max_c:
                        can_fit = True
                
                # 🔒 ถ้าเกิน capacity → ห้ามรวม (ไม่มีข้อยกเว้น)
                # ลบ logic ยอมให้เกิน 10-15% ออก เพื่อบังคับ 100%
