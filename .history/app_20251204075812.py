    ...
    for trip_num in test_df['Trip'].unique():
        ...
        if all_nearby and current_vehicle == '6W':
            ...
            if jb_util <= 140:
                ...
                # JB เต็ม → แยกเป็น JB หลายคัน (เรียงตามพิกัด + ชื่อ)
                new_trips = []
                current_group = []
                current_group_w = 0
                current_group_c = 0
                
                # 🗺️ เรียงตามพิกัด + ชื่อคล้ายกัน
                def smart_sort_for_bangkok(df):
                    """เรียงสาขาให้อยู่ใกล้กัน"""
                    if len(df) == 0:
                        return df
                            ...
                            if not MASTER_DATA.empty:
                                ...
                                if len(m) > 0:
                                    ...
                                    if lat != 0:
                                        ...
                                        df.at[idx, 'sort_key'] = dist
                                    else:
                                        df.at[idx, 'sort_key'] = 999
                            else:
                                df.at[idx, 'sort_key'] = 999
                        
                        return df.sort_values(['sort_key', 'Name', 'Weight'], ascending=[True, True, False])
                    else:
                        return df.sort_values('Weight', ascending=False)
                
                sorted_data = smart_sort_for_bangkok(trip_data)
        ...
        if current_priority > allowed_priority or util_allowed > 130:
            if util_allowed <= 130:
                ...
                if split_needed:
                    ...
                    # 🗺️ เรียงตามพิกัด + ชื่อคล้ายกัน (Intelligent Sorting)
                    def smart_sort_branches(df):
                        """เรียงสาขาโดยพิจารณา: 1) พิกัดใกล้กัน 2) ชื่อคล้ายกัน 3) น้ำหนัก"""
                        if len(df) == 0:
                            return df
                        
                        # เพิ่มข้อมูลพิกัดและชื่อ
                        df = df.copy()
                        df['lat'] = 0.0
                        df['lon'] = 0.0
                        df['name_clean'] = ''
                        ...
                        for idx, row in df.iterrows():
                            ...
                            if not MASTER_DATA.empty:
                                m = MASTER_DATA[MASTER_DATA['Plan Code'] == code]
                                if len(m) > 0:
                                    df.at[idx, 'lat'] = m.iloc[0].get('ละติจูด', 0)
                                    df.at[idx, 'lon'] = m.iloc[0].get('ลองติจูด', 0)
                            
                            # ทำความสะอาดชื่อ
                            name = str(row.get('Name', ''))
                            # ลบ prefix
                            for prefix in ['PTC-MRT-', 'FC PTF ', 'PTC-', 'PTC ', 'FC ', 'CW ', 'NW ']:
                                if name.startswith(prefix):
                                    name = name[len(prefix):]
                                    break
                            df.at[idx, 'name_clean'] = name.strip()
                        
                        # เริ่มจาก centroid ของกลุ่ม
                        avg_lat = df['lat'].mean()
                        avg_lon = df['lon'].mean()
                        
                        # คำนวณระยะทางจาก centroid
                        df['dist_from_center'] = df.apply(
                            lambda r: haversine_distance(avg_lat, avg_lon, r['lat'], r['lon']) 
                            if r['lat'] != 0 else 999,
                            axis=1
                        )
                        
                        # เรียงตาม: ระยะทางจาก centroid → ชื่อ → น้ำหนัก
                        df_sorted = df.sort_values(
                            ['dist_from_center', 'name_clean', 'Weight'],
                            ascending=[True, True, False]
                        )
                        
                        return df_sorted
                    
                    sorted_data = smart_sort_branches(trip_data)
