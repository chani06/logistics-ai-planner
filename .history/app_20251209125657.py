...
# Performance Config - MAXIMUM SPEED (target < 20 seconds)
MAX_DETOUR_KM = 15  # เพิ่มเพื่อลด iteration
MAX_MERGE_ITERATIONS = 3  # ลดเหลือ 3 รอบ
MAX_REBALANCE_ITERATIONS = 1  # แค่ 1 รอบ
MAX_PROCESSING_TIME = 20  # วินาที - เร็วสุด
EARLY_STOP_UTIL = 85  # หยุดถ้าได้ 85%
EARLY_STOP_THRESHOLD = 0.85  # หยุดเร็วขึ้น
SKIP_OPTIMIZATION = True  # ข้าม optimization ทั้งหมด
SKIP_PHASE_3_SPLIT = True  # ข้าม Phase 3 (split 6W)
...
def load_autoplan_restrictions():
    ...
    try:
        ...
        if not file_path:
            # print("⚠️ ไม่พบไฟล์ Auto plan - จะไม่มีข้อจำกัดรถ")  # ซ่อนเพื่อความเร็ว
            return {}
...
def predict_trips(test_df, model_data):
    ...
    # 🎯 Phase 3: ปรับปรุง 6W ให้เหมาะสม (ข้ามเพื่อความเร็วสูงสุด)
    split_count = 0
    
    if not SKIP_PHASE_3_SPLIT:
        pass  # ข้าม Phase 3 ทั้งหมดเพื่อความเร็วสูงสุด
    else:
        # Phase 3 ถูกข้าม
        pass
    ...
    # 🔄 Phase 4: บังคับเปลี่ยน nearby จาก 6W → JB/4W และกระจายทริปน้อย (ข้ามเพื่อความเร็ว)
    low_util_trips = []
    
    # ⚡ Skip Phase 4 ทั้งหมด เพื่อความเร็วสูงสุด
    if True:  # SKIP_PHASE_3_SPLIT or time.time() - start_time > 20:
