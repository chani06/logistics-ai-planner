        ...
        for trip_num in sorted(test_df_result['Trip'].dropna().unique()):
            ...
            if min_max_size < 3:
                ...
                if suggested in LIMITS:
                    ...
                    # 🚨 ถ้าเกิน 100% (ทั้งน้ำหนักหรือคิว) ต้องเพิ่มขนาดรถ (แต่ต้อง respect สาขาจำกัด)
                    if w_util > 100 or c_util > 100:
                        # ลองเปลี่ยนเป็นรถใหญ่ขึ้นจนกว่าจะไม่เกิน (แต่ต้องอยู่ใน allowed)
                        upgraded = False
                        for try_vehicle in ['JB', '6W']:
                            # 🔒 ต้องตรวจสอบว่ารถนี้ได้รับอนุญาตจากสาขาหรือไม่
                            if try_vehicle not in allowed:
                                continue  # ข้ามถ้าสาขาไม่อนุญาต
                            if LIMITS[try_vehicle]['max_c'] >= LIMITS[suggested]['max_c']:
                                ...
                                if try_w_util <= 100 and try_c_util <= 100:
                                    ...
                                    upgraded = True
                                    break
                        # ถ้ายังเกิน → ต้องแยกทริป (ไม่สามารถ upgrade ได้อีก)
                        if not upgraded:
