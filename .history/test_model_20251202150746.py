...
def load_historical_data(folder='Dc', separate_test=True):
    ...
    if train_data:
        # ลบคอลัมน์ที่ไม่จำเป็นก่อน concat
...
def main():
    ...
    model_train_df, trip_pairs, branch_info = create_training_data(train_df)
    
    # 3. Train model
    model, train_acc, test_acc = train_decision_tree(model_train_df)
    
    # 4. Test กับทริปจริง
    accuracy, incorrect = test_model_on_actual_trips(train_df, model, trip_pairs, branch_info)
