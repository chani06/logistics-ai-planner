...
def analyze_trip_file(file_path):
    ...
    try:
        ...
        required_cols = ['Trip', 'Weight', 'Cube']
        missing_cols = [col for col in required_cols if col not in df.columns]
        
        if missing_cols:
            print(f"❌ ไม่พบคอลัมน์: {missing_cols}")
            return None
        ...
        print(f"\n📊 จำนวนทริปทั้งหมด: {df['Trip'].nunique()}")
...
def main():
    ...
    # กรองเฉพาะไฟล์ที่น่าจะเป็นผลลัพธ์
    test_files = [f for f in excel_files if any(keyword in f.name.lower() 
                  for keyword in ['result', 'output', 'trip', 'test', 'ผล'])]
    
    if not test_files:
        print("\n❌ ไม่พบไฟล์ทดสอบ (*.xlsx, *.xls)")
        print("   กรุณาวางไฟล์ผลลัพธ์ในโฟลเดอร์นี้")
        return 1
    ...
    for file_path in test_files:
        stats_df = analyze_trip_file(file_path)
    ...
    if all_results:
        ...
        for result in all_results:
            ...
            print(f"\n{status} - {result['file']}")
            print(f"   ทริปทั้งหมด: {result['total_trips']}")
            
            for vehicle, summary in result['summary'].items():
                optimal_pct = (summary['optimal_95_130'] / summary['count']) * 100
                print(f"   {vehicle}: {summary['count']} ทริป, เหมาะสม {summary['optimal_95_130']} ({optimal_pct:.1f}%)")
        ...
        if all_passed:
            ...
            print("⚠️  มีบางไฟล์ไม่ผ่านการทดสอบ")
            print("   กรุณาปรับปรุงการจัดทริปให้อยู่ในช่วง 95-130%")
        print("=" * 80 + "\n")
        
        return 0 if all_passed else 1
    else:
        print("\n❌ ไม่สามารถวิเคราะห์ไฟล์ใดได้")
        return 1
