...
def predict_trips(test_df, model_data):
    ...
    for trip_num in test_df['Trip'].unique():
        ...
        # 🚛 พื้นที่ไกลมากๆ (ภาคเหนือตอนบน/ใต้ลึก หรือ >300km) → ต้องใช้ 6W เท่านั้น
            ...
            # 3. พื้นที่ไกล/ต่างจังหวัด → ลอง 6W ก่อน แต่ถ้าเกินแล้วกระจายมาใส่ JB/4W ได้
            else:
                # ลอง 6W ก่อน (เหมาะสมสำหรับระยะไกล)
                if cube_util_6w >= 95 and cube_util_6w <= 140 and weight_util_6w <= 140:
                    # 6W เต็มพอดี (95-140%) → ใช้ 6W
                    recommended = '6W'
                    region_changes['far_keep_6w'] += 1
                elif cube_util_6w < 95:
                    # 6W ไม่เต็ม (<95%) → ลอง JB หรือ 4W แทน
                    if cube_util_jb >= 95 and cube_util_jb <= 130 and weight_util_jb <= 130:
                        recommended = 'JB'
                        region_changes['other'] += 1
                    elif cube_util_4w >= 95 and cube_util_4w <= 130 and weight_util_4w <= 130:
                        recommended = '4W'
                        region_changes['other'] += 1
                    else:
                        # ไม่มีรถเล็กพอดี → ใช้ 6W (ต่ำกว่า 95% ยอมได้สำหรับพื้นที่ไกล)
                        recommended = '6W'
                        region_changes['far_keep_6w'] += 1
                else:
                    # 6W เกิน (>140%) → ต้องแยก (จะแยกใน Phase 2.5)
                    recommended = '6W'
                    region_changes['far_keep_6w'] += 1
