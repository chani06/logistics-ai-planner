...
# 🚨 จำกัดระยะห่างสาขาในทริปเดียวกัน
MAX_DISTANCE_IN_TRIP = 50  # km - สาขาในทริปเดียวกันห้ามห่างกันเกิน 50km
...
def predict_trips(test_df, model_data):
    ...
    while all_codes:
        ...
        # 🔄 หาสาขาถัดไปที่ใกล้สุดและอยู่ในจังหวัดเดียวกัน
        while all_codes:
            best_code = None
            best_dist = 9999
            ...
            for code in all_codes:
                ...
                # กฎ: ระยะห่างต้องไม่เกิน MAX_DISTANCE_IN_TRIP (50km)
                if max_dist_in_trip > MAX_DISTANCE_IN_TRIP:
                    continue
                
                # ไม่บังคับจังหวัดเดียวกันอีกต่อไป - ใช้ระยะทางเป็นหลัก
    ...
    # ===============================================
    # 🎯 Phase 6.5: รวม 4W เข้ากับ 6W/JB ที่ใกล้กัน
    # ใช้ระยะทางใกล้สุดเป็นหลัก (ไม่สนจังหวัด)
    # พยายามรวมให้ได้มากที่สุดเพื่อลดจำนวน 4W
    # ===============================================
    merge_4w_count = 0
    ...
    for trip_num in list(test_df['Trip'].unique()):
        ...
        # เก็บทริป 4W ทั้งหมดเพื่อเรียงลำดับ
    ...
    for trip_info in trips_4w_to_merge:
        ...
        for target_num in test_df['Trip'].unique():
            ...
            # ระยะต้องไม่เกิน MAX_DISTANCE_IN_TRIP (50km)
    ...
    # ===============================================
    # 🎯 Phase 7: บังคับรวม 4W ที่ไม่ผ่าน MIN_UTIL
    # เพิ่มระยะทางที่ยอมรับเป็น 100km เพื่อให้รวมได้
    # ===============================================
    FORCED_MAX_DISTANCE = 100  # km - ยอมให้ไกลขึ้นเพื่อรวม 4W
    
    # หาทริป 4W ที่ไม่ผ่าน MIN_UTIL
    for iteration in range(5):  # ทำซ้ำ 5 รอบ
        trips_below_min = []
        ...
        for trip_num in list(test_df['Trip'].unique()):
            ...
            # เช็คข้อจำกัดรถของสาขาในทริป
            trip_codes_list = list(trip_data['Code'].values)
            max_allowed = get_max_vehicle_for_trip(set(trip_codes_list))
            
            # คำนวณ util ตามรถที่อนุญาต
            vehicle_sizes = {'4W': 1, 'JB': 2, '6W': 3}
            allowed_size = vehicle_sizes.get(max_allowed, 3)
            
            # หารถที่เหมาะสมที่สุด (เล็กสุดที่ใส่ได้)
            best_vehicle = None
            best_util = 0
            
            for v in ['4W', 'JB', '6W']:
                if vehicle_sizes.get(v, 3) > allowed_size:
                    continue  # ข้ามรถที่ใหญ่กว่าที่อนุญาต
                
                lim = LIMITS[v]
                util = max((trip_w / lim['max_w']) * 100, (trip_c / lim['max_c']) * 100)
                
                if util <= 100:
                    best_vehicle = v
                    best_util = util
                    break  # เลือกรถเล็กสุดที่ใส่ได้
            
            if not best_vehicle:
                best_vehicle = max_allowed
                best_util = 100
            
            min_util_req = MIN_UTIL.get(best_vehicle, 70)
            
            # ถ้าไม่ผ่าน MIN_UTIL → ต้องหารถไปรวม
            if best_util < min_util_req:
                # หา centroid
                trip_lats, trip_lons = [], []
                for code in trip_codes_list:
                    lat, lon = coord_cache.get(code, (None, None))
                ...
                if trip_lats:
                    trips_below_min.append({
                        'trip_num': trip_num,
                        'vehicle': best_vehicle,
                        'util': best_util,
                        'min_util': min_util_req,
                        'weight': trip_w,
                        'cube': trip_c,
                        'count': trip_count,
                        'codes': trip_codes_list,
        ...
        if not trips_below_min:
            break  # ทุกทริปผ่าน MIN_UTIL แล้ว
        
        # เรียงจาก util น้อยที่สุดก่อน
        trips_below_min.sort(key=lambda x: x['util'])
        
        # พยายามรวมทริปที่ไม่ผ่าน MIN_UTIL
        for low_trip in trips_below_min:
            # เช็คว่าทริปนี้ยังมีอยู่ไหม
            trip_data = test_df[test_df['Trip'] == low_trip['trip_num']]
            if len(trip_data) == 0:
                continue
            ...
            # หาทริปอื่นที่รวมได้
            for target_num in list(test_df['Trip'].unique()):
                if target_num == 0 or target_num == low_trip['trip_num']:
                    continue
                ...
                # เช็คข้อจำกัดรถของ target + low_trip รวมกัน
                combined_codes = set(target_codes + low_trip['codes'])
                max_allowed_combined = get_max_vehicle_for_trip(combined_codes)
                ...
                # หารถที่เหมาะสมหลังรวม
                allowed_size = vehicle_sizes.get(max_allowed_combined, 3)
                best_combined_vehicle = None
                best_combined_util = 0
                ...
                for v in ['4W', 'JB', '6W']:
                    ...
                    lim = LIMITS[v]
                    util = max((new_w / lim['max_w']) * 100, (new_c / lim['max_c']) * 100)
                    
                    # ต้องไม่เกิน 100%
                    if util <= 100:
                        # เช็คจำนวนสาขา
                        max_branches = 12 if v in ['4W', 'JB'] else 20
                        if new_count <= max_branches:
                            best_combined_vehicle = v
                            best_combined_util = util
                            break
                
                if not best_combined_vehicle:
                    continue  # รวมแล้วใส่รถไม่ได้
                
                # เช็คว่ารวมแล้วผ่าน MIN_UTIL ไหม
                min_util_combined = MIN_UTIL.get(best_combined_vehicle, 70)
                if best_combined_util < min_util_combined:
                    continue  # รวมแล้วยังไม่ผ่าน MIN_UTIL
                ...
                dist = haversine_distance(low_trip['lat'], low_trip['lon'], target_lat, target_lon)
                
                # ใช้ระยะทางที่ยอมรับเพิ่มขึ้น (100km)
                if dist > FORCED_MAX_DISTANCE:
                    continue
                
                # คะแนน = ระยะทาง (ยิ่งใกล้ยิ่งดี)
                score = dist
