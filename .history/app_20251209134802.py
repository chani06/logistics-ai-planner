                    ...
                    if st.button("🚀 เริ่มจัดเที่ยว", type="primary", use_container_width=True):
                        with st.spinner("⏳ กำลังประมวลผล..."):
                            ...
                            with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
                                ...
                                # ==============================================
                                # Sheet 2: Punthai Format (ตามไฟล์อัพโหลด)
                                # คอลัมน์: Sep, BU, รหัสสาขา, รหัส WMS, สาขา, Total Cube, Total Wgt, จำนวนชิ้น, Trip, Trip no, วันที่โหลด, เวลาโหลด, ประตู, WAVE, remark, latitude, longitude, Order, Seq., Route, Description, วันที่ตามรอบ, SAL, Delivery Date, Carrier, Booking
                                # ==============================================
                                
                                # สร้าง DataFrame สำหรับ Punthai format
                                punthai_data = []
                                
                                for idx, row in export_df.iterrows():
                                    trip_num = row.get('Trip', 0)
                                    trip_info = trip_no_map.get(trip_num, {'trip_no': '', 'vehicle': '6W'})
                                    ...
                                    seq = row.get('Sequence', idx + 1)
                                    
                                    # ข้อมูลจากไฟล์อัพโหลด (ถ้ามี)
                                    sep = row.get('Sep', '')
                                    bu = row.get('BU', 'PT')
                                    wms_code = row.get('รหัส WMS', code)
                                    qty = row.get('จำนวนชิ้น', row.get('Qty', ''))
                                    load_date = row.get('วันที่โหลด', '')
                                    load_time = row.get('เวลาโหลด(ประมาณ)', '')
                                    door = row.get('ประตู', '')
                                    wave = row.get('WAVE', '')
                                    remark = row.get('remark', '')
                                    order_no = row.get('Order', row.get('Order No', ''))
                                    route = row.get('Route', '')
                                    desc = row.get('Description', '')
                                    round_date = row.get('วันที่ตามรอบ', '')
                                    sal = row.get('SAL', '')
                                    delivery_date = row.get('Delivery Date', '')
                                    carrier = row.get('Carrier', trip_info['vehicle'])
                                    ...
                                    punthai_data.append({
                                        ...
                                        'รหัส WMS': wms_code,
                                        'สาขา': name,
                                        'Total Cube': round(cube, 2) if isinstance(cube, (int, float)) else cube,
                                        'Total Wgt': round(weight, 2) if isinstance(weight, (int, float)) else weight,
                                        'จำนวนชิ้น': qty,
                                        'Trip': trip_num,
                                        'Trip no': trip_info['trip_no'],
                                        'วันที่โหลด': load_date,
                                        'เวลาโหลด(ประมาณ)': load_time,
                                        'ประตู': door,
                                        'WAVE': wave,
                                        'remark': remark,
                                        'latitude': lat,
                                ...
                                punthai_df = pd.DataFrame(punthai_data)
                                
                                # เรียงตาม Trip
                                punthai_df = punthai_df.sort_values(['Trip', 'Seq.'], ascending=[True, True])
                                
                                # เขียน Sheet Punthai (เริ่มที่ B2)
                                workbook = writer.book
                                punthai_ws = workbook.add_worksheet('Punthai')
                                ...
                                # เขียน Header ที่แถว 1 (row=0), เริ่มคอลัมน์ B (col=1)
                                punthai_cols = list(punthai_df.columns)
                                for col_num, col_name in enumerate(punthai_cols):
                                    punthai_ws.write(0, col_num + 1, col_name, header_format)  # เริ่มที่ col=1 (B)
                                
                                # สีสลับสำหรับแต่ละทริป
                                colors = ['#FFE699', '#FFFFFF']  # เหลือง-ขาว สลับ
                                
                                current_trip = None
                                color_index = 0
                                
                                # เขียนข้อมูล (เริ่มที่แถว 2 = row index 1)
                                for row_num, (idx, row) in enumerate(punthai_df.iterrows()):
                                    trip = row['Trip']
                                    ...
                                    # เขียนทุก cell (เริ่มที่ col=1 = B)
                                    for col_num, col_name in enumerate(punthai_cols):
                                        value = row[col_name]
                                        ...
                                        punthai_ws.write(row_num + 1, col_num + 1, value, cell_format)
                                
                                # ปรับความกว้างคอลัมน์
                                punthai_ws.set_column('B:B', 8)   # Sep
                                punthai_ws.set_column('C:C', 6)   # BU
                                punthai_ws.set_column('D:D', 12)  # รหัสสาขา
                                punthai_ws.set_column('E:E', 12)  # รหัส WMS
                                punthai_ws.set_column('F:F', 35)  # สาขา
                                punthai_ws.set_column('G:G', 10)  # Total Cube
                                punthai_ws.set_column('H:H', 10)  # Total Wgt
                                punthai_ws.set_column('I:I', 10)  # จำนวนชิ้น
                                punthai_ws.set_column('J:J', 8)   # Trip
                                punthai_ws.set_column('K:K', 12)  # Trip no
                                
                                # ==============================================
                                # จัดรูปแบบ Sheet 1 (รายละเอียดทริป)
                                # ==============================================
                                worksheet = writer.sheets['รายละเอียดทริป']
                                
                                # สีสำหรับแต่ละทริป (สลับสี)
                                trip_colors = ['#FFE699', '#FFFFFF']  # เหลือง-ขาว สลับ
                                ...
                                for row_num in range(len(export_df)):
                                    ...
                                    if trip != current_trip:
                                        current_trip = trip
                                        color_index = (color_index + 1) % len(trip_colors)
                                    
                                    # สร้าง format สำหรับแถวนี้
                                    cell_format = workbook.add_format({
                                        'bg_color': trip_colors[color_index],
                                        'border': 1
                                    })
