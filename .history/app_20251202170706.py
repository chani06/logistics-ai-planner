...
def predict_trips(test_df, model_data):
    ...
    if use_file_trips:
        ...
        # สร้าง summary - รวม Weight/Cube ตาม Code ก่อน (เพราะมีหลายแถวต่อสาขา)
        summary_data = []
        for trip_num in sorted(test_df_result['Trip'].dropna().unique()):
            trip_data = test_df_result[test_df_result['Trip'] == trip_num]
            
            # รวม Weight/Cube ตาม Code (ไม่นับซ้ำ)
            trip_agg = trip_data.groupby('Code').agg({
                'Weight': 'sum',
                'Cube': 'sum'
            }).reset_index()
            
            total_w = trip_agg['Weight'].sum()
            total_c = trip_agg['Cube'].sum()
            num_branches = len(trip_agg)
