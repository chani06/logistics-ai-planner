                    ...
                    if st.button("🚀 เริ่มจัดเที่ยว", type="primary", use_container_width=True):
                        with st.spinner("⏳ กำลังประมวลผล..."):
                            ...
                            with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
                                ...
                                # จัดเรียงคอลัมน์ตามรูปแบบ Punthai: Trip no, Trip, Code, Name, Cube, Weight, Truck
                                standard_cols = ['Trip no', 'Trip', 'Code', 'Name', 'Cube', 'Weight', 'Truck']
                                other_cols = [c for c in export_df.columns if c not in standard_cols]
                                new_cols = standard_cols + other_cols
                                # กรองเฉพาะคอลัมน์ที่มีอยู่จริง
                                new_cols = [c for c in new_cols if c in export_df.columns]
