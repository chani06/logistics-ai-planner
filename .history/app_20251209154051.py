...
def get_max_vehicle_for_trip(trip_codes):
    ...
    Returns:
        str: '4W', 'JB', หรือ '6W'
    """
    # 🚛 ภาคเหนือ: บังคับใช้ 6W ไม่ต้องดู Auto Plan
    for code in trip_codes:
        prov = get_province(code)
        if prov != 'UNKNOWN' and get_region_type(prov) == 'north':
            return '6W'  # ภาคเหนือบังคับ 6W
...
def suggest_truck(total_weight, total_cube, max_allowed='6W', trip_codes=None):
    ...
    if trip_codes is not None and len(trip_codes) > 0:
        for code in trip_codes:
            ...
            if prov != 'UNKNOWN' and get_region_type(prov) == 'north':
                is_north_trip = True
                max_allowed = '6W'  # ภาคเหนือบังคับ 6W
                max_size = 3
                break
    
    # ตรวจสอบข้อจำกัดของสาขาทั้งหมดในกลุ่ม (ยกเว้นภาคเหนือ)
    branch_max_vehicle = '4W'  # 🔒 เริ่มต้นที่ 4W (เล็กสุด) แล้วขยายเมื่อจำเป็น
    if trip_codes is not None and len(trip_codes) > 0 and not is_north_trip:
        for code in trip_codes:
            branch_max = get_max_vehicle_for_branch(code)
...
def predict_trips(test_df, model_data):
    ...
    if time.time() - start_time > 55:
        ...
        for trip_num in test_df['Trip'].unique():
            ...
            # ⚠️ ยกเว้น: ภาคเหนือ บังคับใช้ 6W ไม่ต้องดู Auto Plan
            vehicle_priority = {'4W': 1, 'JB': 2, '6W': 3}
            recommended_priority = vehicle_priority.get(recommended, 3)
            allowed_priority = vehicle_priority.get(max_allowed, 3)
            
            if recommended_priority > allowed_priority and not has_north:
                # รถที่แนะนำใหญ่กว่าที่อนุญาต → บังคับใช้ max_allowed (ห้ามข้ามขั้น!)
                # ⚠️ ภาคเหนือ bypass - บังคับ 6W เสมอ (ถ้า ≥18 cube)
                recommended = max_allowed
            
            # 🔒 Double check: ห้ามข้ามข้อจำกัดสาขาเด็ดขาด!
            # ⚠️ ยกเว้น: ภาคเหนือ บังคับใช้ 6W
            if max_allowed == '4W' and recommended != '4W' and not has_north:
                recommended = '4W'
            elif max_allowed == 'JB' and recommended == '6W' and not has_north:
                recommended = 'JB'
