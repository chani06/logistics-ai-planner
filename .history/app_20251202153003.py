...
def predict_trips(test_df, model_data, truck_type='6W'):
    """
    จัดทริปด้วย AI โดยใช้กฎลำดับความสำคัญ:
    1. ✅ เคยไปด้วยกันในประวัติ (trip_pairs) - ความแม่นยำ 100%
    2. ✅ ชื่อสาขาคล้ายกัน (เช่น นครราชสีมา1, นครราชสีมา2)
    3. ✅ AI ทำนายจาก Decision Tree Model
    4. ✅ เช็คขีดจำกัดน้ำหนัก/คิวตามประเภทรถ
    """
    model = model_data['model']
    trip_pairs = model_data['trip_pairs']
    branch_info = model_data['branch_info']
    
    max_weight = LIMITS[truck_type]['max_w'] * BUFFER
    max_cube = LIMITS[truck_type]['max_c'] * BUFFER
    ...
    while all_codes:
        ...
        for code in remaining:
            ...
            if trip_weight > max_weight or trip_cube > max_cube:
                continue  # เกินขีดจำกัด ไม่เพิ่ม
            
            # กฎ 1: ถ้าเคยไปด้วยกันในประวัติ = จัดเข้าทริปเดียวกัน (สำคัญที่สุด)
...
def main():
    ...
    # เลือกรถ
    st.markdown("### 🚚 เลือกประเภทรถ")
    truck_type = st.selectbox(
        "ประเภทรถที่ต้องการใช้",
        options=['4W', '6W', 'JB'],
        index=1,  # default = 6W
        help="**4W**: น้ำหนักสูงสุด 2,500 kg, คิวสูงสุด 5.0 m³\n\n**6W**: น้ำหนักสูงสุด 5,800 kg, คิวสูงสุด 22.0 m³\n\n**JB**: น้ำหนักสูงสุด 3,500 kg, คิวสูงสุด 8.0 m³"
    )
    
    # แสดงข้อจำกัดของรถที่เลือก
    limits = LIMITS[truck_type]
    col1, col2 = st.columns(2)
    with col1:
        st.info(f"⚖️ น้ำหนักสูงสุด: **{limits['max_w']:,.0f} kg**")
    with col2:
        st.info(f"📦 คิวสูงสุด: **{limits['max_c']:.1f} m³**")
        ...
        with st.spinner("⏳ กำลังโหลดข้อมูล..."):
            ...
            if df is not None and 'Code' in df.columns:
                ...
                if st.button("🚀 จัดทริปอัตโนมัติ", type="primary", use_container_width=True):
                    with st.spinner(f"⏳ กำลังจัดทริป (รถ {truck_type})..."):
                        result_df, summary = predict_trips(df, model_data, truck_type)
