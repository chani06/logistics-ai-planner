...
def predict_trips(test_df, model_data):
    ...
    if use_file_trips:
        ...
        # สร้าง summary (ใช้ข้อมูลที่ aggregate แล้ว)
        summary_data = []
        for trip_num in sorted(test_df_agg['Trip'].dropna().unique()):
            trip_data = test_df_agg[test_df_agg['Trip'] == trip_num]
            total_w = trip_data['Weight'].sum()
            total_c = trip_data['Cube'].sum()
            branch_count = len(trip_data)  # นับจำนวนสาขา (ไม่ซ้ำแล้ว)
