...
def predict_trips(test_df, model_data):
    ...
    # 7.2 รวมทริปที่ต่ำกว่า MIN_UTIL (เน้น 4W ก่อน)
    for iteration in range(10):
        ...
        # เรียงจาก util น้อยสุด (4W ก่อน)
        low_util_trips.sort(key=lambda x: (0 if x['vehicle'] == '4W' else 1, x['util']))
        ...
        for trip_info in low_util_trips:
            ...
            for target_num in test_df['Trip'].unique():
                ...
                # เลือกรถที่เหมาะสม
                if new_c <= LIMITS['4W']['max_c'] and new_w <= LIMITS['4W']['max_w'] and new_count <= 12:
                    new_vehicle = '4W'
                elif new_c <= LIMITS['JB']['max_c'] and new_w <= LIMITS['JB']['max_w'] and new_count <= 12:
                    new_vehicle = 'JB'
                elif new_c <= LIMITS['6W']['max_c'] and new_w <= LIMITS['6W']['max_w']:
                    new_vehicle = '6W'
                else:
                    continue  # เกิน capacity
            ...
            if best_target is not None:
                ...
                # เลือกรถใหม่
                if new_c <= LIMITS['JB']['max_c'] and new_w <= LIMITS['JB']['max_w']:
                    trip_recommended_vehicles[best_target] = 'JB'
                else:
                    trip_recommended_vehicles[best_target] = '6W'
