...
# ==========================================
# 🚛 LOGISTICS ZONES - จัดกลุ่มตามโครงข่ายถนนและภูมิศาสตร์
# ==========================================
# หลักการ:
# 1. เกาะกลุ่มตามเส้นทางถนนหลัก (Route-Based Clustering)
# 2. แก้ไขรอยต่อจังหวัด (Cross-Border Optimization)  
# 3. แยกโซนภูเขา vs โซนพื้นราบ (Terrain Separation)
# 4. ความหนาแน่น (Density & Workload)

LOGISTICS_ZONES = {
    # ZONE A: พะเยา (แยกเอกเทศ - โซนเหนือสุด ติดเทือกเขา)
    'ZONE_A_พะเยา': {
        'provinces': ['พะเยา'],
        'districts': ['เมืองพะเยา', 'แม่ใจ', 'เชียงคำ', 'เชียงม่วน', 'ดอกคำใต้', 'ปง', 'จุน', 'ภูซาง', 'ภูกามยาว'],
        'priority': 1,
        'description': 'โซนเหนือสุด ไกลจากจุดอื่น บริหารในจังหวัดตัวเอง'
    },
    # ZONE B: น่าน (แยกเอกเทศ - หุบเขา เส้นทางเดียวจากแพร่)
    'ZONE_B_น่าน': {
        'provinces': ['น่าน'],
        ...
        'priority': 2,
        'description': 'เมืองในหุบเขา เส้นทางจากแพร่ทางเดียว (สาย 101)'
    },
    # ZONE C: แพร่ (Gateway เหนือตอนบน - ถนน 101, 11)
    'ZONE_C_แพร่': {
        'provinces': ['แพร่'],
        'districts': ['เมืองแพร่', 'สูงเม่น', 'เด่นชัย', 'ร้องกวาง', 'สอง', 'ลอง', 'วังชิ้น', 'หนองม่วงไข่'],
        'priority': 3,
        'description': 'Gateway เหนือตอนบน ถนน 101 → น่าน, ถนน 11 → อุตรดิตถ์'
    },
    # ZONE D: อุตรดิตถ์ (หน้าด่านก่อนเข้าแพร่ - ถนน 11)
    'ZONE_D_อุตรดิตถ์': {
        'provinces': ['อุตรดิตถ์'],
        'districts': ['เมืองอุตรดิตถ์', 'ตรอน', 'ท่าปลา', 'น้ำปาด', 'ฟากท่า', 'บ้านโคก', 'พิชัย', 'ลับแล', 'ทองแสนขัน'],
        'priority': 4,
        'description': 'หน้าด่านก่อนเข้าแพร่ ถนน 11 วิ่งยาว'
    },
    # ZONE E1: พิษณุโลก-ในเมือง (Hub ใหญ่ โซนตลาด/วัดจันทร์)
    ...
    # ZONE E2: พิษณุโลก-มหาวิทยาลัย (โซน ม.นเรศวร + ทิศใต้)
    'ZONE_E2_พิษณุโลก_มหาวิทยาลัย': {
        'provinces': ['พิษณุโลก'],
        'districts': ['เมืองพิษณุโลก'],
        'subdistricts': ['ท่าโพธิ์', 'อรัญญิก', 'แม่กา', 'สมอแข', 'บ้านป่า'],
        'priority': 6,
        'description': 'โซน ม.นเรศวร + ทิศใต้'
    },
    # ZONE E3: พิษณุโลก-ตะวันออก (วังทอง + กงไกรลาศ สุโขทัย - ถนน 12)
    'ZONE_E3_พิษณุโลก_ตะวันออก': {
        'provinces': ['พิษณุโลก', 'สุโขทัย'],
        'districts': ['วังทอง', 'กงไกรลาศ', 'พรหมพิราม', 'เนินมะปราง', 'บางระกำ', 'ศรีนคร'],
        'priority': 7,
        'description': 'โซนตะวันออก ถนน 12 ไปเขาค้อ + กงไกรลาศ (สุโขทัย)'
    },
    # ZONE F1: พิจิตร-สายหลัก (เมือง + สากเหล็ก - ถนน 115/11)
    'ZONE_F1_พิจิตร_สายหลัก': {
        'provinces': ['พิจิตร'],
        'districts': ['เมืองพิจิตร', 'สากเหล็ก', 'สามง่าม', 'วังทรายพูน'],
        'priority': 8,
        'description': 'พิจิตรสายหลัก ถนน 115/11'
    },
    # ZONE F2: พิจิตร-ตะวันออก (ตะพานหิน + ทับคล้อ - ถนน 113)
    'ZONE_F2_พิจิตร_ตะวันออก': {
        'provinces': ['พิจิตร'],
        'districts': ['ตะพานหิน', 'ทับคล้อ', 'ดงเจริญ', 'บางมูลนาก'],
        'priority': 9,
        'description': 'พิจิตรตะวันออก ถนน 113'
    },
    # ZONE F3: พิจิตร-สาย117 (โพธิ์ประทับช้าง + บึงนาราง - ถนน 117)
    'ZONE_F3_พิจิตร_สาย117': {
        'provinces': ['พิจิตร'],
        'districts': ['โพธิ์ประทับช้าง', 'บึงนาราง', 'วชิรบารมี'],
        'priority': 10,
        'description': 'พิจิตรสาย 117 นครสวรรค์-พิษณุโลก'
    },
    # ZONE G: นครสวรรค์-ตะวันออก (หนองบัว + ท่าตะโก - ถนน 11, 3004)
    'ZONE_G_นครสวรรค์': {
        'provinces': ['นครสวรรค์'],
        'districts': ['หนองบัว', 'ท่าตะโก', 'ไพศาลี', 'ตาคลี', 'บรรพตพิสัย', 'ชุมตาบง', 'ลาดยาว', 'ตากฟ้า'],
        'priority': 11,
        'description': 'นครสวรรค์ตะวันออก ถนน 11 และ 3004'
    },
}

# Mapping อำเภอ → Zone (สำหรับค้นหาเร็ว)
DISTRICT_TO_ZONE = {}
for zone_name, zone_info in LOGISTICS_ZONES.items():
    for district in zone_info.get('districts', []):
        for province in zone_info.get('provinces', []):
            key = f"{district}|{province}"
            DISTRICT_TO_ZONE[key] = zone_name
            # เพิ่ม key แบบไม่มี "เมือง" นำหน้า
            if district.startswith('เมือง'):
                short_district = district[5:]  # ตัด "เมือง" ออก
                DISTRICT_TO_ZONE[f"{short_district}|{province}"] = zone_name

def get_logistics_zone(province, district, subdistrict=None):
    """หา Logistics Zone จากจังหวัด/อำเภอ/ตำบล"""
    province_str = str(province).strip() if province else ''
    district_str = str(district).strip() if district else ''
    subdistrict_str = str(subdistrict).strip() if subdistrict else ''
    
    # 1. ลองหาจาก district+province key ก่อน
    key = f"{district_str}|{province_str}"
    if key in DISTRICT_TO_ZONE:
        zone_name = DISTRICT_TO_ZONE[key]
        # ถ้ามี subdistrict ให้เช็คว่าตรงกับ zone ย่อยไหม
        zone_info = LOGISTICS_ZONES.get(zone_name, {})
        if 'subdistricts' in zone_info and subdistrict_str:
            if subdistrict_str in zone_info['subdistricts']:
                return zone_name
            # ถ้าไม่ตรง ลองหา zone อื่นที่มี subdistrict นี้
            for zn, zi in LOGISTICS_ZONES.items():
                if subdistrict_str in zi.get('subdistricts', []):
                    if province_str in zi.get('provinces', []):
                        return zn
        return zone_name
    
    # 2. ลองหาจากจังหวัดอย่างเดียว
    for zone_name, zone_info in LOGISTICS_ZONES.items():
        if province_str in zone_info.get('provinces', []):
            # ถ้าจังหวัดตรง และ zone นี้ไม่มี districts → ใช้เลย
            if not zone_info.get('districts'):
                return zone_name
    
    return None  # ไม่พบ zone
...
def predict_trips(test_df, model_data):
    ...
    # 🆕 ฟังก์ชันหา Logistics Zone สำหรับ use_file_trips path
    def get_logistics_zone_local(province, district=None):
        """คืนค่า Logistics Zone ตามหลักการโครงข่ายถนน"""
        # ใช้ LOGISTICS_ZONES ที่กำหนดไว้ด้านบน
        prov_str = str(province).strip() if province else ''
        dist_str = str(district).strip() if district else ''
        
        # ลองหา zone จาก district ก่อน (DISTRICT_TO_ZONE)
        if dist_str:
            for dist_key, zone in DISTRICT_TO_ZONE.items():
                if dist_key in dist_str:
                    return zone
        
        # ลองหา zone จาก province
        zone_mapping = {
            'พะเยา': 'ZONE_A_พะเยา',
            'น่าน': 'ZONE_B_น่าน',
            'แพร่': 'ZONE_C_แพร่',
            'อุตรดิตถ์': 'ZONE_D_อุตรดิตถ์',
            'พิษณุโลก': 'ZONE_E1_พิษณุโลก_ในเมือง',  # default
            'สุโขทัย': 'ZONE_D_อุตรดิตถ์',  # สุโขทัยอยู่กับอุตรดิตถ์ตามถนน 12
            'ตาก': 'ZONE_D_อุตรดิตถ์',  # ตากผ่านถนน 12
            'พิจิตร': 'ZONE_F1_พิจิตร_สายหลัก',  # default
            'นครสวรรค์': 'ZONE_G_นครสวรรค์',
            'กำแพงเพชร': 'ZONE_F1_พิจิตร_สายหลัก',
            'เพชรบูรณ์': 'ZONE_E3_พิษณุโลก_ตะวันออก',  # เพชรบูรณ์เชื่อมตะวันออก
            'อุทัยธานี': 'ZONE_G_นครสวรรค์',
            # เหนือตอนบนอื่นๆ
            'เชียงราย': 'ZONE_A_พะเยา',  # เชียงรายใกล้พะเยา
            'เชียงใหม่': 'ZONE_C_แพร่',  # เชียงใหม่มาทางลำปาง
            'ลำปาง': 'ZONE_C_แพร่',  # ลำปางใกล้แพร่
            'ลำพูน': 'ZONE_C_แพร่',
            'แม่ฮ่องสอน': 'ZONE_C_แพร่'
        }
        
        for prov_key, zone in zone_mapping.items():
            if prov_key in prov_str:
                return zone
        
        # ใต้ฝั่งอันดามัน
        andaman_provinces = ['กระบี่', 'ตรัง', 'พังงา', 'ภูเก็ต', 'ระนอง', 'สตูล']
        for p in andaman_provinces:
            if p in prov_str:
                return 'ใต้ฝั่งอันดามัน'
        
        # ใต้ฝั่งอ่าวไทย
        gulf_provinces = ['ชุมพร', 'นครศรีธรรมราช', 'พัทลุง', 'ยะลา', 'สงขลา', 'สุราษฎร์ธานี', 'ปัตตานี', 'นราธิวาส']
        for p in gulf_provinces:
            if p in prov_str:
                return 'ใต้ฝั่งอ่าวไทย'
        
        return None
    
    # 🆕 ฟังก์ชันตรวจว่า Zone สองตัวสามารถรวมกันได้หรือไม่
    def can_merge_zones(zone1, zone2):
        """ตรวจว่า zone สองตัวอยู่ใน "กลุ่มที่รวมได้" หรือไม่"""
        if zone1 == zone2:
            return True
        ...
        # กลุ่มที่รวมได้:
        mergeable_groups = [
            # กลุ่ม พิษณุโลก (E1, E2, E3 รวมกันได้)
            ['ZONE_E1_พิษณุโลก_ในเมือง', 'ZONE_E2_พิษณุโลก_มหาวิทยาลัย', 'ZONE_E3_พิษณุโลก_ตะวันออก'],
            # กลุ่ม พิจิตร (F1, F2, F3 รวมกันได้)
            ['ZONE_F1_พิจิตร_สายหลัก', 'ZONE_F2_พิจิตร_ตะวันออก', 'ZONE_F3_พิจิตร_สาย117'],
            # กลุ่ม D+สุโขทัย (อุตรดิตถ์ + สุโขทัย + ตาก)
            ['ZONE_D_อุตรดิตถ์'],
            # ใต้
            ['ใต้ฝั่งอันดามัน'],
            ['ใต้ฝั่งอ่าวไทย']
    ...
    # 🆕 ฟังก์ชันดึงจังหวัดจาก DataFrame โดยตรง
    def get_province_from_df(df, code):
        """ดึงจังหวัดจาก DataFrame"""
        code_data = df[df['Code'] == code]
        if len(code_data) > 0 and 'Province' in code_data.columns:
            prov = code_data['Province'].iloc[0]
            if pd.notna(prov):
    ...
    def split_different_region_groups(df):
        """แยกสาขาที่อยู่คนละ Logistics Zone - ป้องกันรวม อุตรดิตถ์ + แพร่ + พะเยา"""
        for trip_num in df['Trip'].dropna().unique():
            trip_data = df[df['Trip'] == trip_num]
            trip_codes = list(trip_data['Code'].unique())
            ...
            for code in trip_codes:
                # หา province และ district
                prov = get_province_from_df(df, code)
                # ลองหา district จาก Code หรือ Name
                code_data = df[df['Code'] == code]
                district = None
                if len(code_data) > 0:
                    branch_name = str(code_data.iloc[0].get('Name', ''))
                    # ลองหา district จาก branch name
                    for dist in DISTRICT_TO_ZONE.keys():
                        if dist in branch_name:
                            district = dist
                            break
                
                zone = get_logistics_zone_local(prov, district) if prov else None
    ...
    if use_file_trips:
        ...
        # 🆕 Phase: รวมทริป Zone เดียวกันที่ยัง capacity ไม่เต็ม
        def merge_same_zone_trips(df):
            """รวมทริปที่อยู่ Logistics Zone เดียวกัน (หรือ Zone ที่รวมได้) และยังมี capacity เหลือ"""
            LIMITS_6W = {'max_c': 20.0, 'max_w': 6000}
            BUFFER = 1.05
            ...
            for trip_num in df['Trip'].dropna().unique():
                trip_data = df[df['Trip'] == trip_num]
                codes = list(trip_data['Code'].unique())
                total_c = trip_data['Cube'].sum()
                total_w = trip_data['Weight'].sum()
                
                # หา logistics zone หลัก
                zone_counts = {}
                for code in codes:
                    prov = get_province_from_df(df, code)
                    code_data = df[df['Code'] == code]
                ...
                main_zone = max(zone_counts.items(), key=lambda x: x[1])[0] if zone_counts else None
            ...
            # รวมทริป Zone เดียวกัน (หรือ Zone ที่ merge ได้)
            merged_trips = set()
            for trip1, info1 in list(trip_info.items()):
                if trip1 in merged_trips or info1['zone'] is None:
                    continue
                
                for trip2, info2 in list(trip_info.items()):
                    if trip2 <= trip1 or trip2 in merged_trips:
                        continue
                    if info2['zone'] is None:
                        continue
                    
                    # ตรวจว่า zone รวมกันได้หรือไม่
                    if not can_merge_zones(info1['zone'], info2['zone']):
                        continue
                    
                    # เช็ค capacity รวมกัน
                    combined_c = info1['cube'] + info2['cube']
                    combined_w = info1['weight'] + info2['weight']
                    
                    if combined_c <= LIMITS_6W['max_c'] * BUFFER and combined_w <= LIMITS_6W['max_w'] * BUFFER:
                        # รวมได้! ย้าย trip2 ไป trip1
                        for code in info2['codes']:
                            df.loc[df['Code'] == code, 'Trip'] = trip1
                        
                        # อัปเดต info1
                        info1['codes'].extend(info2['codes'])
                        info1['cube'] = combined_c
                        info1['weight'] = combined_w
                        info1['count'] += info2['count']
                        
                        merged_trips.add(trip2)
            
            return df
        
        test_df_result = merge_same_zone_trips(test_df_result)
    ...
    # 🆕 Mapping อำเภอ → จังหวัด (สำหรับชื่อสาขาที่มีแค่ชื่ออำเภอ)
    DISTRICT_TO_PROVINCE = {
        # แพร่
        'สูงเม่น': 'แพร่', 'เด่นชัย': 'แพร่', 'ลอง': 'แพร่', 'วังชิ้น': 'แพร่', 'สอง': 'แพร่', 'หนองม่วงไข่': 'แพร่', 'ร้องกวาง': 'แพร่',
        # อุตรดิตถ์
        'ท่าปลา': 'อุตรดิตถ์', 'น้ำปาด': 'อุตรดิตถ์', 'ตรอน': 'อุตรดิตถ์', 'ทองแสนขัน': 'อุตรดิตถ์', 'พิชัย': 'อุตรดิตถ์', 'ลับแล': 'อุตรดิตถ์', 'ฟากท่า': 'อุตรดิตถ์', 'บ้านโคก': 'อุตรดิตถ์',
        # น่าน
        'ปัว': 'น่าน', 'เชียงกลาง': 'น่าน', 'ท่าวังผา': 'น่าน', 'เวียงสา': 'น่าน', 'บ่อเกลือ': 'น่าน', 'แม่จริม': 'น่าน', 'บ้านหลวง': 'น่าน', 'นาน้อย': 'น่าน', 'นาหมื่น': 'น่าน', 'สองแคว': 'น่าน', 'ภูเพียง': 'น่าน', 'เฉลิมพระเกียรติ': 'น่าน', 'สันติสุข': 'น่าน', 'ทุ่งช้าง': 'น่าน',
        # พะเยา
        'เชียงม่วน': 'พะเยา', 'เชียงคำ': 'พะเยา', 'จุน': 'พะเยา', 'ดอกคำใต้': 'พะเยา', 'ปง': 'พะเยา', 'แม่ใจ': 'พะเยา', 'ภูซาง': 'พะเยา', 'ภูกามยาว': 'พะเยา',
        # นครราชสีมา
        'สีคิ้ว': 'นครราชสีมา', 'ปากช่อง': 'นครราชสีมา', 'มวกเหล็ก': 'สระบุรี', 'แก่งคอย': 'สระบุรี',
        # อื่นๆ
        'หาดใหญ่': 'สงขลา', 'บางกรวย': 'นนทบุรี', 'ศรีราชา': 'ชลบุรี', 'บ้านบึง': 'ชลบุรี',
    }
    
    def extract_province_from_name(branch_name):
        """ดึงจังหวัดจากชื่อสาขา เช่น 'พิษณุโลก5' -> 'พิษณุโลก' หรือ 'สูงเม่น5' -> 'แพร่'"""
        if not branch_name:
            return None
        name = str(branch_name).strip()
        
        # 1. ลองหาชื่อจังหวัดก่อน
        for province in THAI_PROVINCES:
            if province in name:
                return province
        
        # 2. 🆕 ลองหาชื่ออำเภอแล้ว mapping ไปจังหวัด
    ...
    while all_codes:
        ...
        # 🔥 เช็คว่าเป็นภาคที่ต้องใช้ 6W หรือไม่ (เหนือ/ใต้)
        seed_region = get_region_type(seed_province) if seed_province else 'unknown'
        is_north_south = seed_region in ['north', 'south']  # ภาคเหนือ/ใต้ → บังคับ 6W
        is_far_region = seed_region in ['north', 'south', 'far']  # ภาคที่ใช้ 6W ได้
        
        # 🔥 ถ้า seed อยู่ภาคเหนือ/ใต้ และไม่มีข้อจำกัด → บังคับใช้ 6W
        if is_north_south and seed_max_vehicle == '6W':
            # บังคับ 6W สำหรับภาคเหนือ/ใต้ (ถ้า seed ใช้ 6W ได้)
            max_cube = LIMITS['6W']['max_c'] * BUFFER  # 20 cube
            max_weight = LIMITS['6W']['max_w'] * BUFFER  # 6000 kg
        elif seed_max_vehicle == '4W':
            ...
            # 🔥 ภาคไกล (อีสาน) → ใช้ 6W ให้เต็ม 20 cube
            max_cube = LIMITS['6W']['max_c'] * BUFFER  # 20 cube
            max_weight = LIMITS['6W']['max_w'] * BUFFER  # 6000 kg
        ...
        if seed_ref:
            ...
            for same_code in same_ref_codes:
                ...
                # 🔥 ถ้าทริป 6W (ภาคเหนือ/ใต้) และสาขามีข้อจำกัด → ข้ามสาขานี้
                if is_north_south and seed_max_vehicle == '6W' and branch_max != '6W':
                    continue  # ข้ามสาขาที่มีข้อจำกัด → จะถูกจัดทริปแยก
        ...
        while all_codes:
            ...
            for code in all_codes:
                ...
                # 🔥 ถ้าทริป 6W (ภาคเหนือ/ใต้) และสาขามีข้อจำกัด → ข้ามสาขานี้
                code_branch_max = get_max_vehicle_for_branch(code)
                if is_north_south and seed_max_vehicle == '6W' and code_branch_max != '6W':
                    continue  # ข้ามสาขาที่มีข้อจำกัด → จะถูกจัดทริปแยก
                ...
                # 5. 🆕 ภาคเหนือ/ใต้ ต่างจังหวัดที่ไม่ติดกัน → ห้ามรวม (ป้องกันอุตรดิตถ์+พะเยา)
                if not same_province and not is_adjacent:
                    ...
                    elif seed_region in ['north', 'south'] and code_region == seed_region:
                        # 🆕 ภาคเหนือ/ใต้เดียวกัน แต่ต่างจังหวัดและไม่ติดกัน → ห้ามรวม
                        continue  # ป้องกันรวม อุตรดิตถ์ + พะเยา (ไม่ได้อยู่ใกล้กัน)
    ...
    # 🆕 ฟังก์ชันหา Logistics Zone สำหรับทริป
    def get_logistics_zone_for_code(code):
        """หา Logistics Zone จาก code โดยใช้ province และ district"""
        prov = province_cache.get(code, '') or get_province(code)
        district = district_cache.get(code, '') if 'district_cache' in dir() else None
        
        # ลองหา district จาก DISTRICT_TO_ZONE
        if not district:
            # ลองหาจาก branch name
            code_rows = test_df[test_df['Code'] == code]
            if len(code_rows) > 0:
                branch_name = str(code_rows.iloc[0].get('Name', ''))
                for dist in DISTRICT_TO_ZONE.keys():
                    if dist in branch_name:
                        district = dist
                        break
        
        return get_logistics_zone(prov, district) if prov else None
    ...
    # 🆕 หา Logistics Zone หลักของทริป
    def get_primary_zone(trip):
        """หา Logistics Zone หลักของทริป"""
        zone_counts = {}
        for code in trip['codes']:
            zone = get_logistics_zone_for_code(code)
            if zone:
                zone_counts[zone] = zone_counts.get(zone, 0) + 1
        if zone_counts:
    ...
    for trip in all_trips:
        ...
        # 🆕 เพิ่ม logistics_zone สำหรับการเรียง
        trip['logistics_zone'] = get_primary_zone(trip) or 'zzz_other'
        trip['region_group'] = get_region_group(trip['primary_province']) or 'zzz_other'
        trip['region'] = get_region_type(trip['primary_province']) or 'other'
    
    # 🔄 เรียงตาม Logistics Zone → จังหวัด → ระยะทาง (ป้องกันทริปกระโดดข้าม Zone)
    # ลำดับ region: south → north → far → nearby
    # Zone order: A→B→C→D→E→F→G (ไกลไปใกล้)
    region_order = {'south': 0, 'north': 1, 'far': 2, 'nearby': 3, 'other': 4}
    zone_order = {
        'ZONE_A_พะเยา': 1,
        'ZONE_B_น่าน': 2,
        'ZONE_C_แพร่': 3,
        'ZONE_D_อุตรดิตถ์': 4,
        'ZONE_E1_พิษณุโลก_ในเมือง': 5,
        'ZONE_E2_พิษณุโลก_มหาวิทยาลัย': 6,
        'ZONE_E3_พิษณุโลก_ตะวันออก': 7,
        'ZONE_F1_พิจิตร_สายหลัก': 8,
        'ZONE_F2_พิจิตร_ตะวันออก': 9,
        'ZONE_F3_พิจิตร_สาย117': 10,
        'ZONE_G_นครสวรรค์': 11,
        'ใต้ฝั่งอันดามัน': 20,
        'ใต้ฝั่งอ่าวไทย': 21
    }
    all_trips.sort(key=lambda x: (
        region_order.get(x.get('region', 'other'), 4),  # 1. ภูมิภาค (ใต้→เหนือ→ไกล→ใกล้)
        zone_order.get(x.get('logistics_zone', 'zzz_other'), 99),  # 2. Logistics Zone (A→G)
        x['primary_province'],  # 3. จังหวัด
        -x['distance_from_dc'],  # 4. ระยะทางจาก DC (ไกลไปใกล้)
        x['count'],
        x['util']
    ))
    ...
    # ===============================================
    # 🎯 Phase 0.6: รวมเศษเหนือ/ใต้กับภาคกลางที่ตำบล/อำเภอติดกัน → ใช้ JB
    # เงื่อนไข: ตำบล/อำเภอต้องติดกัน (adjacent_districts)
    # ===============================================
    merge_north_south_central_count = 0
    
    # หาทริปเหนือ/ใต้ที่ยังมีเศษ (cube < 95% ของ 6W)
    remaining_north_south = []
    central_trips = []
    ...
    for idx, trip in enumerate(all_trips):
        ...
        primary_prov = trip['primary_province']
        region = get_region_type(primary_prov)
        cube_util = (trip['cube'] / LIMITS['6W']['max_c']) * 100
        
        if region in ['north', 'south'] and cube_util < 95:
            remaining_north_south.append({'idx': idx, 'trip': trip, 'region': region})
        elif region in ['nearby', 'far']:
            central_trips.append({'idx': idx, 'trip': trip})
    
    # รวมเศษเหนือ/ใต้กับภาคกลาง ถ้าตำบล/อำเภอติดกัน
    for ns_item in remaining_north_south:
        ns_trip = ns_item['trip']
        ns_idx = ns_item['idx']
        
        if all_trips[ns_idx] is None:
            continue
        
        # หาสาขาที่อยู่ชายขอบ (ติดกับภาคกลาง)
        for c_item in central_trips:
            c_trip = c_item['trip']
            c_idx = c_item['idx']
            
            if all_trips[c_idx] is None:
                continue
            
            # เช็คว่าตำบล/อำเภอติดกันหรือไม่
            adjacent_found = False
            for ns_code in ns_trip['codes']:
                ns_district = district_cache.get(ns_code, '')
                ns_province = province_cache.get(ns_code, '')
                
                for c_code in c_trip['codes']:
                    c_district = district_cache.get(c_code, '')
                    c_province = province_cache.get(c_code, '')
                    
                    # เช็คด้วย are_adjacent_districts
                    if are_adjacent_districts(ns_district, ns_province, c_district, c_province):
                        adjacent_found = True
                        break
                if adjacent_found:
                    break
            ...
            # เช็ค capacity รวมกัน (ใช้ JB limit)
            combined_cube = ns_trip['cube'] + c_trip['cube']
            combined_weight = ns_trip['weight'] + c_trip['weight']
            combined_count = ns_trip['count'] + c_trip['count']
            
            # ใช้ JB limit (7 cube, 3500 kg) เพราะอยู่ชายขอบ
            if combined_cube > LIMITS['JB']['max_c'] * BUFFER:
                continue
            if combined_weight > LIMITS['JB']['max_w'] * BUFFER:
                continue
            if combined_count > 12:  # JB limit
                continue
            
            # รวมได้! ย้ายสาขาจาก north/south trip ไป central trip
            for code in ns_trip['codes']:
                test_df.loc[test_df['Code'] == code, 'Trip'] = c_trip['trip']
            
            # อัปเดต central trip
            c_trip['codes'].update(ns_trip['codes'])
            c_trip['cube'] = combined_cube
            c_trip['weight'] = combined_weight
            c_trip['count'] = combined_count
            c_trip['provinces'].update(ns_trip['provinces'])
            
            # ลบ north/south trip
            all_trips[ns_idx] = None
            merge_north_south_central_count += 1
            break  # หยุดหาสำหรับ ns_trip นี้
    ...
    if time.time() - start_time > 55:
        ...
        for trip_num in test_df['Trip'].unique():
            ...
            # ⚠️ ภาคเหนือ/ใต้ → ใช้ 6W เป็นหลัก แต่ต้องสนใจข้อจำกัดสาขา
            vehicle_priority = {'4W': 1, 'JB': 2, '6W': 3}
            recommended_priority = vehicle_priority.get(recommended, 3)
            allowed_priority = vehicle_priority.get(max_allowed, 3)
            
            # 🔥 ภาคเหนือ/ใต้ → ใช้ 6W เป็นหลัก แต่ต้องเคารพข้อจำกัดสาขา
            if has_north or has_south:
                # ถ้าไม่มีข้อจำกัดสาขา (max_allowed = 6W) → ใช้ 6W
                if max_allowed == '6W':
                    recommended = '6W'
                else:
                    # มีสาขาที่จำกัด 4W/JB → ต้องใช้รถตามข้อจำกัด
                    # สาขาเหล่านี้ควรถูกแยกออกไปทริปอื่นในขั้นตอนจัดทริป
                    recommended = max_allowed
            elif recommended_priority > allowed_priority:
