...
def main():
    ...
    if st.button("🚀 เริ่มวางแผน", type="primary", use_container_width=True):
        ...
        with st.spinner("⏳ กำลังประมวลผล..."):
            ...
            # Export - สร้างชื่อไฟล์ใหม่ตามวันเวลา
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            test_filename = test_file.name.replace('.xlsx', '').replace('.xls', '')
            output_filename = f'AI_Plan_{test_filename}_{timestamp}.xlsx'
            ...
            st.success(f"📄 ชื่อไฟล์: {output_filename}")
