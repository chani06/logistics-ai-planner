                            ...
                            try:
                                ...
                                for trip_num in sorted(result_df['Trip'].unique()):
                                    ...
                                    for _, row in trip_data.iterrows():
                                        ...
                                        # คอลัมน์: A=Sep, B=BU, C=รหัสสาขา, D=รหัส WMS, E=สาขา, F=Cube, G=Weight, H=ชิ้น, I=Trip, J=Trip no
                                        data = [
                                            '',  # A: Sep
                                            row.get('BU', 211),  # B: BU
                                            ...
                                            row.get('Pieces', 0) if pd.notna(row.get('Pieces')) else 0,  # H: ชิ้น
                                            int(trip_num),  # I: Trip
                                            trip_no,  # J: Trip no
                                            '',  # K: วันที่โหลด
                                            '',  # L: เวลาโหลด
                                ...
                                # เขียน header
                                headers = ['Sep.', 'BU', 'รหัสสาขา', 'รหัส WMS', 'สาขา', 'Total Cube', 'Total Wgt', 'จำนวนชิ้น', 'Trip', 'Trip no']
                                ...
                                for trip_num in sorted(result_df['Trip'].unique()):
                                    ...
                                    for _, row in trip_data.iterrows():
                                        data = ['', 211, row.get('Code', ''), row.get('Code', ''), row.get('Name', ''),
                                                round(row.get('Cube', 0), 2), round(row.get('Weight', 0), 2), 0, int(trip_num), trip_no]
                                        for col_idx, value in enumerate(data, 1):
                                            cell = ws.cell(row=row_num, column=col_idx, value=value)
                                            cell.fill = fill
                                        row_num += 1
