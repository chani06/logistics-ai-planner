...
def main():
    ...
    if st.button("🚀 เริ่มวางแผน", type="primary", use_container_width=True):
        ...
        with st.spinner("⏳ กำลังประมวลผล..."):
            ...
            with tab1:
                st.subheader("📊 สรุปกลุ่มสาขาแต่ละทริป")
                st.markdown("""
                **คำอธิบาย:**
                - ✅ **เคยไปด้วยกันในประวัติ**: สาขาในกลุ่มนี้เคยถูกจัดให้ไปด้วยกันในไฟล์ประวัติ
                - ❌ **ไม่เคยไปด้วยกันในประวัติ**: เป็นการจับคู่ใหม่ที่ระบบ AI จัดให้
                """)
                
                # แสดงสถิติ
                total_groups = len(df_groups)
                historical_groups = len(df_groups[df_groups['เคยไปด้วยกันในประวัติ'].str.contains('ใช่')])
                ...
                # แสดงคำเตือนสำหรับกลุ่มที่ไม่เคยไปด้วยกัน
                if new_groups > 0:
                    st.warning(f"⚠️ มี {new_groups} กลุ่มที่ไม่เคยไปด้วยกันในประวัติ กรุณาตรวจสอบความเหมาะสม")
                    
                    new_group_list = df_groups[df_groups['เคยไปด้วยกันในประวัติ'].str.contains('ไม่')]
                    with st.expander("📋 ดูรายละเอียดการจับคู่ใหม่"):
