...
def main():
    ...
    if st.button("🚀 เริ่มวางแผน", type="primary", use_container_width=True):
        ...
        with st.spinner("⏳ กำลังประมวลผล..."):
            ...
            st.info(f"🗺️ จำนวนสาขาที่รู้จักภูมิภาค: {len([k for k, v in regions.items() if v != 'UNKNOWN'])} สาขา")
            ...
            # Load geo และจังหวัด - ใช้ข้อมูลจาก training files
            geo = {}
            for df in tr_dfs:
                if df is not None:
                    ...
                    # อ่านข้อมูลจังหวัดจากไฟล์ประวัติ
                    if 'Province' in df.columns and 'Code' in df.columns:
                        for _, r in df.iterrows():
                            if pd.notna(r['Province']) and pd.notna(r['Code']):
                                code = normalize(str(r['Code']))
                                prov = str(r['Province']).strip()
                                zone = get_province_zone(prov)
                                if code not in regions or regions[code] == 'UNKNOWN':
                                    regions[code] = zone
            ...
            # ดึงพิกัดและจังหวัดจากชีต Location ในไฟล์ Test (ถ้ามี)
            ...
            if df_location is not None:
                ...
                if df_location_processed is not None:
                    ...
                    # อ่านข้อมูลจังหวัดจาก Location sheet
                    if 'Province' in df_location_processed.columns and 'Code' in df_location_processed.columns:
                        for _, r in df_location_processed.iterrows():
                            if pd.notna(r['Province']) and pd.notna(r['Code']):
                                code = normalize(str(r['Code']))
                                prov = str(r['Province']).strip()
                                zone = get_province_zone(prov)
                                if code not in regions or regions[code] == 'UNKNOWN':
                                    regions[code] = zone
                        st.info(f"📍 โหลดข้อมูลจังหวัดจากชีต Location: {len(df_location_processed)} สาขา")
