...
def plan_trips_v2(df, master_data):
    ...
    while remaining_indices:
        ...
        for idx in remaining_indices[:]:
            ...
            # 1. เช็คภูมิภาค - ถ้าต่างภูมิภาค → ไม่รวม (ยกเว้น UNKNOWN)
            # CENTRAL กับภาคอื่นไม่ควรรวมกันเลย เพราะอยู่คนละทิศทางของประเทศ
            regions_compatible = True
            if seed_region == 'UNKNOWN' or branch_region == 'UNKNOWN':
                pass  # UNKNOWN สามารถรวมกับอะไรก็ได้
            elif seed_region == 'CENTRAL' and branch_region not in ['CENTRAL', 'UNKNOWN']:
                # CENTRAL ผสมภาคอื่น → ไม่รวม
                regions_compatible = False
            elif branch_region == 'CENTRAL' and seed_region not in ['CENTRAL', 'UNKNOWN']:
                # ภาคอื่นผสม CENTRAL → ไม่รวม
                regions_compatible = False
            elif seed_region not in ['CENTRAL', 'UNKNOWN'] and branch_region not in ['CENTRAL', 'UNKNOWN']:
                # ภาคอื่นๆ ต้องเป็นภาคเดียวกัน (NORTH, NORTHEAST, SOUTH)
    ...
    # Phase 3.75: เรียงลำดับหมายเลขทริปใหม่ (1, 2, 3, ... ไม่มีช่องว่าง)
