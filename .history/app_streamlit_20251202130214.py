...
def process_dataframe(df):
    ...
    # กรองข้อมูลที่ต้อง exclude ออก
    mask_ex = pd.Series([False] * len(df), index=df.index)
    
    # เช็ค Code
    for exc in EXCLUDE:
        exc_norm = normalize(exc)
        mask_ex |= df['Code'].apply(lambda x: exc_norm in str(x).upper() or str(x).upper() in exc_norm)
    
    # เช็ค Name
    if 'Name' in df.columns:
        for exc in EXCLUDE:
            mask_ex |= df['Name'].apply(lambda x: exc.upper() in str(x).upper() or normalize(exc) in normalize(str(x)))
    
    # กรองน้ำหนัก/คิวที่เป็น 0 ออก (PROJECT, DC)
    mask_ex |= (df['Wgt'] == 0) & (df['Cube'] == 0)
