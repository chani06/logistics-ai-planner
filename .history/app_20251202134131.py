...
HISTORICAL_ONLY_MODE = True  # จัดเฉพาะสาขาที่เคยไปด้วยกันในประวัติ
...
def process_dataframe(df):
    ...
    for c in df.columns:
        ...
        if 'BRANCHCODE' in cu or 'รหัสสาขา' in cu or 'รหัสWMS' in cu: rename_map[c] = 'Code'
        elif 'BRANCH' in cu or 'ชื่อสาขา' in cu or 'สาขา'==c: rename_map[c] = 'Name'
        elif 'TOTALWGT' in cu or 'WGT' in cu or 'น้ำหนัก' in cu: rename_map[c] = 'Wgt'
        elif 'TOTALCUBE' in cu or 'CUBE' in cu or 'คิว' in cu: rename_map[c] = 'Cube'
        elif 'LATITUDE' in cu or 'LAT' in cu: rename_map[c] = 'Lat'
        elif 'LONGITUDE' in cu or 'LON' in cu: rename_map[c] = 'Lon'
        elif c.strip() == 'Trip' or 'BOOKING' in cu: rename_map[c] = 'Trip'
        elif c.strip() == 'Trip no' or 'TRIPNO' in cu or 'VEHICLE' in cu: rename_map[c] = 'Vehicle'
        elif 'จังหวัด' in cu or 'PROVINCE' in cu: rename_map[c] = 'Province'
    ...
    # สร้าง mask แยกเพื่อหลีกเลี่ยงปัญหา duplicate index
    mask_ex = pd.Series([False] * len(df), index=df.index)
    mask_ex = mask_ex | df['Code'].isin(EXCLUDE)
    
    if 'Name' in df.columns:
        mask_name = df['Name'].apply(lambda x: any(k in str(x) for k in EXCLUDE))
        mask_ex = mask_ex | mask_name
    
    return df[~mask_ex].reset_index(drop=True).copy()
...
def run_prediction(df_test, G, geo, constraints, region_map):
    ...
    # ถ้าเปิด HISTORICAL_ONLY_MODE ให้แต่ละสาขามี Cluster ของตัวเอง (แยกทริป)
    # แล้วค่อยรวมเฉพาะที่มีประวัติร่วมกันในขั้นตอนการสร้างทริป
    if HISTORICAL_ONLY_MODE:
        # ให้แต่ละสาขามี cluster เป็นของตัวเอง เพื่อให้ไปทีละสาขา
        df_test['Cluster'] = df_test['Code'].map(lambda x: f"SINGLE-{x}")
    else:
        # โหมดปกติ - ใช้ connected components
    ...
    # ถ้าไม่ใช่ HISTORICAL_ONLY_MODE ให้ทำการจัดกลุ่มสาขาที่ไม่รู้จัก
        ...
        while pool:
            ...
            while True:
                ...
                for i, cand in enumerate(pool):
                    # เช็คประวัติก่อนทุกอย่าง (ถ้าเปิด HISTORICAL_ONLY_MODE)
                    if HISTORICAL_ONLY_MODE:
                        # ต้องมีประวัติไปด้วยกันกับ seed หรือกับสาขาใดๆ ในรถปัจจุบัน
                        has_history = False
            ...
            if cube_util < MIN_CUBE_UTILIZATION and len(pool) > 0 and drops < MAX_DROPS_FLEX:
                ...
                while i < len(pool) and cube_util < MIN_CUBE_UTILIZATION and drops < MAX_DROPS_FLEX:
                    ...
                    if new_w_test <= 5800 and new_c_test <= 22.0 * FLEX_CUBE_UTILIZATION:
                        ...
                        # เช็คประวัติก่อน (ถ้าเปิด HISTORICAL_ONLY_MODE)
...
def analyze_branch_groups(df_result, G):
    ...
    ระบุว่ากลุ่มใดเคยไปด้วยกันในประวัติ
    """
    branch_groups = []
    ...
    for booking_no, group in df_result.groupby('Booking No'):
        ...
        if len(codes) > 1:
            # ตรวจสอบว่าทุกคู่ของสาขาในกลุ่มมี edge ใน Graph หรือไม่
            all_paired = True
            for i in range(len(codes)):
                for j in range(i+1, len(codes)):
        ...
        group_info = {
            ...
            'จำนวนสาขา': len(codes),
            'รายชื่อสาขา': ', '.join(codes),
            'รายชื่อสาขาเต็ม': '\n'.join([f"{code}: {name}" for code, name in zip(group['รหัสสาขา'], group['สาขา'])]),
                ...
                **คำอธิบาย:**
                ...
                if len(df_groups_historical) > 0:
                    ...
                    column_config={
                        ...
                        'รายชื่อสาขา': st.column_config.TextColumn('รายชื่อสาขา (รหัส)', width='large'),
