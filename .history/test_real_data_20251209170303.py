...
def load_punthai_file():
    ...
    for i in range(min(5, len(df))):
        row = df.iloc[i]
        if 'สาขา' in str(row.values) or 'Code' in str(row.values):
    ...
    for col in df.columns:
        ...
        if 'สาขา' in col_str or col_str == 'code':
            col_map['Code'] = col
        elif 'ชื่อสาขา' in col_str or 'name' in col_str:
            col_map['Name'] = col
        elif 'weight' in col_str or 'น้ำหนัก' in col_str:
            col_map['Weight'] = col
        elif 'cube' in col_str or 'คิวบ์' in col_str or 'totalcube' in col_str:
            col_map['Cube'] = col
    ...
    df_grouped = df.groupby('Code').agg(agg_cols).reset_index()
