...
def export_with_colors(result_df, output_file, original_file, sheet_name="2.Punthai"):
    """Export กลับไฟล์เดิมพร้อมสีเหลืองโทนส้ม (ไม่สลับ) และใส่แค่ Code กับ Trip"""
    ...
    # สีเหลืองโทนส้ม (ไม่สลับ)
    yellow_orange = PatternFill(start_color='FFE4B5', end_color='FFE4B5', fill_type='solid')
    
    code_col = 3      # Code column
    trip_col = 9      # Trip column
    
    # สร้าง mapping สำหรับ Trip
    ...
    for row_idx in range(3, ws.max_row + 1):
        ...
        if code and code in code_to_trip:
            ...
            # ระบายสีเฉพาะคอลัมน์ Code (3) และ Trip (9)
            ws.cell(row=row_idx, column=code_col).fill = yellow_orange
            ws.cell(row=row_idx, column=trip_col).fill = yellow_orange
