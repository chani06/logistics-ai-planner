...
def process_dataframe(df):
    ...
    for c in df.columns:
        ...
        if c_stripped == 'BranchCode' or c_stripped == 'รหัส WMS':
            rename_map[c] = 'Code'
    ...
    # ใช้ vectorized operations เพื่อความเร็ว - แก้ไขให้ถูกต้อง
    mask_to_keep = ~df['Code'].isin(EXCLUDE)
    
    if 'Name' in df.columns:
        # ใช้ vectorized string operations
        for exclude_key in EXCLUDE:
            name_contains = df['Name'].astype(str).str.contains(exclude_key, na=False, regex=False)
            mask_to_keep = mask_to_keep & ~name_contains
