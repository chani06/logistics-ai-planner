...
def predict_trips(test_df, model_data):
    ...
    for balance_iter in range(MAX_BALANCE_ITERATIONS):
        ...
        for low_trip in low_util_trips_balance:
            ...
            for trip_num in test_df['Trip'].unique():
                ...
                max_branches = 12 if target_vehicle in ['4W', 'JB'] else 20
    ...
    # ===============================================
    # 🎯 Phase 6.5: รวม 4W เล็กเข้ากับ 6W/JB ที่ใกล้กัน
    # ใช้ระยะทางใกล้สุดเป็นหลัก (ไม่สนจังหวัด)
    # ===============================================
    merge_4w_count = 0
    
    # หาทริป 4W ที่ util ต่ำ
    for trip_num in list(test_df['Trip'].unique()):
        ...
        # ถ้า util >= MIN_UTIL['4W'] (70%) → ไม่ต้องรวม
        ...
        for target_num in test_df['Trip'].unique():
            ...
            # เช็คว่า target ยังผ่าน MIN_UTIL ไหมหลังรวม
            min_util_target = MIN_UTIL.get(target_vehicle, 80)
            if new_util < min_util_target:
                continue  # ถ้ารวมแล้ว util ยังต่ำกว่า min → ไม่รวม
