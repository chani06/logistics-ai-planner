...
def predict_trips(test_df, model_data):
    ...
    # ถ้าทริปเหนือ/ใต้ยังไม่เต็ม 6W (< 7 cube) → รวมกับทริป nearby ใกล้สุด
    # ===============================================
    ...
    for idx, trip in enumerate(all_trips):
        ...
        if region in ['north', 'south'] and trip['cube'] < 7.0:
            ...
            trip_lat, trip_lon = trip.get('centroid_lat'), trip.get('centroid_lon')
            
            # หาทริป nearby ที่ใกล้ที่สุด และยังใส่ได้
            ...
            # ถ้าหา nearby ได้ → รวมเข้าไป
            if best_nearby_idx is not None and best_distance < 100:  # ไม่เกิน 100 กม.
