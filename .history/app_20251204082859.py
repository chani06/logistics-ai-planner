    ...
    for trip_num in sorted(test_df['Trip'].unique()):
        ...
        if current_priority > allowed_priority or util_allowed > 130:
            if util_allowed <= 130:
                ...
                # 🎯 กลยุทธ์ใหม่: 
                #   - ถ้าจำกัด 4W → เปลี่ยนเป็น JB (ทุกพื้นที่)
                #   - ถ้า JB เต็ม → แยกเป็น JB หลายคัน
                
                target_vehicle = max_allowed
                
                # 🚨 กรณีพิเศษ: 4W จำกัด → บังคับใช้ JB (ทุกพื้นที่)
                if max_allowed == '4W':
                ...
                # 🚨 พื้นที่ห้าม 6W (กรุงเทพ+ปริมณฑล+ภาคกลาง) → บังคับใช้ JB
                elif all_nearby and max_allowed == '6W':
                    target_vehicle = 'JB'
