                    ...
                    if st.button("🚀 เริ่มจัดเที่ยว", type="primary", use_container_width=True):
                        with st.spinner("⏳ กำลังประมวลผล..."):
                            ...
                            with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
                                ...
                                # สร้างคอลัมน์ Trip no (เช่น 4W001, 4WJ002, 6W003) - รองรับ Jumbo
                                trip_no_map = {}
                                ...
                                # เพิ่มคอลัมน์ Trip no (ตรงกับ Punthai)
                                export_df['Trip no'] = export_df['Trip'].map(trip_no_map)
                                
                                # 🔥 จัดเรียงคอลัมน์ตามรูปแบบ Punthai template
                                # Sep., BU, รหัสสาขา, รหัส WMS, สาขา, Total Cube, Total Wgt, จำนวนชิ้น, Trip, Trip no
                                
                                # Map คอลัมน์จากไฟล์ต้นฉบับ → รูปแบบ Punthai
                                punthai_columns = []
                                
                                # 1. Sep. - ถ้ามี
                                if 'Sep.' in export_df.columns:
                                    punthai_columns.append('Sep.')
                                
                                # 2. BU - ถ้ามี
                                if 'BU' in export_df.columns:
                                    punthai_columns.append('BU')
                                
                                # 3. รหัสสาขา = Code
                                if 'Code' in export_df.columns:
                                    export_df['รหัสสาขา'] = export_df['Code']
                                    punthai_columns.append('รหัสสาขา')
                                
                                # 4. รหัส WMS - ถ้ามี
                                if 'รหัส WMS' in export_df.columns:
                                    punthai_columns.append('รหัส WMS')
                                elif 'WMS' in export_df.columns:
                                    export_df['รหัส WMS'] = export_df['WMS']
                                    punthai_columns.append('รหัส WMS')
                                
                                # 5. สาขา = Name
                                if 'Name' in export_df.columns:
                                    export_df['สาขา'] = export_df['Name']
                                    punthai_columns.append('สาขา')
                                
                                # 6. Total Cube = Cube
                                if 'Cube' in export_df.columns:
                                    export_df['Total Cube'] = export_df['Cube']
                                    punthai_columns.append('Total Cube')
                                
                                # 7. Total Wgt = Weight
                                if 'Weight' in export_df.columns:
                                    export_df['Total Wgt'] = export_df['Weight']
                                    punthai_columns.append('Total Wgt')
                                
                                # 8. จำนวนชิ้น - ถ้ามี
                                qty_cols = ['จำนวนชิ้น', 'QTY', 'Quantity', 'Original QTY']
                                for qty_col in qty_cols:
                                    if qty_col in export_df.columns:
                                        if qty_col != 'จำนวนชิ้น':
                                            export_df['จำนวนชิ้น'] = export_df[qty_col]
                                        punthai_columns.append('จำนวนชิ้น')
                                        break
                                
                                # 9. Trip
                                punthai_columns.append('Trip')
                                
                                # 10. Trip no
                                punthai_columns.append('Trip no')
                                
                                # 11. Truck (ประเภทรถ)
                                if 'Truck' in export_df.columns:
                                    punthai_columns.append('Truck')
                                
                                # เพิ่มคอลัมน์อื่นๆ ที่เหลือจากไฟล์ต้นฉบับ
                                remaining_cols = [c for c in export_df.columns if c not in punthai_columns 
                                                 and c not in ['Code', 'Name', 'Cube', 'Weight']]  # ข้ามคอลัมน์ที่ map แล้ว
                                
                                final_cols = punthai_columns + remaining_cols
                                ...
                                sort_by_col = 'Total Wgt' if 'Total Wgt' in export_df.columns else 'Weight'
                                if sort_by_col in export_df.columns:
                                    export_df = export_df.sort_values(['Trip', sort_by_col], ascending=[True, False])
                                ...
                                # เขียศ Excel - ใช้ชื่อ sheet เหมือน Punthai
