...
def simple_plan_trips(df, master_data):
    ...
    for idx, row in result_df.iterrows():
        ...
        if len(current_trip_codes) == 0:
            ...
            if all_punthai and not is_punthai:
                all_punthai = False
...
def export_to_excel_with_colors(df, filename, original_file):
    """Export ไฟล์ Excel พร้อมแยกสีตามทริป"""
    ...
    ws.cell(row=1, column=trip_col, value='Trip')
    
    # สร้างสีสำหรับแต่ละทริป (50 สีแรก)
    colors = [
        'FFE6E6', 'E6F2FF', 'E6FFE6', 'FFF9E6', 'FFE6F9',
        'E6E6FF', 'FFE6CC', 'E6FFFF', 'F9E6FF', 'FFFFE6',
        'FFD9D9', 'D9E9FF', 'D9FFD9', 'FFF4D9', 'FFD9F4',
        'D9D9FF', 'FFD9B3', 'D9FFFF', 'F4D9FF', 'FFFFD9',
        'FFCCCC', 'CCE0FF', 'CCFFCC', 'FFEFCC', 'FFCCEF',
        'CCCCFF', 'FFCC99', 'CCFFFF', 'EFCCFF', 'FFFFCC',
    ] * 2  # 60 สี
    
    # แมพ code -> trip
    code_to_trip = dict(zip(df['Code'], df['Trip']))
    
    # ระบายสี
    for row_idx in range(2, ws.max_row + 1):
        code_cell = ws.cell(row=row_idx, column=2)  # สมมติว่าคอลัมน์ B เป็น Code
        code = code_cell.value
        
        if code in code_to_trip:
            trip = code_to_trip[code]
            color = colors[(trip - 1) % len(colors)]
    ...
    wb.save(filename)
    st.success(f"✅ บันทึกไฟล์: {filename}")
