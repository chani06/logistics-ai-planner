...
def process_dataframe(df):
    ...
    print(f"[RENAMED] {list(df.columns[:10])}")
...
def learn_trip_patterns(df, exclude_hubs=True):
    ...
    # ตัด Hub nodes ออก (DC, Distribution Center)
    if exclude_hubs:
        hub_keywords = ['DC', 'PTDC', 'DISTRIBUTION']
    ...
    print(f"[INFO] Total trips: {df['Trip'].nunique()}")
    print(f"[INFO] Total branches: {df['Code'].nunique()}")
    ...
    print(f"[OK] Learned {len(trip_pairs)} pairs")
    
    return trip_pairs, trip_details

def reconstruct_trips(df, trip_pairs):
    """สร้างทริปใหม่จากโมเดล - รองรับ Hub nodes (DC, Distribution Center)"""
    ...
    print(f"\n[START] Creating trips for {len(remaining_codes)} branches")
    ...
    # ตรวจหา Hub nodes (สาขาที่มีการเชื่อมต่อมากกว่า 20 สาขา)
    hub_nodes = set()
    for code, neighbors in adjacency.items():
        if len(neighbors) > 20:  # ถ้ามีเพื่อนบ้านเยอะมาก = เป็น hub
            hub_nodes.add(code)
            print(f"[HUB] Detected hub node: {code} (connects to {len(neighbors)} branches)")
    
    # เรียงตาม connectivity (สาขาที่มีเพื่อนบ้านน้อยก่อน)
    def get_connectivity(code):
        if code not in adjacency:
            return 0
        return len([c for c, count in adjacency[code] if c in remaining_codes and c not in hub_nodes])
    
    while remaining_codes:
        # เลือกสาขาที่ไม่ใช่ hub และมี connectivity น้อยที่สุด
        non_hub_codes = remaining_codes - hub_nodes
        if non_hub_codes:
            seed = min(non_hub_codes, key=get_connectivity)
        else:
            # ถ้าเหลือแต่ hub ให้เลือก hub
        ...
        for neighbor, count in neighbors:
            if neighbor in remaining_codes:
                # ถ้าเป็น hub node ให้เพิ่มได้เสมอ
                if neighbor in hub_nodes:
                    current_trip.append(neighbor)
                    remaining_codes.remove(neighbor)
                    continue
                
                # ตรวจสอบว่า neighbor นี้เคยไปกับสมาชิกในทริปปัจจุบันหรือไม่ (ยกเว้น hub)
                can_add = True
                for member in current_trip:
                    if member in hub_nodes:
                        continue  # ข้าม hub node
        ...
        print(f"  Trip AI-{trip_id:03d}: {len(current_trip)} branches")
        trip_id += 1
    
    print(f"[OK] Created {trip_id - 1} trips")
...
def main():
    ...
    # เลือกไฟล์ที่มีชื่อยาวที่สุด (น่าจะเป็นไฟล์หลัก)
    test_file = max(files, key=lambda x: len(os.path.basename(x)))
    print(f"\n[TEST] Using file: {test_file}")
    print("="*80)
