...
def analyze_trips(df, master_df):
    ...
    for trip_num, branches in sorted(trips.items()):
        ...
        # เช็คระยะห่างระหว่างสาขาติดกัน (consecutive) ไม่ใช่ทุกคู่
        codes = [str(b['Code']) for b in branches]
        max_consecutive_dist = 0
        far_pair = None
        
        # วัดระยะระหว่างสาขาติดกัน: 1→2, 2→3, 3→4, ...
        ...
        if max_consecutive_dist > 50:  # เปลี่ยนเป็น 50km ตาม MAX_DISTANCE_IN_TRIP
            issues['far_30km'].append({
                'trip': trip_num,
                'max_dist': max_consecutive_dist,
                'branches': len(branches),
    ...
    print(f"\n  Trips > 50km consecutive: {len(issues['far_30km'])}")
