...
# ถ้ายังไม่มีคอลัมน์ที่จำเป็น ให้หาจากชื่อที่คล้ายกัน
if 'Code' not in df.columns:
    # หาคอลัมน์ที่น่าจะเป็น Code (มักอยู่หลัง BU)
    cols = df.columns.tolist()
    if 'BU' in cols:
        bu_idx = cols.index('BU')
        if bu_idx + 1 < len(cols):
            df.rename(columns={cols[bu_idx + 1]: 'Code'}, inplace=True)
