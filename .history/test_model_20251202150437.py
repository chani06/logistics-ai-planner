...
def load_historical_data(folder='Dc'):
    ...
    for file_path in files:
        try:
            ...
            # ต้องมีคอลัมน์พื้นฐาน - ยืดหยุ่นกว่าเดิม
            has_code = 'Code' in df.columns
            has_trip = 'Trip' in df.columns
            has_location = 'Latitude' in df.columns and 'Longitude' in df.columns
            
            # ถ้ามีแค่พิกัด ก็ใช้ได้ (สำหรับไฟล์ปริมาณงานต่อสาขา)
            if has_code and (has_trip or has_location):
                if has_code:
                ...
                if has_trip:
                    ...
                    # ถ้าไม่มี Trip แต่มีพิกัด ให้สร้าง Trip dummy
                    df['Trip'] = 'LOC-' + df.index.astype(str)
                ...
                if len(df) == 0 or (has_trip and df['Trip'].nunique() == 0):
                    print(f"⏭️  {os.path.basename(file_path)}: ว่างเปล่า (สำหรับทดสอบ)")
                    continue
                
                df['File'] = os.path.basename(file_path)
                df = df.reset_index(drop=True)  # Reset index เพื่อป้องกัน duplicate
                all_data.append(df)
                
                trip_count = df['Trip'].nunique() if has_trip else 0
                print(f"✅ {os.path.basename(file_path)}: {len(df)} แถว, {trip_count} ทริป")
            else:
                print(f"⚠️  {os.path.basename(file_path)}: ไม่มีคอลัมน์ที่จำเป็น (ต้องมี Code + (Trip หรือ Location))")
