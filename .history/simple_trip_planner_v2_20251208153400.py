...
def plan_trips_v2(df, master_data):
    ...
    print("\n🔍 Phase 2.6: ตรวจสอบทริปที่เกิน limit")
    for round_num in range(5):  # เพิ่มเป็น 5 รอบเพื่อความแน่ใจ
        ...
        for trip in sorted(result_df['Trip'].unique()):
            ...
            # DEBUG: แสดงทริปที่น่าสงสัย
            if trip in [22, 67, 68, 69]:
                print(f"  DEBUG Trip {trip}: {len(trip_data)} branches, {total_cubes:.2f} cubes, {truck} (limit={cube_limit})")
