...
def suggest_truck(total_weight, total_cube, max_allowed='6W', trip_codes=None):
    ...
    3. ใช้รถที่เล็กที่สุดที่ใส่ได้ (ประหยัดค่าใช้จ่าย)
    """
    ...
    # ลองจากรถเล็กไปใหญ่ เลือกคันแรกที่ใส่ได้
    ...
    # ถ้าไม่มีรถที่ใส่ได้เลย (เกินทุกรถ) → ต้องแยกทริป
    # แต่ถ้าไม่สามารถแยกได้ ใช้รถใหญ่สุด
    return '6W+'
    ...
    for trip_num in sorted(test_df['Trip'].unique()):
        ...
        if must_use_6w:
            ...
            if must_use_6w and suggested != '6W':
                suggested = '6W'
            # ✅ เช็คว่ารถที่แนะนำใส่ของได้จริงหรือไม่ (ไม่เกิน 100%)
            if suggested in LIMITS:
                can_fit = (total_w <= LIMITS[suggested]['max_w'] and 
                          total_c <= LIMITS[suggested]['max_c'])
                if not can_fit:
                    # ประวัติแนะนำรถเล็กเกินไป → ใช้ AI suggest ใหม่
                    suggested = suggest_truck(total_w, total_c, max_allowed_vehicle, trip_codes)
                    source = "🤖 AI (ประวัติไม่เหมาะสม)"
                else:
                    source = "📜 ประวัติ"
            else:
                source = "📜 ประวัติ"
