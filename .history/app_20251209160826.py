        ...
        for trip_num in test_df['Trip'].unique():
            ...
            if farthest_dist > 40 and farthest_code:
                ...
                for other_trip in test_df['Trip'].unique():
                    ...
                    # ถ้าใกล้กว่า → เช็คว่าใส่ได้ไหม
                    # ถ้าใกล้กว่า → เช็คว่าใส่ได้ไหม (ให้ priority กับจังหวัดเดียวกัน)
                    improvement_needed = 5 if same_province else 15  # จังหวัดเดียวกันต้องดีขึ้นแค่ 5km
                    
                    if dist_to_other < best_new_dist - improvement_needed:
                        # ถ้าจังหวัดเดียวกัน → ให้ priority สูงกว่า
                        if same_province and not best_same_province:
                            # เจอทริปจังหวัดเดียวกันครั้งแรก → บังคับเลือก
                            pass
                        elif not same_province and best_same_province:
                            # มีทริปจังหวัดเดียวกันอยู่แล้ว → ข้าม
                            continue
