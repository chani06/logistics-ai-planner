...
def simple_trip_assignment(df, coord_lookup, province_lookup, subdistrict_lookup):
    """จัดทริปแบบง่าย - เน้นจังหวัดเดียวกัน + ใกล้กัน + รถเหมาะสม"""
    print("\n" + "="*60)
    print("🚀 เริ่มจัดทริป (Logic ใหม่)")
    print("="*60)
    
    # กฎการจัดทริป - JB = 7 cube, ไม่ใช่ 8
    LIMITS = {
        '4W': {'max_w': 2500, 'max_c': 5.0, 'max_drops': 12},
        'JB': {'max_w': 3500, 'max_c': 7.0, 'max_drops': 7},  # Punthai = 7 drops, 7 cube
    ...
    for prov, branches in codes_by_province.items():
        ...
        while remaining:
            ...
            while i < len(remaining):
                ...
                # 🔒 กฎใหม่: เลือกรถที่เหมาะสม
                # 4W: ≤5 cube, JB: ≤7 cube, 6W: ≥18 cube
                if new_cube <= 5.0:
                    limit = LIMITS['4W']
                elif new_cube <= 7.0:
                    limit = LIMITS['JB']
                else:
                    # cube > 7 → ต้องหยุดแล้วแยกทริป
                    i += 1
                    continue
            ...
            # 🔒 เลือกรถที่เหมาะสมสำหรับทริปนี้
            if trip_cube <= 5.0 and trip_weight <= 2500:
                ...
                vehicle = '6W'
            elif trip_cube > 7.0 and trip_cube < 18.0:
                # 🔒 กฎใหม่: cube 7-18 → ควรแยกเป็น JB หลายคัน แต่ถ้าไม่ได้ → รอรวม
                vehicle = 'JB*'  # แสดงว่าควรแยก
            else:
                vehicle = '4W'
...
def analyze_trips(trips, coord_lookup):
    ...
    LIMITS = {
        ...
        'JB*': {'max_w': 3500, 'max_c': 7.0},  # เพิ่ม JB* สำหรับทริปที่ควรแยก
