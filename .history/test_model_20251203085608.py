...
def load_historical_data(folder='Dc', separate_test=True):
    ...
    for file_path in files:
        try:
            ...
            # โหลดไฟล์จริง - ใช้ data_only=True เพื่อไม่โหลด formula
            print(f"   กำลังโหลด {os.path.basename(file_path)}...")
            df = pd.read_excel(file_path, sheet_name=target_sheet, header=header_row, engine='openpyxl', data_only=True)
