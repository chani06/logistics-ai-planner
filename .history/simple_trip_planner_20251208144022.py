...
def simple_plan_trips(df, master_data):
    ...
    # เรียงลำดับ: ระยะทาง → ชื่อ → จังหวัด → อำเภอ → ตำบล
    ...
    for idx, row in result_df.iterrows():
        ...
        if len(current_trip_codes) == 0:
            ...
            # 3. เช็คว่าเกินคิวหรือไม่
            if current_trip_cubes + cube > max_cubes_4w:
                should_start_new_trip = True
            
            # 4. เช็คจำนวน drop สำหรับ JB (ถ้า punthai ล้วน ไม่เกิน 7)
            if all_punthai and len(current_trip_codes) >= 7:
                should_start_new_trip = True
...
def export_to_excel_with_colors(df, filename, original_file):
    """Export ไฟล์ Excel พร้อมแยกสีตามทริป (เหลือง-ขาว สลับกัน)"""
    
    # โหลดไฟล์ต้นฉบับ
    wb = load_workbook(original_file)
    
    # เข้าถึงชีต "2.Punthai"
    if "2.Punthai" in wb.sheetnames:
        ws = wb["2.Punthai"]
    else:
        # ถ้าไม่มีชีต ใช้ชีตแรก
        ws = wb.active
    
    # หาคอลัมน์ที่มีรหัสสาขา (คอลัมน์หลัง BU)
    # สมมติว่าแถวที่ 1 หรือ 2 เป็น header
    header_row = 1
    code_col = None
    bu_col = None
    
    # หาคอลัมน์ BU และ Code
    for col_idx in range(1, ws.max_column + 1):
        cell_value = str(ws.cell(row=header_row, column=col_idx).value).upper()
        if 'BU' in cell_value:
            bu_col = col_idx
        if bu_col and not code_col:
            # คอลัมน์ถัดจาก BU คือ Code
            code_col = col_idx + 1 if col_idx == bu_col else col_idx
    
    # ถ้าไม่เจอ ให้เดาว่าคอลัมน์ที่ 2-3
    if not code_col:
        bu_col = 1
        code_col = 2
    
    # สร้างสีเหลือง-ขาว สลับกันตามทริป
    yellow = 'FFFF99'  # เหลืองอ่อน
    white = 'FFFFFF'   # ขาว
    
    # แมพ code -> trip
    code_to_trip = dict(zip(df['Code'].astype(str), df['Trip']))
    
    # เพิ่มคอลัมน์ Trip ถ้ายังไม่มี
    trip_col = ws.max_column + 1
    ws.cell(row=header_row, column=trip_col, value='Trip')
    
    # ระบายสีตามทริป (เริ่มจากแถวที่ 2)
    current_trip = None
    use_yellow = True
    ...
    for row_idx in range(2, ws.max_row + 1):
        ...
        if code and code in code_to_trip:
            ...
            # เลือกสี
            color = yellow if use_yellow else white
            ...
            # ใส่เลขทริป
            ws.cell(row=row_idx, column=trip_col, value=trip)
