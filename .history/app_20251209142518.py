    ...
    if unassigned_codes:
        ...
        for code in unassigned_codes:
            ...
            for trip_num in test_df['Trip'].dropna().unique():
                ...
                if new_util > 100:  # เกิน 100% ไม่รับ - ห้ามเกินเด็ดขาด
                    continue
    ...
    while merged and len(all_trips) > 1 and iteration < max_iterations:
        ...
        for i, trip1 in small_trips:
            ...
            for j in candidate_indices:
                ...
                can_fit = False
                if allowed_priority >= 3 and combined_6w_util <= 100:  # 6W - ห้ามเกิน 100%
                    can_fit = True
                ...
                with tab1:
                    ...
                    if st.button("🚀 เริ่มจัดเที่ยว", type="primary", use_container_width=True):
                        with st.spinner("⏳ กำลังประมวลผล..."):
                            ...
                            # ดาวน์โหลด - เขียนทับชีต 2.Punthai ด้วยสีเหลืองโทนส้ม-ขาว
                            output = io.BytesIO()
                            ...
                            # เตรียม export DataFrame ตามโครงสร้าง 2.Punthai
                            # คอลัมน์: Sep, BU, รหัสสาขา, รหัส WMS, สาขา, Total Cube, Total Wgt, จำนวนชิ้น, Trip, Trip no, ...
                            export_rows = []
                            ...
                            for trip_num in sorted(result_df['Trip'].unique()):
                                ...
                                for _, row in trip_data.iterrows():
                                    export_rows.append({
                                        'Sep.': '',
                                        'BU': row.get('BU', 211),
                                        'รหัสสาขา': row.get('Code', ''),
                                        'รหัส WMS': row.get('Code', ''),
                                        'สาขา': row.get('Name', ''),
                                        'Total Cube': round(row.get('Cube', 0), 2) if pd.notna(row.get('Cube')) else 0,
                                        'Total Wgt': round(row.get('Weight', 0), 2) if pd.notna(row.get('Weight')) else 0,
                                        'จำนวนชิ้น': row.get('Pieces', 0) if pd.notna(row.get('Pieces')) else 0,
                                        'Trip': int(trip_num),
                                        'Trip no': trip_no,
                                        'วันที่โหลด': '',
                                        'เวลาโหลด(ประมาณ)': '',
                                        ...
                                        'WAVE': '',
                                        'remark': '',
                                        'latitude': row.get('Latitude', 0) if pd.notna(row.get('Latitude')) else 0,
                                        'longitude': row.get('Longitude', 0) if pd.notna(row.get('Longitude')) else 0,
                                        'Order': '',
                                        'Seq.': '',
                                        'Route': '',
                                        'Description': '',
                            ...
                            punthai_export = pd.DataFrame(export_rows)
                            
                            # เขียน Excel ด้วย openpyxl เพื่อสลับสีเหลืองโทนส้ม-ขาว
                            ...
                            # สีเหลืองโทนส้ม-ขาว (เหมือน simple_trip_planner_v2.py)
                            ...
                            headers = list(punthai_export.columns)
                            ...
                            # เขียนข้อมูล (แถวที่ 2 เป็นต้นไป) พร้อมสลับสี
                            current_trip = None
                            use_yellow = True
                            
                            for row_num, (_, row) in enumerate(punthai_export.iterrows(), 2):
                                trip = row['Trip']
                            ...
                            column_widths = {
                                'A': 6, 'B': 6, 'C': 12, 'D': 12, 'E': 35,
                                'F': 12, 'G': 12, 'H': 10, 'I': 6, 'J': 10,
                                'K': 12, 'L': 15, 'M': 8, 'N': 8, 'O': 15,
                                'P': 12, 'Q': 12, 'R': 8, 'S': 8, 'T': 10,
                                'U': 15, 'V': 12, 'W': 6, 'X': 12, 'Y': 10, 'Z': 15
                            }
                            for col_letter, width in column_widths.items():
                                ws.column_dimensions[col_letter].width = width
                            
                            # เพิ่มชีตสรุป
                            ws_summary = wb.create_sheet('สรุปทริป')
                            for col_num, header in enumerate(summary.columns, 1):
                                cell = ws_summary.cell(row=1, column=col_num, value=header)
                                cell.fill = header_fill
                                cell.font = header_font
                                cell.border = thin_border
                            
                            for row_num, (_, row) in enumerate(summary.iterrows(), 2):
                                for col_num, value in enumerate(row.values, 1):
