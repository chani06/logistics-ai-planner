...
def predict_trips(test_df, model_data):
    ...
    # 🔒 Final enforcement of vehicle constraints (ย้ายมาประกาศก่อนใช้)
    ...
    if 'Trip' in test_df.columns and test_df['Trip'].notna().any():
        ...
        for trip_id, group in test_df.groupby('Trip'):
            if pd.isna(trip_id):
                continue
            # แปลงเป็น string ทั้งหมดเพื่อหลีกเลี่ยง error
            codes = [str(c) for c in group['Code'].unique().tolist()]
    ...
    if use_file_trips:
        ...
        for trip_num in sorted(test_df_result['Trip'].dropna().unique()):
            ...
            if suggested in LIMITS:
                ...
                # 🚨 ถ้าเกิน 100% (ทั้งน้ำหนักหรือคิว) ต้องเพิ่มขนาดรถ
                if w_util > 100 or c_util > 100:
                    # ลองเปลี่ยนเป็นรถใหญ่ขึ้นจนกว่าจะไม่เกิน
                    upgraded = False
                    for try_vehicle in ['JB', '6W']:
                        if LIMITS[try_vehicle]['max_c'] >= LIMITS[suggested]['max_c']:
                            try_w_util = (total_w / LIMITS[try_vehicle]['max_w']) * 100
                            try_c_util = (total_c / LIMITS[try_vehicle]['max_c']) * 100
                            # ต้องไม่เกิน 100% ทั้งน้ำหนักและคิว
                            if try_w_util <= 100 and try_c_util <= 100:
                                suggested = try_vehicle
                                source = source + f" → {try_vehicle}"
                                w_util, c_util = try_w_util, try_c_util
                                upgraded = True
                                break
                    
                    # ถ้ายังเกิน 6W → ต้องแยกทริป (flag ไว้)
                    if not upgraded:
                        source = source + " (🚫 เกินต้องแยก)"
            else:
                w_util = c_util = 0
        ...
        # 🚨 Phase: Split trips that exceed 100% even with 6W
        trips_to_split = []
        for idx, row in summary_df.iterrows():
            if row['Weight_Use%'] > 100 or row['Cube_Use%'] > 100:
                trips_to_split.append(row['Trip'])
        
        if trips_to_split:
            # แยกทริปที่เกิน 100%
            new_trips = []
            max_trip_num = int(test_df_result['Trip'].max())
            
            for trip_num in trips_to_split:
                trip_mask = test_df_result['Trip'] == trip_num
                trip_data = test_df_result[trip_mask].copy()
                codes = trip_data['Code'].unique()
                
                # หาว่ารถขนาดไหนที่สาขาอนุญาต
                max_allowed = get_max_vehicle_for_trip(codes)
                target_vehicle = max_allowed if max_allowed else '6W'
                target_limit = LIMITS[target_vehicle]
                
                # แยกสาขาออกเป็นกลุ่มๆ ให้แต่ละกลุ่มไม่เกิน 100%
                sub_trips = []
                current_codes = []
                current_w = 0
                current_c = 0
                ...
                for code in codes:
                    code_data = trip_data[trip_data['Code'] == code]
                    code_w = code_data['Weight'].sum()
                    code_c = code_data['Cube'].sum()
                    
                    # ถ้าสาขาเดียวก็เกิน → ต้องยอมรับ (ไม่สามารถแบ่งได้อีก)
                    if code_w > target_limit['max_w'] or code_c > target_limit['max_c']:
                        # ใช้รถใหญ่สุด
                        if current_codes:
                            sub_trips.append(current_codes.copy())
                        sub_trips.append([code])  # สาขาเดียวที่เกิน
                        current_codes = []
                        current_w = 0
                        current_c = 0
                        continue
                    
                    # ถ้าเพิ่มสาขานี้แล้วเกิน → เริ่มทริปใหม่
                    if current_w + code_w > target_limit['max_w'] or current_c + code_c > target_limit['max_c']:
                        if current_codes:
                            sub_trips.append(current_codes.copy())
                        current_codes = [code]
                        current_w = code_w
                ...
                # เก็บกลุ่มสุดท้าย
                if current_codes:
                    sub_trips.append(current_codes)
                
                # อัปเดต Trip numbers
                if len(sub_trips) > 1:
                    # กลุ่มแรกใช้ trip_num เดิม
                    first_codes = sub_trips[0]
                    test_df_result.loc[trip_mask & test_df_result['Code'].isin(first_codes), 'Trip'] = trip_num
                    
                    # กลุ่มที่เหลือใช้ trip_num ใหม่
                    for sub_codes in sub_trips[1:]:
                        max_trip_num += 1
                        new_trip_num = max_trip_num
                        test_df_result.loc[trip_mask & test_df_result['Code'].isin(sub_codes), 'Trip'] = new_trip_num
            
            # สร้าง summary_data ใหม่หลังจากแยกทริป
            summary_data = []
            for trip_num in sorted(test_df_result['Trip'].unique()):
                trip_data = test_df_result[test_df_result['Trip'] == trip_num]
                trip_codes = trip_data['Code'].unique()
                total_w = trip_data.groupby('Code')['Weight'].first().sum()
                total_c = trip_data.groupby('Code')['Cube'].first().sum()
                
                max_allowed = get_max_vehicle_for_trip(trip_codes)
                
                # หาขนาดรถที่เหมาะสม (ไม่เกิน 100%)
                for veh in ['4W', 'JB', '6W']:
                    if max_allowed and LIMITS[veh]['max_c'] > LIMITS.get(max_allowed, {'max_c': 20})['max_c']:
                        continue  # ข้ามถ้ารถใหญ่กว่าที่อนุญาต
                    
                    w_util = (total_w / LIMITS[veh]['max_w']) * 100
                    c_util = (total_c / LIMITS[veh]['max_c']) * 100
                    
                    if w_util <= 100 and c_util <= 100:
                        suggested = veh
                        break
                else:
                    suggested = max_allowed if max_allowed else '6W'
                    w_util = (total_w / LIMITS[suggested]['max_w']) * 100
                    c_util = (total_c / LIMITS[suggested]['max_c']) * 100
    ...
    # 1. ทริปเกิน 100% → เปลี่ยนเป็นรถใหญ่ขึ้น หรือแยกออก
    # 2. ทริปต่ำกว่า MIN_UTIL → เปลี่ยนรถเล็กลง/ใหญ่ขึ้น หรือรวมกับทริปอื่น
    # ===============================================
    
    # 7.1 แก้ทริปที่เกิน 100% - ลองเปลี่ยนรถใหญ่ขึ้นก่อน ถ้าไม่ได้ค่อยแยก
    for iteration in range(10):
        ...
        for trip_num in list(test_df['Trip'].unique()):
            ...
            w_util = (trip_w / limits['max_w']) * 100
            c_util = (trip_c / limits['max_c']) * 100
            
            # เช็คทั้งน้ำหนักและคิว - ห้ามเกิน 100% ทั้งคู่!
            if w_util > 100 or c_util > 100:
        ...
        for trip_info in over_capacity_trips:
            ...
            # 🔧 วิธี 1: ลองเปลี่ยนเป็นรถใหญ่ขึ้น
            vehicle_order = ['4W', 'JB', '6W']
            current_idx = vehicle_order.index(current_vehicle) if current_vehicle in vehicle_order else 0
            ...
            # 🔧 วิธี 2: แยกทริปออกเป็น 2 คัน
            if len(trip_codes) < 2:
                continue  # ไม่สามารถแยกได้
            
            # แยกโดยดูน้ำหนักและคิว - แยกครึ่งตามจำนวน
            ...
            # กำหนดรถสำหรับทริปใหม่ (เลือกรถที่เหมาะสม)
            stay_w = test_df[test_df['Trip'] == trip_num]['Weight'].sum()
            stay_c = test_df[test_df['Trip'] == trip_num]['Cube'].sum()
            move_w = test_df[test_df['Trip'] == new_trip_num]['Weight'].sum()
            move_c = test_df[test_df['Trip'] == new_trip_num]['Cube'].sum()
            
            # หารถที่เหมาะสมสำหรับทั้งสองทริป
            for v in ['4W', 'JB', '6W']:
                v_limits = LIMITS[v]
                if stay_w <= v_limits['max_w'] and stay_c <= v_limits['max_c']:
                    trip_recommended_vehicles[trip_num] = v
                    break
            
            for v in ['4W', 'JB', '6W']:
                v_limits = LIMITS[v]
                if move_w <= v_limits['max_w'] and move_c <= v_limits['max_c']:
                    trip_recommended_vehicles[new_trip_num] = v
                    break
    ...
    for trip_num in sorted(test_df['Trip'].unique()):
        ...
        # 🔒 ตรวจสอบ MIN_UTIL และ MAX_UTIL (100%) - เช็คทั้งน้ำหนักและคิว!
        max_util = max(w_util, c_util)
        min_util_required = MIN_UTIL.get(suggested, 70)
        
        # สร้าง status - เช็คทั้ง Weight และ Cube ห้ามเกิน 100%
        if w_util > 100 or c_util > 100:
            if w_util > 100 and c_util > 100:
                status = '🚫 เกินทั้งคู่'
            elif w_util > 100:
            ...
            status = f'⚠️ ต่ำ{min_util_required:.0f}%'
        else:
            status = '✅ ผ่าน'
