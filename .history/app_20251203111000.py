...
def load_punthai_reference():
    ...
    try:
        ...
        # เรียนรู้ข้อจำกัดรถของแต่ละสาขา (123 สาขาใช้ 4W เท่านั้น, 155 สาขาต้องใช้ 6W)
        ...
        for branch_code in df_clean['BranchCode'].unique():
            ...
            if vehicles_used:
                if len(vehicles_used) == 1:
                    ...
                    branch_vehicle_restrictions[str(branch_code)] = {
                        'max_vehicle': vehicle,
                        'allowed': [vehicle]
                    }
                else:
                    # ใช้ได้หลายประเภท = ยืดหยุ่น (เลือกตามน้ำหนัก/ลูกบาศก์)
        ...
        location_stats = {
            'same_province': 0,
            'mixed_province': 0,
            'branch_restrictions': len([b for b, r in branch_vehicle_restrictions.items() if len(r['allowed']) == 1]),
            'avg_branches': 0
        }
        ...
        return {
            ...
            'branch_restrictions': branch_vehicle_restrictions
        }
    except:
        return {'patterns': {}, 'stats': {}, 'branch_restrictions': {}}
...
def check_branch_vehicle_compatibility(branch_code, vehicle_type):
    """ตรวจสอบว่าสาขานี้ใช้รถประเภทนี้ได้ไหม (เรียนรู้จาก Punthai)"""
    restrictions = PUNTHAI_PATTERNS.get('branch_restrictions', {})
    branch_code_str = str(branch_code).strip()
    
    if branch_code_str in restrictions:
        allowed = restrictions[branch_code_str].get('allowed', [])
        return vehicle_type in allowed
    return True  # ถ้าไม่มีข้อมูล = ยืดหยุ่น

def get_max_vehicle_for_branch(branch_code):
    """ดึงรถใหญ่สุดที่สาขานี้รองรับ (จาก Punthai)"""
    restrictions = PUNTHAI_PATTERNS.get('branch_restrictions', {})
    branch_code_str = str(branch_code).strip()
    
    if branch_code_str in restrictions:
        return restrictions[branch_code_str].get('max_vehicle', '6W')
    return '6W'  # ถ้าไม่มีข้อมูล = ใช้รถใหญ่ได้
