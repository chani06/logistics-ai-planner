...
# ลิมิตใหม่: 6W เต็มคิวที่ 19-20, JB ห้ามเกิน 7 คิว
LIMITS = {
    '6W': {'max_c': 20, 'max_w': 9000, 'max_drops': 20},
    'JB': {'max_c': 7, 'max_w': 3500, 'max_drops': 12},  # ห้ามเกิน 7 คิว
    '4W': {'max_c': 5, 'max_w': 1800, 'max_drops': 12}
}
...
def plan_trips_v2(df, master_data):
    ...
    while remaining_indices:
        ...
        # บริเวณไกล (>290km): ใช้ 6W (แต่ถ้าสาขาในเส้นไม่เต็ม → JB ได้)
        # บริเวณใกล้ (<150km): ใช้รถเล็ก
        # บริเวณกลาง (150-290km): ใช้ JB ห้ามเกิน 7 คิว
        if seed_distance > FAR_DC_THRESHOLD:
            # ไกล (>290km) → เริ่มด้วย JB (ถ้าสาขาไม่เยอะให้ใช้ JB ก่อน)
            # คำนวณคร่าวๆ ว่าสาขาในบริเวณนี้มีกี่คิว
            estimated_cubes = current_cubes
            for idx in remaining_indices[:20]:  # ดู 20 สาขาถัดไป
                r = result_df.iloc[idx]
                if r['Distance_DC'] > FAR_DC_THRESHOLD - 50:  # สาขาในบริเวณเดียวกัน
                    estimated_cubes += r['Cube'] if pd.notna(r['Cube']) else 0
            
            # ถ้าสาขารวมแล้วไม่เต็ม 6W → ใช้ JB
            if estimated_cubes < 15:  # น้อยกว่า 15 คิว → ใช้ JB
                truck_type = 'JB'
            else:
                truck_type = '6W'  # มากกว่า 15 คิว → ใช้ 6W
        elif seed_distance < NEAR_DC_THRESHOLD:
            # ใกล้ (<150km) → เริ่มด้วยรถเล็ก
            ...
            # ระยะกลาง (150-290km) → ใช้ JB ห้ามเกิน 7 คิว
            if current_cubes > LIMITS['JB']['max_c']:
                truck_type = '6W'  # เกิน 7 คิว → ใช้ 6W
            elif current_cubes > LIMITS['4W']['max_c']:
                truck_type = 'JB'
            else:
                truck_type = '4W'
