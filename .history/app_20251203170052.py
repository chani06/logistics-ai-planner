...
MAX_MERGE_ITERATIONS = 30  # จำกัดรอบการรวมทริป (ลดจาก 50)
...
# Cache สำหรับ haversine distance
_distance_cache = {}

def haversine_distance(lat1, lon1, lat2, lon2):
    """
    คำนวณระยะทางระหว่างจุดสองจุดบนพื้นโลก (km)
    ใช้สูตร Haversine + cache เพื่อความเร็ว
    """
    # Check cache
...
def predict_trips(test_df, model_data):
    ...
    # Pre-compute trip data เพื่อความเร็ว
    trip_numbers = [t for t in test_df['Trip'].unique() if t != 0]
    ...
    # Pre-compute สำหรับความเร็ว
    trip_list = sorted([t for t in test_df['Trip'].unique() if t != 0])
    
    for trip_num in trip_list:
        if trip_num == 0:
            continue
    ...
    # Pre-compute trip list
    trip_list_25 = sorted([t for t in test_df['Trip'].unique() if t != 0])
    
    for trip_num in trip_list_25:
        if trip_num == 0:
            continue
                ...
                with tab1:
                    ...
                    if st.button("🚀 เริ่มจัดเที่ยว", type="primary", use_container_width=True):
                        ...
                        with progress_placeholder:
                            with st.spinner("⏳ กำลังประมวลผล... (อาจใช้เวลา 30-60 วินาที)"):
