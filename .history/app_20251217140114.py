...
def predict_trips(test_df, model_data):
    ...
    if use_file_trips:
        ...
        # 🚨 เพิ่ม TripStatus สำหรับตรวจสอบว่าทริปผ่านเกณฑ์หรือไม่
        def check_trip_status(row):
            """ตรวจสอบว่าทริปผ่านเกณฑ์หรือไม่ (ห้ามเกิน 100%)"""
            trip_num = row['Trip']
            if pd.isna(trip_num):
                return '❓ ไม่มีทริป'
            ...
            # หาประเภทรถของทริป
            truck_str = str(row.get('Truck', '6W'))
            truck_type = truck_str.split()[0] if truck_str else '6W'
            if truck_type not in LIMITS:
                truck_type = '6W'
            
            # คำนวณ utilization
            limits = LIMITS[truck_type]
            w_util = (total_w / limits['max_w']) * 100
            c_util = (total_c / limits['max_c']) * 100
            max_util = max(w_util, c_util)
            ...
            # เช็ค 1: เกิน 100%?
            if max_util > 100:
                issues.append(f'⛔ เกิน {max_util:.0f}%')
            
            # เช็ค 2: รถผิดประเภท?
            max_allowed = get_max_vehicle_for_trip(set(trip_codes))
            vehicle_sizes = {'4W': 1, 'JB': 2, '6W': 3}
            if vehicle_sizes.get(truck_type, 3) > vehicle_sizes.get(max_allowed, 3):
                issues.append(f'⛔ ใช้ {truck_type} แต่จำกัดแค่ {max_allowed}')
            
            # เช็ค 3: TripValidation มีปัญหา?
            validation = str(row.get('TripValidation', '')) if pd.notna(row.get('TripValidation')) else ''
            if '⚠️' in validation:
                issues.append('⚠️ จับคู่ไม่เหมาะสม')
            
            if issues:
                return '❌ ' + ', '.join(issues)
            return '✅ ผ่าน'
        
        test_df_result['TripStatus'] = test_df_result.apply(check_trip_status, axis=1)
                ...
                with tab1:
                    ...
                    if st.session_state.processed and st.session_state.result_df is not None:
                        ...
                        try:
                            ...
                            # สีเหลืองโทนส้ม-ขาว และสีแดงสำหรับทริปที่ไม่ผ่าน
                            ...
                            red_fill_light = PatternFill(start_color='FFCCCC', end_color='FFCCCC', fill_type='solid')  # แดงอ่อน
                            red_fill_dark = PatternFill(start_color='FF9999', end_color='FF9999', fill_type='solid')  # แดงเข้ม
                            red_font = Font(color='CC0000', bold=True)  # ตัวอักษรสีแดง
                            
                            # 🔍 หาทริปที่ไม่ผ่านเกณฑ์ (เกิน 100% หรือรถผิดประเภท)
                            failed_trips = set()
                            for trip_num in result_df['Trip'].unique():
                                if trip_num == 0:
                                    continue
                                trip_data = result_df[result_df['Trip'] == trip_num]
                                if 'TripStatus' in trip_data.columns:
                                    if any('❌' in str(s) or '⛔' in str(s) for s in trip_data['TripStatus'].values if pd.notna(s)):
                                        failed_trips.add(trip_num)
                                # เช็ค utilization เกิน 100%
                                trip_summary_row = summary[summary['Trip'] == trip_num]
                                if len(trip_summary_row) > 0:
                                    w_use = trip_summary_row.iloc[0].get('Weight_Use%', 0)
                                    c_use = trip_summary_row.iloc[0].get('Cube_Use%', 0)
                                    if max(w_use, c_use) > 100:
                                        failed_trips.add(trip_num)
                            ...
                            for trip_num in sorted(result_df['Trip'].unique()):
                                ...
                                # 🚨 ถ้าทริปไม่ผ่านเกณฑ์ → ใช้สีแดง
                                is_failed_trip = trip_num in failed_trips
                                if is_failed_trip:
                                    fill = red_fill_light if use_yellow else red_fill_dark
                                else:
                            ...
                            white_fill = PatternFill(start_color='FFFFFF', end_color='FFFFFF', fill_type='solid')
                            # 🚨 สีแดงสำหรับทริปที่ไม่ผ่านเกณฑ์
                            red_fill_light = PatternFill(start_color='FFCCCC', end_color='FFCCCC', fill_type='solid')
                            red_fill_dark = PatternFill(start_color='FF9999', end_color='FF9999', fill_type='solid')
