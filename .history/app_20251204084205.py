...
def predict_trips(test_df, model_data):
    ...
    # หมายเหตุ: 6W ไม่จำกัด Cube - ข้ามการตรวจสอบ
    cube_split_count = 0
    ...
    for trip_num in sorted(test_df['Trip'].unique()):
        ...
        if current_vehicle == '4W':
            ...
            # 🚛 6W ไม่จำกัด Cube - เช็คเฉพาะจำนวนสาขา ≤18
            # ถ้าเกิน 18 สาขา → แยก
            if len(trip_data) > 18:
                should_split = True
                target_vehicle = '6W'
    ...
    # หมายเหตุ: 6W ไม่จำกัด Cube - เช็คเฉพาะจำนวนสาขา ≤18
    # ข้าม Phase 3 เพราะ 6W ไม่มีข้อจำกัด Cube
    split_count = 0
    
    # หาทริปที่ใช้ 6W และเกิน 18 สาขา (ถ้ามี)
    trips_to_check = []
    for trip_num in test_df['Trip'].unique():
        if trip_num == 0:
            continue
        ...
        # 🚛 6W ไม่จำกัด Cube - เช็คเฉพาะจำนวนสาขา
        # ถ้าเกิน 18 สาขา → ต้องแยก
        if len(trip_data) > 18:
            total_w = trip_data['Weight'].sum()
            total_c = trip_data['Cube'].sum()
            ...
            trips_to_check.append({
                'trip': trip_num,
                'data': trip_data,
                'cube_util': 0,  # ไม่ใช้ Cube % อีกต่อไป
                'total_w': total_w,
                'total_c': total_c,
                'force_split': True  # บังคับแยกเพราะเกิน 18 สาขา
            })
    
    # แยกทริปที่เกิน 18 สาขา (บังคับแยก)
