...
def simple_plan_trips(df, master_data):
    ...
    for idx, row in result_df.iterrows():
        ...
        if len(current_trip_codes) == 0:
            ...
            # ถ้าทริปปัจจุบันเกิน 5 คิวแล้ว = เป็น 6W แล้ว (ไม่ตัดทริปอีก)
            # แต่ถ้ายังไม่เกิน 5 คิว และจะเกิน 4W limit = ต้องตัดทริป
            is_already_6w = current_trip_cubes > 5
            will_exceed_4w = (current_trip_cubes + cube) > max_cubes_4w
            
            if not is_already_6w and will_exceed_4w:
                should_start_new_trip = True
            
            # 4. เช็คจำนวน drop สำหรับ JB (ถ้า punthai ล้วน ไม่เกิน 7 และยังเป็น 4W/JB)
            if not is_already_6w and all_punthai and len(current_trip_codes) >= 7:
                should_start_new_trip = True
