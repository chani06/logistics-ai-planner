...
def predict_trips(test_df, model_data):
    ...
    # 🔒 ฟังก์ชันเช็คระยะทางจากสาขาใหม่ไปยังทุกสาขาในทริป
    def check_distance_to_all_trip_branches(new_code, trip_codes, max_dist=40):
        """
        เช็คว่าสาขาใหม่ใกล้กับทุกสาขาในทริปหรือไม่
    ...
    def find_closest_trip_for_branch(branch_code, all_trip_codes_dict, exclude_trip=None):
        """
        หาทริปที่เหมาะสมที่สุดสำหรับสาขา
        all_trip_codes_dict: {trip_num: [codes]}
        คืนค่า: (best_trip_num, avg_distance) หรือ (None, 9999)
        """
        ...
        for trip_num, codes in all_trip_codes_dict.items():
            ...
            # คำนวณระยะทางเฉลี่ยจากสาขานี้ไปยังทุกสาขาในทริป
            distances = []
            for code in codes:
                code_lat, code_lon = coord_cache.get(code, (None, None))
                if code_lat:
    ...
    # ===============================================
    # 🔒 Post-processing: สลับสาขาให้อยู่ทริปที่ใกล้กันที่สุด
    # ===============================================
    def optimize_branch_placement():
        """สลับสาขาระหว่างทริปให้อยู่กับกลุ่มที่ใกล้กันที่สุด"""
        swapped = True
        max_swap_iterations = 3  # จำกัดรอบสลับ
        iteration = 0
        
        while swapped and iteration < max_swap_iterations:
            swapped = False
            iteration += 1
            ...
            for trip_num, codes in list(trip_codes_dict.items()):
                if len(codes) <= 1:
                    continue  # ทริปเดี่ยว ไม่ต้องเช็ค
                
                # วนทุกสาขาในทริป
                for code in codes[:]:  # ทำ copy
                    if code not in trip_codes_dict[trip_num]:
                        continue  # ถูกย้ายไปแล้ว
                    
                    # คำนวณระยะทางเฉลี่ยจากสาขานี้ไปยังสาขาอื่นในทริปปัจจุบัน
                    other_codes = [c for c in trip_codes_dict[trip_num] if c != code]
                    if not other_codes:
                        continue
                    
                    avg_dist_current, max_dist_current, _ = check_distance_to_all_trip_branches(code, other_codes, max_dist=100)
                    
                    # หาทริปที่ใกล้ที่สุดสำหรับสาขานี้
                    best_trip, best_avg_dist = find_closest_trip_for_branch(code, trip_codes_dict, exclude_trip=trip_num)
                    
                    # ถ้ามีทริปอื่นที่ใกล้กว่าอย่างมีนัยสำคัญ (> 10km ดีกว่า)
                    if best_trip and best_avg_dist < avg_dist_current - 10:
                        # เช็คว่าสาขานี้มีข้อจำกัดรถหรือไม่
                        code_max_vehicle = get_max_vehicle_for_branch(code)
                        
                        # เช็คว่าทริปเป้าหมายมีข้อจำกัดรถที่เข้ากันได้
