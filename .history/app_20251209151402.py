...
# - ถ้าผสม (Punthai + อื่น): ถ้า Cube 3-4 → ใช้ 6W ได้, ถ้าเกิน → ตัดเป็น 4W เท่านั้น
PUNTHAI_LIMITS = {
    '4W': {'max_w': 2500, 'max_c': 5.0, 'max_drops': 12},  # Punthai ล้วน: ถ้าเกิน 5 cube → ใช้ JB
    'JB': {'max_w': 3500, 'max_c': 8.0, 'max_drops': 7},   # Punthai ล้วน: ไม่เกิน 7 drop
...
# 🔒 ข้อมูลข้อจำกัดรถจาก Auto Plan (sheet info)
AUTO_PLAN_RESTRICTIONS = {}  # จะโหลดจากไฟล์ที่อัปโหลด
...
def get_punthai_vehicle_limits(trip_data, total_cube, branch_count):
    ...
    if punthai_type == 'punthai_only':
        ...
        # ผสม (Punthai + อื่น)
        if 3.0 <= total_cube <= 4.0:
            # Cube 3-4 → ใช้ 6W ได้
            return {'max_vehicle': '6W', 'max_drops': 999, 'should_split': False}
        elif total_cube > 4.0:
            # Cube เกิน 4 → ตัดเป็น 4W เท่านั้น
            return {'max_vehicle': '4W', 'max_drops': 12, 'should_split': True}
        else:
            # Cube < 3 → ใช้ 4W
            return {'max_vehicle': '4W', 'max_drops': 12, 'should_split': False}
...
def load_excel(file_content, sheet_name=None):
    ...
    try:
        ...
        # 🔒 อ่าน sheet "info" สำหรับข้อจำกัดรถ (MaxTruckType)
        load_auto_plan_restrictions(xls)
...
def load_auto_plan_restrictions(xls):
    """
    อ่านข้อมูล MaxTruckType จาก sheet "info" ในไฟล์ Auto Plan
    
    คอลัมน์ที่ใช้:
    - "Location Code" / "รหัสสถานที่": รหัสสาขา
    - "MaxTruckType" / "ประเภทรถบรรทุกใหญ่สุด": ประเภทรถสูงสุด (4W, 6W, 10W, etc.)
    """
    global AUTO_PLAN_RESTRICTIONS
    
    try:
        # หา sheet ที่ชื่อ "info"
        info_sheet = None
        for sheet_name in xls.sheet_names:
            if 'info' in sheet_name.lower():
                info_sheet = sheet_name
                break
        
        if not info_sheet:
            return  # ไม่มี sheet info
        ...
        if location_col and truck_col:
            # สร้าง mapping รหัสสาขา → ประเภทรถสูงสุด
            for _, row in df_info.iterrows():
                ...
                if code and truck_type and code != 'nan' and truck_type != 'NAN':
                    # แปลง 10W, 6WF, 4WF → รูปแบบมาตรฐาน
                    if '4W' in truck_type:
                        AUTO_PLAN_RESTRICTIONS[code] = '4W'
                    elif 'JB' in truck_type or 'JUMBO' in truck_type:
                        AUTO_PLAN_RESTRICTIONS[code] = 'JB'
                    elif '6W' in truck_type:
                        AUTO_PLAN_RESTRICTIONS[code] = '6W'
                    elif '10W' in truck_type:
                        AUTO_PLAN_RESTRICTIONS[code] = '6W'  # 10W → ใช้ 6W
                    else:
                        AUTO_PLAN_RESTRICTIONS[code] = '6W'  # default
            
            if AUTO_PLAN_RESTRICTIONS:
                st.info(f"📋 โหลดข้อจำกัดรถจาก Auto Plan: {len(AUTO_PLAN_RESTRICTIONS)} สาขา")
    
    except Exception as e:
        pass  # ไม่มี sheet info หรืออ่านไม่ได้ - ใช้ค่าเดิม
