...
def process_dataframe(df):
    ...
    # ใช้ vectorized operations เพื่อความเร็ว
    mask_to_keep = ~df['Code'].isin(EXCLUDE)
    
    if 'Name' in df.columns:
        # สร้าง mask สำหรับ Name โดยไม่ใช้ iterrows
        name_mask = df['Name'].astype(str).apply(lambda x: not any(k in x for k in EXCLUDE))
