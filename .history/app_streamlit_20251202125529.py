...
def run_prediction(df_test, G, geo, constraints, region_map):
    ...
    for cid, group in df_test.groupby('Cluster'):
        ...
        while pool:
            ...
            if cube_util < MIN_CUBE_UTILIZATION and len(pool) > 0 and drops < 15:
                # สแกนหาสาขาที่ใส่เพิ่มได้โดยไม่เกินขีดจำกัด
                # ใช้ while loop และตรวจสอบ index ที่ถูกต้อง
                i = 0
                while i < len(pool) and cube_util < MIN_CUBE_UTILIZATION and drops < 15:
                    cand = pool[i]
                    ...
                    if new_w_test <= 5800 and new_c_test <= 22.0 * FLEX_CUBE_UTILIZATION:
                        ...
                        if STRICT_ZONE_MODE:
                            ...
                            if can_add and not is_same_zone(seed['Code'], cand['Code'], region_map, geo):
                                can_add = False
