...
MAX_DISTANCE_IN_TRIP = 100  # km - สาขาในทริปเดียวกันห้ามห่างกันเกิน 100km
...
def calculate_optimal_vehicle_split(total_weight, total_cube, max_allowed='6W', branch_count=0):
    ...
    if max_priority >= 3:
        ...
        if total_cube >= 18.0:
            ...
            # 6W ว่างมาก (<8 cube) → ลดเป็น JB หรือ 4W
...
def predict_trips(test_df, model_data):
    ...
    if time.time() - start_time > 55:
        ...
        for trip_num in test_df['Trip'].unique():
            ...
            # 🚛 ภาคใต้ทั้งหมด → บังคับใช้ 6W เท่านั้น (ถ้าได้ ≥18 cube)
                ...
                # 🔒 6W ไม่ถึง 18 cube (8-18) → แยกเป็น JB แทน
                elif total_c >= 8.0:
    ...
    # 🚨 Phase 5: Distance Optimization - สลับสาขาให้ใกล้กันมากขึ้น
    # ตรวจสอบระยะห่างในทริป ถ้าเกิน MAX_DISTANCE_IN_TRIP → หาสาขาที่ใกล้กว่าจากทริปอื่นมาสลับ
    distance_swaps = 0
    max_distance_swaps = 100  # เพิ่มจำนวนการสลับ
    ...
    def get_province_for_code(code):
        if not MASTER_DATA.empty:
            master = MASTER_DATA[MASTER_DATA['Plan Code'] == code]
            if len(master) > 0:
                return master.iloc[0].get('จังหวัด', '')
        return ''
    
    def calculate_location_bonus(code1_loc, trip_locations):
        """
        คำนวณ bonus ตามความใกล้เคียงของพื้นที่
        - ตำบลเดียวกัน: +15km
        - อำเภอเดียวกัน: +10km  
        - จังหวัดเดียวกัน: +5km
        """
        ...
        for loc in trip_locations:
            subdistrict2, district2, province2 = loc
            
            # ตำบลเดียวกัน + จังหวัดเดียวกัน
            if subdistrict1 and subdistrict1 == subdistrict2 and province1 == province2:
                best_bonus = max(best_bonus, 15)
            # อำเภอเดียวกัน + จังหวัดเดียวกัน
            elif district1 and district1 == district2 and province1 == province2:
    ...
    for iteration in range(5):  # เพิ่มเป็น 5 รอบ
        if distance_swaps >= max_distance_swaps:
            break
        ...
        for trip_num in test_df['Trip'].unique():
            ...
            if farthest_dist > 40 and farthest_code:
                ...
                for other_trip in test_df['Trip'].unique():
                    ...
                    # 🔒 เช็คระยะทางจริง - ไม่ใช่แค่จังหวัด
                    # บางทีต่างจังหวัดอาจใกล้กว่าในจังหวัดเดียวกัน
                    # ใช้ระยะทางเป็นหลัก + bonus ถ้าจังหวัดเดียวกัน
                    province_bonus = 5 if same_province else 0  # จังหวัดเดียวกันได้ bonus 5km
                    effective_dist = dist_to_other - province_bonus
                    ...
                    if effective_dist < best_new_dist - 10:
                        ...
                        if other_util <= 100 and len(other_codes) < max_branches:
                            ...
                            best_same_province = same_province
