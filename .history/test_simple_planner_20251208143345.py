...
print(f"\n📂 โหลดไฟล์: {input_file}")
df = pd.read_excel(input_file, header=0)  # อ่านแถวแรกเป็น header
print(f"✅ โหลดสำเร็จ: {len(df)} รายการ")

# ตรวจสอบคอลัมน์
print(f"📋 คอลัมน์ก่อนแก้: {df.columns.tolist()}")

# ลองอ่านใหม่ถ้าคอลัมน์ไม่ถูก
if 'Code' not in df.columns and 'Unnamed: 1' in df.columns:
    # อาจจะมีหลายแถว header
    df = pd.read_excel(input_file, skiprows=0)
    # หรือใช้แถวที่ 1 เป็น header
    if df.iloc[0].astype(str).str.contains('STORERKEY|สาขา').any():
