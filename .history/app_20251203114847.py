...
def predict_trips(test_df, model_data):
    ...
    for trip_num in sorted(test_df['Trip'].unique()):
        ...
        # เช็คว่ามีสาขาที่ต้องใช้ 6W เพราะอยู่ไกลหรือไม่ (แต่ต้องไม่ขัดกับข้อจำกัดสาขา)
        must_use_6w = False
        for code in trip_codes:
            required_vehicle, distance = get_required_vehicle_by_distance(code)
            # ใช้ 6W เฉพาะเมื่อสาขาสามารถใช้ได้ (ไม่จำกัดเป็น 4W/JB)
            if required_vehicle == '6W' and min_max_size >= 3:
                must_use_6w = True
                break
        
        # เลือกรถ: ลำดับความสำคัญ 1) ข้อจำกัดสาขา 2) ระยะทาง 3) ประวัติ 4) AI
        if min_max_size < 3:
            # มีข้อจำกัดสาขา - ต้องเคารพข้อจำกัดนี้
            suggested = max_allowed_vehicle
            source = f"🚫 จำกัดสาขา ({max_allowed_vehicle})"
        elif must_use_6w:
            # ไม่มีข้อจำกัดสาขา แต่ระยะไกล
            suggested = '6W'
            source = "📍 ระยะไกล"
