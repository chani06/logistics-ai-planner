...
def simple_plan_trips(df, master_data):
    ...
    for idx, row in result_df.iterrows():
        code = row['Code']
        cube = row['Cube'] if pd.notna(row['Cube']) and row['Cube'] > 0 else 0.1  # ถ้า cube = 0 ให้เป็น 0.1
        ...
        if len(current_trip_codes) == 0:
            ...
            # ถ้าทริปปัจจุบันรวมกันแล้วเกิน 5 คิว = ใช้ 6W (ไม่ตัดทริป)
            # แต่ถ้ายังไม่เกิน 5 คิว = ใช้ 4W/JB (ต้องตัดทริปตามขีดจำกัด)
            will_exceed_6w_limit = (current_trip_cubes + cube) > 10  # สมมติ 6W ไม่เกิน 10 คิว
            will_exceed_4w_limit = (current_trip_cubes + cube) > max_cubes_4w
            
            if will_exceed_4w_limit and not will_exceed_6w_limit:
                # เกิน 4W แต่ไม่เกิน 6W = ตัดทริป
                should_start_new_trip = True
            
            # 4. เช็คจำนวน drop สำหรับ JB (ถ้า punthai ล้วน ไม่เกิน 7)
            if all_punthai and len(current_trip_codes) >= 7 and current_trip_cubes <= 5:
                should_start_new_trip = True
