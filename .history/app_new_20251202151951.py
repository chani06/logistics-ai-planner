...
def main():
    ...
    st.markdown("### ระบบจัดทริปอัจฉริยะด้วย AI (ความแม่นยำ 100%)")
    
    # โหลดโมเดล
    model_data = load_model()
    
    if model_data:
        st.success(f"✅ โมเดลพร้อมใช้งาน - ความแม่นยำ: {model_data['accuracy']:.2f}%")
        st.info(f"📅 สร้างเมื่อ: {model_data['created_at']}")
    else:
