...
LIMITS = {
    '4W': {'max_w': 2500, 'max_c': 5.0},   # 4W: ไม่เกิน 12 จุด, Cube ไม่เกิน 5
    'JB': {'max_w': 3500, 'max_c': 8.0},   # JB: ไม่เกิน 12 จุด, Cube ไม่เกิน 8
    '6W': {'max_w': 5500, 'max_c': 20.0}   # 6W: ไม่จำกัดจุด, Cube ต้องเต็ม
}
...
def calculate_optimal_vehicle_split(total_weight, total_cube, max_allowed='6W', branch_count=0):
    """
    🚛 คำนวณการแบ่งรถที่เหมาะสม - เป้าหมาย Utilization 99-100%
    
    ลำดับการเลือก:
    1. 4W (≤12 จุด, util 99-105%)
    2. JB (≤12 จุด, util 99-105%)
    3. JB + 4W (แยกเป็น 2 คัน)
    4. JB + JB (แยกเป็น 2 คัน)
    5. 6W + JB (แยกเป็น 2 คัน)
    6. 4W + 4W (แยกเป็น 2 คัน)
    7. 6W (ไม่จำกัดจุด แต่ cube ต้อง ≥100%)
    ...
    # คำนวณ utilization สำหรับแต่ละรถ
    util_4w = max((total_weight / LIMITS['4W']['max_w']) * 100, (total_cube / LIMITS['4W']['max_c']) * 100)
    util_jb = max((total_weight / LIMITS['JB']['max_w']) * 100, (total_cube / LIMITS['JB']['max_c']) * 100)
    util_6w = max((total_weight / LIMITS['6W']['max_w']) * 100, (total_cube / LIMITS['6W']['max_c']) * 100)
    
    # 🎯 เป้าหมาย: Utilization 99-105%
    TARGET_MIN = 99
    TARGET_MAX = 105
    MAX_UTIL = 130  # เกินกว่านี้ต้องแยก
    
    # ตรวจสอบจำนวนสาขา (4W/JB ไม่เกิน 12 จุด)
    branch_ok_for_small = branch_count <= 12 or branch_count == 0
    
    # 1. ลอง 4W ก่อน (ถ้าข้อจำกัดอนุญาต)
    if max_priority >= 1 and util_4w <= MAX_UTIL and branch_ok_for_small:
        if TARGET_MIN <= util_4w <= TARGET_MAX:
            return ('4W', False, None)
        elif util_4w < TARGET_MIN:
            # น้อยเกินไป แต่ยังใช้คันเดียวได้
            return ('4W', False, None)
    
    # 2. ลอง JB (ถ้าข้อจำกัดอนุญาต)
    if max_priority >= 2 and util_jb <= MAX_UTIL and branch_ok_for_small:
        if TARGET_MIN <= util_jb <= TARGET_MAX:
            return ('JB', False, None)
        elif util_jb < TARGET_MIN and util_4w > TARGET_MAX:
            # JB ไม่เต็ม แต่ 4W เกิน
            return ('JB', False, None)
    
    # 3. ถ้ารถเดียวไม่พอ ต้องแยก
    if util_jb > MAX_UTIL or (not branch_ok_for_small and max_priority <= 2):
        # ลองแบบต่างๆ ตามลำดับ
        
        # JB + 4W
        if max_priority >= 2:
            jb_share = 0.6  # JB รับ 60%
            util_jb_split = util_jb * jb_share
            util_4w_split = util_4w * (1 - jb_share)
            if TARGET_MIN <= util_jb_split <= TARGET_MAX or TARGET_MIN <= util_4w_split <= TARGET_MAX:
                return ('JB', True, {'split': ['JB', '4W'], 'ratio': [0.6, 0.4]})
        
        # JB + JB
        if max_priority >= 2:
            util_jb_half = util_jb * 0.5
            if TARGET_MIN <= util_jb_half <= TARGET_MAX:
                return ('JB', True, {'split': ['JB', 'JB'], 'ratio': [0.5, 0.5]})
        
        # 6W + JB (ถ้าอนุญาต 6W)
        if max_priority >= 3:
            return ('6W', True, {'split': ['6W', 'JB'], 'ratio': [0.7, 0.3]})
        
        # 4W + 4W (ถ้าจำกัดแค่ 4W)
        if max_priority == 1:
    ...
    if max_priority >= 3:
        if util_6w >= 100:
            ...
            # 6W ไม่เต็ม แต่ยังพอได้
            return ('6W', False, None)
        else:
            # 6W ว่างมาก → ลดเป็น JB
            if util_jb <= MAX_UTIL and branch_ok_for_small:
                return ('JB', False, None)
