...
def predict_trips(test_df, model_data):
    ...
    for trip_num in test_df['Trip'].unique():
        ...
        # 🔒 ถ้าไม่มีข้อมูลจังหวัด → ให้ถือว่าเป็น nearby (ใช้ 4W เป็นค่าเริ่มต้น)
        all_nearby = all(get_region_type(p) == 'nearby' for p in provinces) if provinces else True
        ...
        # ⚠️ สำคัญ: ถ้าทุกจังหวัดเป็น nearby หรือไม่มีข้อมูล → ห้าม 6W เด็ดขาด! ใช้ 4W
        if all_nearby:
            very_far = False  # บังคับให้ไม่ใช้ 6W
            if max_allowed == '6W':
                max_allowed = '4W'  # 🔒 บังคับเป็น 4W (ไม่ใช่ JB)
            elif max_allowed == 'JB':
                max_allowed = '4W'  # ถ้ามีข้อจำกัดเป็น JB ก็ลดเป็น 4W ในปริมณฑล
        else:
