...
def plan_trips_v2(df, master_data):
    ...
    while remaining_indices:
        ...
        for idx in remaining_indices[:]:
            ...
            # ถ้าต่างภูมิภาค → ไม่รวม (ยกเว้น CENTRAL กับ UNKNOWN)
    ...
    # Phase 3.75: เรียงลำดับทริปใหม่ให้ต่อเนื่อง (1, 2, 3, ...)
    unique_trips = sorted([t for t in result_df['Trip'].unique() if t != 0])
    trip_mapping = {old_trip: new_trip for new_trip, old_trip in enumerate(unique_trips, start=1)}
    result_df['Trip'] = result_df['Trip'].map(lambda x: trip_mapping.get(x, x))
    ...
    # Phase 5: สร้าง Trip_No (6W001, 4WJ001, 4W001)
    def generate_trip_no(truck_type, is_punthai):
        """สร้างรหัสทริปตามประเภทรถ"""
        if truck_type == '6W':
            prefix = '6W'
        elif truck_type == 'JB':
            prefix = '4WJ'
    ...
    # สร้าง Trip_No สำหรับแต่ละทริป
    trip_no_map = {}
    counters = {'6W': 1, '4WJ': 1, '4W': 1}
    ...
    for trip in sorted(result_df['Trip'].unique()):
        ...
        prefix = generate_trip_no(truck, all_punthai)
        trip_no = f"{prefix}{counters[prefix]:03d}"
        trip_no_map[trip] = trip_no
        counters[prefix] += 1
    
    # เพิ่มคอลัมน์ Trip_No
    result_df['Trip_No'] = result_df['Trip'].map(trip_no_map)
...
def export_with_colors(result_df, output_file, original_file, sheet_name="2.Punthai"):
    """Export กลับไฟล์เดิมพร้อมสีเหลืองเข้ม-ส้มอ่อน และ Trip_No"""
    ...
    # สีใหม่: เหลืองเข้ม-ส้มอ่อน
    yellow_dark = PatternFill(start_color='FFD700', end_color='FFD700', fill_type='solid')
    orange_light = PatternFill(start_color='FFE4B5', end_color='FFE4B5', fill_type='solid')
    
    code_col = 3      # Code column
    trip_col = 9      # Trip column
    trip_no_col = 10  # Trip_No column (NEW)
    
    # สร้าง mapping สำหรับ Trip และ Trip_No
    code_to_trip = dict(zip(result_df['Code'].astype(str), result_df['Trip']))
    code_to_trip_no = dict(zip(result_df['Code'].astype(str), result_df['Trip_No']))
    
    current_trip = None
    use_yellow_dark = True
    ...
    for row_idx in range(3, ws.max_row + 1):
        ...
        if code and code in code_to_trip:
            ...
            trip_no = code_to_trip_no.get(code, '')
            
            if current_trip != trip:
                current_trip = trip
                use_yellow_dark = not use_yellow_dark
            
            # สลับสีเหลืองเข้ม-ส้มอ่อน
            fill = yellow_dark if use_yellow_dark else orange_light
            for col in range(1, ws.max_column + 1):
                ws.cell(row=row_idx, column=col).fill = fill
            
            # เขียน Trip และ Trip_No
            ws.cell(row=row_idx, column=trip_col, value=int(trip))
            ws.cell(row=row_idx, column=trip_no_col, value=trip_no)
