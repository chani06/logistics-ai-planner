                ...
                if split_needed:
                    ...
                    if current_group:
                        ...
                        if final_util < 95 and len(new_trips) > 0:
                            ...
                            # วิธี 1: ลองรวมกับทริปก่อนหน้า (ถ้าไม่เกิน 140%)
                            last_group = new_trips[-1]
                            last_group_w = sum(test_df[test_df['Code'] == c]['Weight'].sum() for c in last_group)
                            last_group_c = sum(test_df[test_df['Code'] == c]['Cube'].sum() for c in last_group)
                            combined_w = current_group_w + last_group_w
                            combined_c = current_group_c + last_group_c
                            ...
                            # วิธี 2: ถ้ารวมไม่ได้ → กระจายสาขาไปทริปอื่นๆ
                            if not merged:
                                for code in current_group:
                                    ...
                                    for idx, trip_codes in enumerate(new_trips):
                                        ...
                                        # เลือกทริปที่เพิ่มแล้วไม่เกิน 140% และใกล้ 100% ที่สุด
                                        if new_util <= 140 and abs(new_util - 100) < abs(min_util_after - 100):
                                            min_util_after = new_util
                                            best_trip_idx = idx
                                    
                                    if best_trip_idx >= 0:
                                        new_trips[best_trip_idx].append(code)
                                    else:
                                        # ไม่มีทริปไหนรองรับได้ → สร้างทริปใหม่ (ยอมรับน้อยกว่า 95%)
                    ...
                    # ถ้ามีทริปไหนน้อยกว่า 95% ให้รวมกับทริปอื่น
                    final_trips = []
                    ...
                    for code in low_util_branches:
                        ...
                        for idx, trip_codes in enumerate(final_trips):
                            ...
                            if new_util <= 140 and new_util < best_util:
                                best_util = new_util
                                best_trip_idx = idx
