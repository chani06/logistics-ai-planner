...
def predict_trips(test_df, model_data):
    ...
    # ===============================================
    # 🎯 Renumber trips: เรียงจากไกล → ใกล้ DC แล้วเริ่มจาก 1, 2, 3, ...
    # ===============================================
    
    # คำนวณระยะทางจาก DC สำหรับแต่ละทริป (ใช้จุดศูนย์กลาง/centroid ของทริป)
    trip_distances = {}
    for trip_num in test_df['Trip'].dropna().unique():
        trip_data = test_df[test_df['Trip'] == trip_num]
        lats = []
        ...
        if lats and lons:
            ...
            dist = haversine_distance(DC_WANG_NOI_LAT, DC_WANG_NOI_LON, centroid_lat, centroid_lon)
        else:
            dist = 0
        trip_distances[trip_num] = dist
    
    # เรียงทริปจากไกลไปใกล้ DC (ระยะทางมากสุดก่อน)
    sorted_trips = sorted(trip_distances.keys(), key=lambda x: -trip_distances[x])
    
    # สร้าง mapping: ทริปเดิม → ทริปใหม่ (1, 2, 3, ...)
