...
def predict_trips(test_df, model_data):
    ...
    # 🚀 **NEW ALGORITHM: จัดกลุ่มประวัติก่อน → เรียงภูมิภาค → จัดทริป**
    # 
    # ขั้นตอน:
    # 1. 📜 จัดกลุ่มสาขาตามประวัติ (trip_pairs) - สาขาที่เคยไปด้วยกันต้องอยู่ติดกัน
    # 2. 🗺️ เรียงกลุ่มตามภูมิภาค (ใต้ → เหนือ → อีสาน → กทม/ปริมณฑล)
    # 3. 🚛 จัดทริปตามลำดับที่เรียงแล้ว
    
    all_codes_original = test_df['Code'].unique().tolist()
    
    # 📜 Phase 0: จัดกลุ่มสาขาตามประวัติ (trip_pairs clustering)
    ...
    # จัดกลุ่มตามประวัติ
    history_clusters = cluster_by_history(all_codes_original, trip_pairs)
