...
def predict_trips(test_df, model_data):
    ...
    while all_codes:
        ...
        for code in remaining:
            # เช็คจำนวนสาขาก่อน - ถ้าเกิน MAX แล้วไม่เพิ่ม (ยกเว้นถ้าอยู่ในประวัติ)
            if len(current_trip) >= MAX_BRANCHES_PER_TRIP:
                pair = tuple(sorted([seed_code, code]))
                if pair not in trip_pairs:  # ถ้าไม่ได้อยู่ในประวัติ ข้ามไป
                    continue
            ...
            if pair in trip_pairs:
                ...
                # เช็คจังหวัดก่อน - ต้องอยู่กลุ่มเดียวกันหรือใกล้เคียง
                ...
                if not is_nearby_province(seed_province, code_province):
                    continue  # จังหวัดไม่ใกล้เคียงกัน ข้ามไป
                
                # ถ้าเกิน TARGET_BRANCHES แล้ว ต้องเข้มงวดมากขึ้น
                if len(current_trip) >= TARGET_BRANCHES_PER_TRIP:
                    # ต้องเป็นจังหวัดเดียวกันเท่านั้น
