...
def predict_trips(test_df, model_data):
    ...
    if use_file_trips:
        ...
        # 🆕 Phase: รวมทริปกลุ่มภาคเดียวกันที่ยัง capacity ไม่เต็ม
        def merge_same_region_group_trips(df):
            """รวมทริปที่อยู่กลุ่มภาคเดียวกันและยังมี capacity เหลือ"""
            ...
            for trip_num in df['Trip'].dropna().unique():
                ...
                # หา region_group หลัก
                prov_groups = {}
                for code in codes:
                    prov = get_province_from_df(df, code)
                    if prov:
                        rg = get_region_group_local(prov)
                        if rg:
                            prov_groups[rg] = prov_groups.get(rg, 0) + 1
                
                main_rg = max(prov_groups.items(), key=lambda x: x[1])[0] if prov_groups else None
                
                trip_info[trip_num] = {
                    'codes': codes,
                    'cube': total_c,
                    'weight': total_w,
                    'region_group': main_rg,
                    'count': len(codes)
                }
            
            # รวมทริปกลุ่มภาคเดียวกัน
            merged_trips = set()
            for trip1, info1 in list(trip_info.items()):
                if trip1 in merged_trips or info1['region_group'] is None:
                    continue
                
                for trip2, info2 in list(trip_info.items()):
                    if trip2 <= trip1 or trip2 in merged_trips:
                        continue
                    if info2['region_group'] != info1['region_group']:
                        continue
        ...
        test_df_result = merge_same_region_group_trips(test_df_result)
    ...
    # 🆕 ฟังก์ชันหากลุ่มภาค (ต้องนิยามก่อนใช้)
