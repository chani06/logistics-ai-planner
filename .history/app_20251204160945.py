...
def is_similar_name(name1, name2, similarity_threshold=85):
    ...
    def extract_keywords(name):
        ...
        return keywords, thai_chars, eng_chars, s
    ...
    if keywords1 and keywords2:
        ...
        # ✅ Case 1: ชื่อพื้นฐานเดียวกัน (เช่น "คลองหลวง" ใน คลองหลวง3, คลองหลวง4)
        base_names = {k for k in common_keywords if len(k) >= 3 and not k.isdigit()}
        if base_names:
            # ถือว่าเป็นสาขาเดียวกัน (เช่น คลองหลวง 3, 4, 8, 10)
            return True
        ...
        for i, code1 in enumerate(codes):
            ...
            for j, code2 in enumerate(codes):
                ...
                if is_similar_name(name1, name2, similarity_threshold=80):
                    # เช็คระยะทาง - ต้องไม่ไกลเกินไป (< 50km)
                    lat1, lon1 = get_lat_lon_from_master(code1)
                    lat2, lon2 = get_lat_lon_from_master(code2)
                    
                    if lat1 and lat2:
                        dist = haversine_distance(lat1, lon1, lat2, lon2)
                        if dist < 50:  # ยอมให้ไกลหน่อย เพราะสาขาชื่อเดียวกันอาจกระจาย
